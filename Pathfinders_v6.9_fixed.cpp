// Pathfinders_v6_8_unified.cpp
// Single-file SDL2 + SDL_ttf sandbox + mission testbed
// Includes: AI squads, A* pathfinding, foliage transparency,
// mission params panel with buttons, faction toggles, respawn
// waves, door-carved prefabs, and per-mission squad compositions.
...
namespace cfg {
    ...
    constexpr float PlayerWalkSpeed = 70.0f; // was faster //
    constexpr float PlayerSprintSpeed = 120.0f; // was faster //
    constexpr float PlayerSneakSpeed = 40.0f; // new: sneak = slow & quiet //

    constexpr float BulletSpeed = 520.0f;
    constexpr float BulletMaxRange = 800.0f;

    // AI movement
    constexpr float AIWalkSpeed = 60.0f;
    constexpr float AISprintSpeed = 100.0f;
...
enum class SquadMode : uint8_t {
    Calm = 0,       // holding positions around anchor
    PatrolSweep,    // moving between checkpoints
    Contact,        // active threat / firefight
    Defensive,      // taking cover when under fire but no LOS
...
static std::string compass8(const Vec2& from, const Vec2& to) {
    ...
    float a = std::atan2(d.y, d.x); // -pi..pi (y grows down)
    ...
    if (playerPresent) {
        ...
        if (labelsEnabled) {
            std::string lab = "ALLY HP " + std::to_string(player.hp) +
                " M" + std::to_string(player.gun.inMag);
    ...
    for (int idx = 0; idx < (int)actors.size(); ++idx) {
        ...
        if (labelsEnabled) {
            ...
            std::string lab = std::string(tn) + " HP " +
                std::to_string(e.hp) + " M" +
                std::to_string(e.gun.inMag) +
                (e.isLeader ? " *" : "") +
                (e.isHVT ? " [HVT] " : " ") +
...
if (!contactNow && s.timeSinceContact > 10.0f) {
        Vec2 guardAnchor = (s.roleAnchor.x != 0.f || s.roleAnchor.y != 0.f)
            ? s.roleAnchor : s.home;
        int n = 0;
        for (int idx : s.members) {
        ...
        for (int idx : s.members) {
            ...
            float angle = (6.2831853f * (float)i) / std::max(1, n);
            float radius = 32.0f + 8.0f * (i % 3);
            Vec2 offset{ std::cos(angle) * radius, std::sin(angle) * radius };
            a.hasOrder = true;
            a.orderPos = guardAnchor + offset;
            ++i;
...
// 3) From here on, use hasThreat / states as usual...
    ...
    case AIState::Attack: {
        if (hasThreat && seesThreat) {
            ...
            if (a.gun.canFire()) {
                ...
                b.dir = normalize(to);
                b.traveled = 0.f;
                b.dmg = a.gun.bulletDamage;
                b.src = a.team;
                bullets.push_back(b);
                a.gun.inMag--;
                a.gun.fireTimer = a.gun.fireCooldownS;
    ...
    if (a.gun.inMag <= 0 && !a.gun.reloading) {
        a.gun.startReload();
    }
}
...
void Game::handleEvents() {
    ...
    while (SDL_PollEvent(&e)) {
        ...
        case SDL_KEYDOWN: {
            ...
            case SDLK_F4: visionViz = !visionViz;      break;
...
void Game::update(float dt) {
    ...
    if (playerPresent && player.alive()) {
        ...
        if (mouseDownL && mode == Mode::Player && player.gun.canFire()) {
            Vec2 dir = normalize(Vec2{ (float)worldMouseX, (float)worldMouseY } - player.pos);
            Bullet b;
            b.pos = player.pos;
            b.dir = dir;
            b.traveled = 0.f;
            b.dmg = player.gun.bulletDamage;
            b.src = player.team;
            bullets.push_back(b);
            player.gun.inMag--;
            player.gun.fireTimer = player.gun.fireCooldownS;
    ...
    // Bullets move
    const float bulletSpeed = cfg::BulletSpeed;
    const float bulletRange = cfg::BulletMaxRange;
    ...
    for (int bi = 0; bi < (int)bullets.size(); ++bi) {
        Bullet& b = bullets[bi];
        if (b.traveled > bulletRange || bulletHitsSolid(b)) {
            bulletDead[bi] = true;
            continue;
        }
