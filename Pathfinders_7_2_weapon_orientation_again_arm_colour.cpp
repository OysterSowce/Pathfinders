...
void Game::drawProcCharacter(const Actor& a, SDL_Color col) {
    ...
    SDL_Color armCol = rebel ? SDL_Color{ 200,200,200,255 } : SDL_Color{ 190,190,190,255 };
    SDL_Color headCol = rebel ? SDL_Color{ 220,220,220,255 } : SDL_Color{ 210,210,210,255 };
    ...
    // weapon base + muzzle
    Vec2 weapBase = a.pos + wf * (torsoL * 0.25f) + wr * (torsoW * 0.10f);
    float weapLen = 14.0f;
    if (activeW(a).id != WeaponId::None) {
        const WeaponDef& wd = weaponDef(activeW(a).id);
    ...
    // initial direction: roughly toward weapon
    chainInit(armR, segs, segLen, shR, wf);
    chainInit(armL, segs, segLen, shL, wf);
