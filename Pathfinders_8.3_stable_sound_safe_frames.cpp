...
static float gBusGain[(int)MixBus::COUNT] = {
    ...
    0.70f, // WeaponsDistant (later)
    0.75f  // VoiceDistant (later)
};

static inline void Audio_PrintMix() {
    SDL_Log("MIX: Master=%.2f Weap=%.2f Imp=%.2f Step=%.2f Voice=%.2f",
...
struct StepVoice : public AVoice {
    ABiquad thumpBP;
    StepVoice(float speedNorm) {
        // speedNorm 0..1 influences weight
        float thumpHz = 120.0f + speedNorm * 60.0f;
        thumpBP = ABiquad::bandpass(thumpHz, 1.2f, gAudioSR);
        T = 0.11f;
        gain = 0.24f + 0.20f * speedNorm; //much gentler
    }
    void render(float* outL, float* outR, int frames, int sr) override {
        float dt = 1.0f / sr;
        for (int i = 0; i < frames; i++) {
            if (!alive) break;
            float u = t / T;
            float env = expf(-u * 10.0f);
            float s = thumpBP.tick(arand11() * 0.9f) * env * gain;
            float l = s * (0.7071f * (1.0f - pan));
            float r = s * (0.7071f * (1.0f + pan));
            outL[i] += l; outR[i] += r;
            t += dt;
...
static void Audio_Spawn(const AudioEvent& e) {
    float pan = 0.0f, att = 1.0f;
    if (!Audio_Spatialize(e.x, e.y, e.maxAudible, pan, att)) return;

    AVoice* v = nullptr;

    // 1) Create the voice
    switch (e.type) {
    case AudioEvent::Fire:     v = new ShotVoice(e.weaponId); break;
    case AudioEvent::Reload:   v = new ReloadVoice(e.weaponId); break;
    case AudioEvent::Footstep: v = new StepVoice(e.p0); break;              // p0 = speedNorm
    ...
    case AudioEvent::Bark:     v = new BarkVoice(e.faction); break;
    default: break;
    }
...
static inline void Audio_PostFootstep(float x, float y, float speedNorm, int faction) {
    AudioEvent e; e.type = AudioEvent::Footstep; e.x = x; e.y = y; e.p0 = speedNorm; e.faction = faction;
    Audio_Post(e);
    e.maxAudible = 240.0f; // footsteps
}
...
static inline void Audio_PostBark(float x, float y, int faction) {
...
static void Audio_UpdateFootstepsFor(Actor& a, const Vec2& vel, float walkSpeed, float dt) {
    ...
    if (a.footAccS >= a.footNextS) {
        ...
        Audio_PostFootstep(a.pos.x, a.pos.y, n, (int)a.team);
        ...
        if (s.intent == SquadIntent::Flank || s.intent == SquadIntent::Advance) {
            ...
            if (frand(0.f, 1.f) < 0.03f) {
                for (int idx : s.members) {
                    ...
                    Audio_PostBark(a.pos.x, a.pos.y, (int)a.team);
