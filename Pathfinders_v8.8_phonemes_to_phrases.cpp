...
// Add to this over time; it makes intelligibility jump immediately.
...
static const LexEntry gLex[] = {
    {"GO", PH_GO, (uint8_t)std::size(PH_GO)},
    {"CONTACT", PH_CONTACT, (uint8_t)std::size(PH_CONTACT)},
    {"RELOAD", PH_RELOAD, (uint8_t)std::size(PH_RELOAD)},
    {"FLANK", PH_FLANK, (uint8_t)std::size(PH_FLANK)},
    {"LEFT", PH_LEFT, (uint8_t)std::size(PH_LEFT)},
    {"RIGHT", PH_RIGHT, (uint8_t)std::size(PH_RIGHT)},
    {"MOVE", PH_MOVE, (uint8_t)std::size(PH_MOVE)},
    {"HOLD", PH_HOLD, (uint8_t)std::size(PH_HOLD)},
    {"PUSH", PH_PUSH, (uint8_t)std::size(PH_PUSH)},
    {"RETREAT", PH_RETREAT, (uint8_t)std::size(PH_RETREAT)},
    {"ENEMY", PH_ENEMY, (uint8_t)std::size(PH_ENEMY)},
};
...
static inline void BuildSpeechTokens(AudioEvent& e, const char* text)
{
    ...
    auto flushWord = [&]() {
        ...
        float phraseT = std::min(1.0f, wordIndex / 7.0f);
        float contour = question ? (0.98f + 0.14f * phraseT) : (1.06f - 0.16f * phraseT);

        // stress: first vowel in word gets stress=1, maybe 2 if yell
        bool stressed = false;
        ...
        for (int i = 0; i < phN && e.speechN < MAX; i++) {
            ...
            float hz = base * contour * (stress ? 1.10f : 1.0f);

            // durations by class (open-air: slightly longer vowels)
            int durMs = 85;
            if (PhIsVowel(p))      durMs = (stress ? 190 : 150);  // slower vowels
            else if (PhIsStop(p))  durMs = 65;                    // stops less “clicky”
            else if (PhIsFric(p))  durMs = 110;                   // longer frics = less “ticks”
            else if (PhIsNasal(p)) durMs = 120;
            else                   durMs = 90;
        ...
        // small inter-word pause (less robotic)
        pushPauseMs(75);
    ...
    for (int i = 0; text[i] && e.speechN < MAX; ++i) {
        ...
        if (c == ',') { flushWord(); pushPauseMs(120); continue; }
        if (c == '.') { flushWord(); pushPauseMs(160); continue; }
        if (c == '!') { flushWord(); pushPauseMs(170); continue; }
        if (c == '?') { flushWord(); pushPauseMs(180); continue; }
        ...
        for (int i = 0; i < frames; i++) {
            ...
            if (p == Ph::PAU || p == Ph::SIL || tokI >= tokN) {
                ...
                if (PhIsVowel(p)) {
                    ...
                    vMix = voicedOn ? 0.30f : 0.08f;
                    nMix = nmix * 0.70f;   // soften hiss
                }

                else if (PhIsStop(p)) {
                    vMix = voicedOn ? 0.60f : 0.12f;
                    nMix = 0.22f + 0.28f * burst; // less noisy pop
                }
