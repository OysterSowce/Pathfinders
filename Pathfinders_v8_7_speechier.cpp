...
// ---------- SpeechVoice (CC2/CC3-ish procedural speech) ----------
enum class ConsKind : uint8_t { None, Plosive, Fricative, Nasal, Liquid };

static inline ConsKind cc2_kind(int cc2) {
    // Rough grouping: good enough to sound speech-like
    // Random/None
    if (cc2 <= 6 || cc2 >= 125) return ConsKind::None;

    // Plosives-ish: B/D/G/K/P/T (and clusters)
    if ((cc2 >= 4 && cc2 <= 16) ||  // B/Bj/Bl/Br
        (cc2 >= 20 && cc2 <= 26) ||  // D/Dr
        (cc2 >= 40 && cc2 <= 49) ||  // G/Gl/Gr
        (cc2 >= 56 && cc2 <= 65) ||  // K/Kl/Kr
        (cc2 >= 79 && cc2 <= 88) ||  // P/Pl/Pr
        (cc2 >= 102 && cc2 <= 108))  // T/Tr
        return ConsKind::Plosive;

    // Fricatives-ish: F/H/S/Sh/Th/V (and clusters)
    if ((cc2 >= 27 && cc2 <= 39) ||   // F/Fj/Fl/Fr
        (cc2 >= 50 && cc2 <= 55) ||   // H/Dj-ish (treat Dj as noisy)
        (cc2 >= 92 && cc2 <= 101) ||  // S/Sl/Sh
        (cc2 >= 109 && cc2 <= 114) || // Th/Thr
        (cc2 >= 115 && cc2 <= 121))   // V/W
        return ConsKind::Fricative;

    // Nasals: M/N/Nj and vowel "m"
    if ((cc2 >= 69 && cc2 <= 78)) return ConsKind::Nasal;

    // Liquids: L/R/Y etc
    if ((cc2 >= 66 && cc2 <= 72) || (cc2 >= 89 && cc2 <= 91) || (cc2 >= 122 && cc2 <= 124))
        return ConsKind::Liquid;

    return ConsKind::Fricative;
}

// CC3 vowel → formant targets (Hz). (Not exact TE values, but matches the vowel set)
static inline void cc3_formants(int cc3, float& F1, float& F2, float& F3) {
    // Default schwa
    F1 = 500; F2 = 1500; F3 = 2500;

    if (cc3 <= 7) { // random bucket
        int r = (cc3 * 37 + 13) % 8;
        cc3 = 8 + r * 12;
    }

    // Approx vowel regions
    if (cc3 <= 15) { F1 = 700; F2 = 1100; F3 = 2450; } // ɑː
    else if (cc3 <= 22) { F1 = 600; F2 = 1400; F3 = 2500; } // aɪ
    else if (cc3 <= 30) { F1 = 660; F2 = 1700; F3 = 2500; } // æ
    else if (cc3 <= 37) { F1 = 500; F2 = 1500; F3 = 2500; } // ə
    else if (cc3 <= 45) { F1 = 500; F2 = 900; F3 = 2400; } // ɔː
    else if (cc3 <= 52) { F1 = 520; F2 = 1100; F3 = 2400; } // ɔɪ
    else if (cc3 <= 60) { F1 = 520; F2 = 1000; F3 = 2400; } // ɒ
    else if (cc3 <= 67) { F1 = 580; F2 = 1300; F3 = 2500; } // ʌ
    else if (cc3 <= 75) { F1 = 350; F2 = 900; F3 = 2200; } // uː
    else if (cc3 <= 82) { F1 = 300; F2 = 2200; F3 = 3000; } // iː
    else if (cc3 <= 90) { F1 = 380; F2 = 1800; F3 = 2800; } // ɪə
    else if (cc3 <= 97) { F1 = 420; F2 = 2000; F3 = 2900; } // eɪ
    else if (cc3 <= 105) { F1 = 450; F2 = 1700; F3 = 2600; } // ɛə
    else if (cc3 <= 112) { F1 = 420; F2 = 1400; F3 = 2500; } // ʊə
    else if (cc3 <= 120) { F1 = 250; F2 = 1200; F3 = 2200; } // m (nasal-ish vowel)
    else { F1 = 500; F2 = 1500; F3 = 2500; } // None
}
...
static inline float frand01(uint32_t& s) { return (xorshift32(s) & 0x00FFFFFF) / float(0x00FFFFFF); }

// One speech “unit”: consonant + vowel pair
struct SpeechUnit {
    uint8_t cc2 = 0, cc3 = 0;
    float   dur = 0.10f;
    float   pitch = 240.f;
...
struct SpeechVoice : public AVoice {
    ...
    float curF1 = 500, curF2 = 1500, curF3 = 2500;
    float tgtF1 = 500, tgtF2 = 1500, tgtF3 = 2500;
    ...
    SpeechVoice(int faction_, uint32_t textHash) {
        faction = faction_;
        seed = (textHash ? textHash : 0xC0FFEEu) ^ (uint32_t)faction * 0x9E3779B9u;

        // faction profile (simple but effective)
        float basePitch = 250.0f;
        float formMul = 1.0f;
        float grit = 0.25f;

        // You can adjust these to match your enum ordering
        // 0 Axis deep/stern, 1 Allies refined, 2 Rebels gravelly, 3 Militia dopey/high
        if (faction == 0) { basePitch = 205.f; formMul = 0.92f; grit = 0.18f; }
        else if (faction == 1) { basePitch = 255.f; formMul = 1.04f; grit = 0.16f; }
        else if (faction == 2) { basePitch = 235.f; formMul = 0.98f; grit = 0.33f; }
        else if (faction == 3) { basePitch = 295.f; formMul = 1.08f; grit = 0.22f; }

        // Build 4–9 units deterministically from the bark hash
        int n = 4 + int(frand01(seed) * 6.0f);

        for (int i = 0; i < n; i++) {
            SpeechUnit u;

            // CC2 consonant: use your “known range” buckets
            int cc2 = int(frand01(seed) * 128.0f);
            if (cc2 < 4) cc2 = 0; // allow random bucket sometimes
            u.cc2 = (uint8_t)cc2;

            // CC3 vowel
            int cc3 = int(frand01(seed) * 128.0f);
            if (cc3 < 8) cc3 = 0; // random bucket sometimes
            u.cc3 = (uint8_t)cc3;

            // duration + stress
            u.dur = 0.07f + frand01(seed) * 0.09f;
            u.stressed = (frand01(seed) > 0.72f);

            // pitch contour
            float p = basePitch * (0.92f + 0.16f * frand01(seed));
            if (u.stressed) p *= 1.08f;
            u.pitch = p;

            units.push_back(u);
        }

        // Total length: sum of units + tiny gaps
        T = 0.0f;
        for (auto& u : units) T += u.dur;
        T += 0.06f; // tail

        // gain (use mixer to tune overall)
        gain = 0.26f;

        // init formants
        cc3_formants(units[0].cc3, tgtF1, tgtF2, tgtF3);
        tgtF1 *= formMul; tgtF2 *= formMul; tgtF3 *= formMul;
        curF1 = tgtF1; curF2 = tgtF2; curF3 = tgtF3;

        f1 = ABiquad::bandpass(curF1, 8.0f, gAudioSR);
        f2 = ABiquad::bandpass(curF2, 9.0f, gAudioSR);
        f3 = ABiquad::bandpass(curF3, 10.0f, gAudioSR);

        // store grit in p0 via seed (cheap): we’ll derive it each sample from faction
        // (kept simple—grit affects noise mix below)
    }
    ...
    void render(float* outL, float* outR, int frames, int sr) override {
        ...
        for (int i = 0; i < frames; i++) {
            ...
            // step units
            if (unitIdx < (int)units.size()) {
                unitDur = units[unitIdx].dur;

                // enter unit (update targets)
                if (unitT <= 0.00001f) {
                    float F1, F2, F3;
                    cc3_formants(units[unitIdx].cc3, F1, F2, F3);

                    float formMul = 1.0f;
                    if (faction == 0) formMul = 0.92f;
                    else if (faction == 1) formMul = 1.04f;
                    else if (faction == 2) formMul = 0.98f;
                    else if (faction == 3) formMul = 1.08f;
            ...
            // smooth formants
            float a = 0.06f; // smoothing factor
            curF1 += (tgtF1 - curF1) * a;
            curF2 += (tgtF2 - curF2) * a;
            curF3 += (tgtF3 - curF3) * a;

            // rebuild biquads occasionally (cheap / stable)
            if ((i & 7) == 0) {
                f1 = ABiquad::bandpass(curF1, 8.0f, gAudioSR);
                f2 = ABiquad::bandpass(curF2, 9.0f, gAudioSR);
                f3 = ABiquad::bandpass(curF3, 10.0f, gAudioSR);
            ...
            // unit envelope
            float uGlobal = (T > 0.001f) ? (t / T) : 1.0f;

            // global env
            float gEnvA = (uGlobal < 0.06f) ? (uGlobal / 0.06f) : 1.0f;
            float gEnvD = expf(-uGlobal * 1.4f);
            float gEnv = gEnvA * gEnvD;

            float local = 0.0f;
            ConsKind ck = ConsKind::None;
            float pitch = 250.0f;

            if (unitIdx < (int)units.size()) {
                SpeechUnit& U = units[unitIdx];
                pitch = U.pitch;

                float ul = unitT / std::max(0.001f, unitDur);
                float aAtk = 0.18f;
                float aRel = 0.30f;

                float env = 1.0f;
                if (ul < aAtk) env = ul / aAtk;
                else if (ul > (1.0f - aRel)) env = std::max(0.0f, (1.0f - ul) / aRel);

                ck = cc2_kind((int)U.cc2);

                // consonant burst at unit start (plosives/fricatives)
                float burst = 0.0f;
                if (ck == ConsKind::Plosive && ul < 0.12f) burst = (1.0f - ul / 0.12f);
                if (ck == ConsKind::Fricative && ul < 0.35f) burst = expf(-ul * 4.0f);

                // nasal: less bright, smoother
                float nasal = (ck == ConsKind::Nasal) ? 1.0f : 0.0f;

                // source: glottal-ish + noise
                phase += pitch * dt;
                ...
                float grit = 0.18f;
                if (faction == 2) grit = 0.34f; // Rebels gravelly
                if (faction == 0) grit = 0.16f; // Axis cleaner/darker
                if (faction == 1) grit = 0.14f; // Allies refined
                if (faction == 3) grit = 0.22f; // Militia wobble

                float n = arand11();

                float srcBuzz = buzz * (0.85f - 0.25f * burst);
                float srcNoise = n * (grit + 0.55f * burst);

                // nasals reduce noise, emphasize lower bands
                if (nasal > 0.5f) srcNoise *= 0.35f;

                float src = srcBuzz + srcNoise;

                float v = (0.48f * f1.tick(src) + 0.34f * f2.tick(src) + 0.18f * f3.tick(src));

                local = v * env * (U.stressed ? 1.10f : 1.0f);
            }

            float s = local * gEnv * gain;
...
static void Audio_Spawn(const AudioEvent& e) {
    ...
    case AudioEvent::Bark: v = new SpeechVoice(e.faction, e.textHash); break;
...
static inline void Audio_PostBark(float x, float y, int faction, const char* text) {
    AudioEvent e; e.type = AudioEvent::Bark; e.x = x; e.y = y; e.faction = faction;
    e.textHash = fnv1a32(text);
    e.maxAudible = 520.0f; // voice
    Audio_Post(e);
}
