...
namespace cfg {
    ...
    constexpr float ZoomPlayer = 1.55f;
    constexpr float ZoomSandbox = 1.55f;
...
static inline int ammoRollForTeam(int team) {
    // Small range; Axis/Allies higher max than Militia/Rebels
    switch (team) {
    case 0: /* Allies */     return irand(18, 48);
    case 1: /* Axis */   return irand(16, 44);
    case 2: /* Rebels */   return irand(10, 28);
    case 3: /* Militia */  return irand(8, 24);
    default:               return irand(10, 28);
    }
}

static bool gSpeechAudioEnabled = false; // set true later when you want it back
...
struct WeaponInstance {
    ...
    uint32_t uid = 0;         // unique identity for persistence / meta
    uint32_t visualSeed = 0;  // stable procedural look seed
      ...
      { WeaponId::WELROD, "Welrod (Silenced)", WeaponTier::Intermediate, AmmoType::PARA_9MM,
        ...
        { WeaponId::SUOMI, "Suomi", WeaponTier::Advanced, AmmoType::PARA_9MM,
          ...
          { WeaponId::KAR98K, "Kar98k", WeaponTier::Basic, AmmoType::NATO_762,
            5, 25, 1.20f, 2.10f, 42.0f, 2.0f, 1, 980.0f, 1100.0f },

          { WeaponId::MOSIN, "Mosin-Nagant", WeaponTier::Basic, AmmoType::RUS_762x54R,
            5, 25, 1.20f, 2.10f, 44.0f, 2.2f, 1, 980.0f, 1150.0f },

          { WeaponId::SMLE, "Lee-Enfield", WeaponTier::Intermediate, AmmoType::NATO_762,
            10, 40, 1.20f, 2.05f, 40.0f, 2.2f, 1, 980.0f, 1100.0f },
...
enum class EjectType {
    None,
    SemiAuto,      // eject per shot immediately
    BoltAction,    // delayed per shot
    GarandClip,    // clip eject when empty
    ShotgunPump,   // shell eject per shot
    ShotgunBreak   // shells eject on reload/open
};

static inline bool isAutoWeapon(const WeaponDef& wd) {
    return (wd.fireCooldownS > 0.0f && wd.fireCooldownS < 0.14f);
}

static inline EjectType weaponEjectType(WeaponId wid, const WeaponDef& wd) {
    // Garand special
    if (wid == WeaponId::GARAND) return EjectType::GarandClip;

    // Shotguns: pellets > 1 is your signal
    if (wd.pellets > 1) {
        // crude but effective: 2-round = break/open (double barrel)
        if (wd.magSize <= 2) return EjectType::ShotgunBreak;
        return EjectType::ShotgunPump;
    }

    // Bolt actions (include Welrod oddity)
    switch (wid) {
    case WeaponId::MOSIN:
    case WeaponId::KAR98K:
    case WeaponId::SMLE:
    case WeaponId::WELROD:
        return EjectType::BoltAction;
    default: break;
    }

    // Default: semi/auto both eject immediately (brass)
    return EjectType::SemiAuto;
}
...
struct LootDrop {
    ...
    WeaponInstance pInst{}; bool hasP = false;
    WeaponInstance sInst{}; bool hasS = false;
...
static inline int findNearestLoot(const std::vector<LootDrop>& drops, const Vec2& p, float maxDist) {
    float best2 = maxDist * maxDist;
    int bestIdx = -1;

    for (int i = 0; i < (int)drops.size(); ++i) {
        if (drops[i].taken) continue;
        float d2 = dist2(drops[i].pos, p);
        if (d2 < best2) {
            best2 = d2;
            bestIdx = i;
        }
...
static inline int ammoCapForWeapon(WeaponId wid) {
    // Lightweight v0: per-weapon cap (ammo types later).
    const WeaponDef& wd = weaponDef(wid);

    int cap = std::max(30, wd.magSize * 4); // ~4 mags baseline

    // Fast firing: allow more carry (SMG-ish)
    if (wd.fireCooldownS > 0.0f && wd.fireCooldownS < 0.12f)
        cap = (int)std::round(cap * 1.5f);

    // Shotgun: smaller reserve by feel
    if (wd.pellets > 1) cap = 28;

    return std::clamp(cap, 24, 240);
}

static inline float weaponAIScore(WeaponId wid) {
    const WeaponDef& wd = weaponDef(wid);

    // Rough “combat value”: damage throughput + range
    float rof = (wd.fireCooldownS > 0.001f) ? (1.0f / wd.fireCooldownS) : 1.0f;
    float dps = wd.baseDamage * rof;

    // Shotguns: pellets matter, but range matters more
    float pelletMul = std::max(1, wd.pellets);

    float score = dps * (0.75f + 0.12f * pelletMul) + wd.maxRange * 0.015f;

    // Very small weapons get a mild penalty (encourages rifles/SMGs over pistols)
    if (wd.magSize <= 8) score *= 0.85f;

    return score;
}


static inline float weaponNoiseRadiusPx(WeaponId wid) {
    const WeaponDef& wd = weaponDef(wid);

    // Start from your old baseline: cfg::GunshotHearTiles * TileSize
    float r = cfg::GunshotHearTiles * cfg::TileSize;

    // Scale by weapon character
    r += wd.baseDamage * 6.0f;
    if (wd.pellets > 1) r += 140.0f; // shotguns loud

    // Fast autos draw attention
    if (wd.fireCooldownS > 0.0f && wd.fireCooldownS < 0.12f) r += 90.0f;

    // Welrod: quieter (your table already names it silenced)
    if (wid == WeaponId::WELROD) r *= 0.25f;

    return std::clamp(r, 120.0f, 720.0f);
}


static inline void adsParamsForWeapon(WeaponId wid, float& outZoomMul, float& outPull) {
    const WeaponDef& wd = weaponDef(wid);
    ...
    // Shotguns: small ADS benefit, short fights
    if (wd.pellets > 1) {
        outZoomMul = 1.10f;
        outPull = 0.18f;
        return;
    ...
    // Very fast weapons (SMG-ish): smaller zoom, smaller pull (keeps it scrappy)
    if (wd.fireCooldownS > 0.0f && wd.fireCooldownS < 0.12f) {
        outZoomMul = 1.12f;
        outPull = 0.19f;
        return;
    ...
    // Rifles / long range: more “exact” ADS
    if (wd.maxRange >= 420.0f) {
        outZoomMul = 1.28f;
        outPull = 0.30f;
        return;
    }
...
static inline Vec2 aimDirFromMouse(const Vec2& from, int worldMouseX, int worldMouseY) {
    Vec2 aim{ (float)worldMouseX, (float)worldMouseY };
    Vec2 d = aim - from;
    float L = length(d);
    if (L < 0.001f) return Vec2{ 1, 0 };
    return d * (1.0f / L);
}
...
struct MuzzleFlash {
    ...
    Vec2 dir{ 1,0 };
    float ttl = 0.0f;
    float size = 6.0f;   // primitive scale
    int passes = 2;      // thickness
};

struct ImpactFX {
    Vec2 pos{ 0,0 };
    Vec2 dir{ 1,0 };     // incoming dir (bullet dir)
    float ttl = 0.0f;  // time left
    float age = 0.0f;  // time since spawn
    float size = 6.0f; // base size
    bool flesh = false; // blood vs dust
};

enum class CasingKind { Brass, Shell, Clip };
...
struct CasingFX {
    ...
    float rot = 0.0f;
    float rotVel = 0.0f;
    CasingKind kind = CasingKind::Brass;
    bool ping = false; // for Garand clip
};
...
struct Actor {
    ...
    Vec2    lastShotDir{ 1,0 };
    float   lastShotT = 0.0f; // seconds left to lock weapon visual to shot direction
    ...
    WeaponInstance weapon; // primary

    //NEW (11A): secodary + active slot
    WeaponInstance weapon2; // secondary
    bool hasWeapon2 = false;
    uint8_t activeSlot = 0; // 0=primary 1=secondary

    //casings stuff
    float pendingEjectS = 0.0f;
    int pendingEjectCount = 0;
    CasingKind pendingEjectKind = CasingKind::Brass;
    bool pendingEjectPing = false;
    ...
    // --- Burst fire control (7A)
    int burstShotsLeft = 0;
    float burstCooldownS = 0.0f;
    float panicS = 0.0f;      // >0 means "panic hose" window
    ...
    // -----------------------------------------------------------
// Procedural plan-view character animation cache
// -----------------------------------------------------------
    struct ProcAnim {
        Vec2  prevPos{ 0,0 };
        Vec2  vel{ 0,0 };          // px/s
        Vec2  moveDir{ 1,0 };       // normalized vel when moving
        float speed = 0.0f;

        float gait = 0.0f;        // radians
        float torsoAng = 0.0f;    // smoothed torso angle (passenger to weapon)
        float weaponAng = 0.0f;   // facing angle
...
static inline void updateProcAnim(Actor& a, float dt);

// Visual + hit-rig body length scale (keeps width the same, shortens forward/back)
static constexpr float kBodyLenMul = 0.70f; // 0.65–0.80 is the sweet spot
...
static void buildDefaultHitRig(Actor& a) {
    ...
    float halfF = (a.w * kBodyLenMul) * 0.5f; // shortened torso length
    float halfR = a.h * 0.5f;                 // KEEP width exactly the same
    ...
    a.hitRig = {
        ...
        HitBox{ HitZone::Torso, -halfF * 0.10f, -halfR * 0.35f,  halfF * 0.45f,  halfR * 0.70f },
        HitBox{ HitZone::Legs,  -halfF * 0.80f, -halfR * 0.40f,  halfF * 0.55f,  halfR * 0.80f },
        HitBox{ HitZone::ArmL,  -halfF * 0.15f, -halfR * 1.05f,  halfF * 0.55f,  halfR * 0.55f },
        HitBox{ HitZone::ArmR,  -halfF * 0.15f, +halfR * 0.50f,  halfF * 0.55f,  halfR * 0.55f },
    };
}
...
// -----------------------------------------------------------
// 11A: Active weapon helpers (primary/secondary)
// -----------------------------------------------------------
static inline WeaponInstance& primaryW(Actor& a) { return a.weapon; }
static inline const WeaponInstance& primaryW(const Actor& a) { return a.weapon; }

static inline WeaponInstance& secondaryW(Actor& a) { return a.weapon2; }
static inline const WeaponInstance& secondaryW(const Actor& a) { return a.weapon2; }

static inline WeaponInstance& activeW(Actor& a) {
    if (a.activeSlot == 1 && a.hasWeapon2 && a.weapon2.id != WeaponId::None) return a.weapon2;
    return a.weapon;
}
static inline const WeaponInstance& activeW(const Actor& a) {
    if (a.activeSlot == 1 && a.hasWeapon2 && a.weapon2.id != WeaponId::None) return a.weapon2;
    return a.weapon;
}

static inline void setActiveSlot(Actor& a, uint8_t slot) {
    if (slot == 1 && (!a.hasWeapon2 || a.weapon2.id == WeaponId::None)) {
        a.activeSlot = 0;
    }
    else {
        a.activeSlot = slot;
...
static inline float heldWeaponLen(const WeaponDef& wd) {
    float len = 10.0f + std::clamp(wd.maxRange * 0.020f, 0.0f, 18.0f);
    // Pistols shorter
    if (wd.magSize <= 10 && wd.maxRange < 260.0f) len *= 0.60f;
    // Shotguns a touch longer
    if (wd.pellets > 1) len += 2.0f;
    return len;
}

static inline Vec2 weaponMuzzlePos(const Actor& a, const WeaponDef& wd) {
    Vec2 fwd = normalize(a.facing);
    if (length(fwd) < 0.001f) fwd = Vec2{ 1,0 };
    Vec2 right = perpRight(fwd);

    // Same anchor you used for held weapon
    Vec2 base = a.pos + fwd * (a.w * 0.30f) + right * (a.h * 0.10f);
    float len = heldWeaponLen(wd);
    return base + fwd * len;
}

static inline void pushMuzzleFlashRing(std::vector<MuzzleFlash>& mfs,
    size_t& writeIdx,
    const Vec2& pos,
    const Vec2& dir,
    const WeaponDef& wd)
{
    if (mfs.empty()) {
        SDL_Log("MUZZLE PUSH CALLED WITH EMPTY POOL");
        return;
    }
    
    if (mfs.empty()) return;

    MuzzleFlash& mf = mfs[writeIdx];
    writeIdx++;
    if (writeIdx >= mfs.size()) writeIdx = 0;

    mf.pos = pos;
    mf.dir = dir;
    mf.ttl = 0.06f;

    // keep your existing sizing rules if you had them
    mf.size = 6.0f + std::clamp(wd.baseDamage * 0.06f, 0.0f, 8.0f);
    mf.passes = 2 + (wd.pellets > 1 ? 1 : 0);
}


static inline void pushImpactRing(std::vector<ImpactFX>& impacts,
    size_t& writeIdx,
    const Vec2& pos,
    const Vec2& dir,
    bool flesh)
{
    if (impacts.empty()) {
        SDL_Log("IMPACTS PUSH CALLED WITH EMPTY POOL");
        return;
    }
    
    if (impacts.empty()) return;

    ImpactFX& fx = impacts[writeIdx];
    writeIdx++;
    if (writeIdx >= impacts.size()) writeIdx = 0;
    ...
    fx.age = 0.0f;
    fx.ttl = 0.18f;
    fx.size = flesh ? 7.0f : 6.0f;
}


static inline void pushCasingRing(std::vector<CasingFX>& casings,
    size_t& writeIdx,
    const Vec2& pos,
    const Vec2& facing,
    CasingKind kind,
...
{
    if (casings.empty()) {
        SDL_Log("CASING PUSH CALLED WITH EMPTY POOL");
        return;
    }
    ...
    // pick slot, advance head
    CasingFX& c = casings[writeIdx];
    writeIdx++;
    if (writeIdx >= casings.size()) writeIdx = 0;

    c.pos = pos;
    c.kind = kind;
    c.ping = ping;

    // Ejection direction: mostly right, slightly forward
    Vec2 dir = normalize(right * frand(0.8f, 1.2f) + fwd * frand(0.15f, 0.45f));

    float speed =
        (kind == CasingKind::Clip) ? frand(18.f, 28.f) :
        (kind == CasingKind::Shell) ? frand(25.f, 40.f) :
        frand(55.f, 85.f);

    c.vel = dir * speed;
    c.ttl = (kind == CasingKind::Clip) ? 1.2f : frand(0.55f, 0.95f);

    c.rot = frand(0.f, 6.28318f);
    c.rotVel = frand(-10.f, 10.f);
}




static inline float heldWeaponLenFromDef(const WeaponDef& wd) {
    float len = 10.0f + std::clamp(wd.maxRange * 0.020f, 0.0f, 18.0f);
    if (wd.magSize <= 10 && wd.maxRange < 260.0f) len *= 0.60f; // pistols
    if (wd.pellets > 1) len += 2.0f; // shotguns a touch longer
    return len;
}

static inline void weaponBasis(const Actor& a, const WeaponDef& wd, Vec2& outFwd, Vec2& outRight, Vec2& outBase, float& outLen) {
    outFwd = normalize(a.facing);
    if (length(outFwd) < 0.001f) outFwd = Vec2{ 1,0 };
    outRight = perpRight(outFwd);

    outLen = heldWeaponLenFromDef(wd);

    // same “hands” anchor feel as your held-weapon draw
    outBase = a.pos + outFwd * (a.w * 0.30f) + outRight * (a.h * 0.10f);
}

static inline Vec2 weaponEjectPos(const Actor& a, const WeaponDef& wd) {
    Vec2 fwd, right, base;
    float len;
    weaponBasis(a, wd, fwd, right, base, len);

    // Ejection port generally sits ~1/3 along the receiver,
    // slightly to the right side of the weapon.
    float along = (wd.magSize <= 10 && wd.maxRange < 260.0f) ? 0.18f : 0.28f; // pistols vs rifles
    float side = (wd.pellets > 1) ? 0.18f : 0.22f; // shotguns a bit tighter

    return base + fwd * (len * along) + right * (a.h * side);
}
...
static WeaponId gPlayerStartWeapon2 = WeaponId::WELROD; // choose something valid in your table
static int gPlayerStartMag2Override = -1;
static int gPlayerStartReserve2Override = -1;
...
static uint32_t gNextWeaponUID = 1;
...
static inline void ensureWeaponIdentity(WeaponInstance& w) {
    if (w.uid == 0) w.uid = gNextWeaponUID++;
    if (w.visualSeed == 0) w.visualSeed = hash32(w.uid ^ (uint32_t)w.id * 2654435761u);
}
...
static inline WeaponId pickSidearmForFaction(Faction f) {
    // Keep it simple: a few pistols that you definitely have in the table
    switch (f) {
    case Faction::Axis:   return WeaponId::LUGER_P08;
    case Faction::Allies: return WeaponId::M1911;
    case Faction::Rebels: return WeaponId::M1911;
    case Faction::Militia:return WeaponId::TOKAREV_TT33;
    default:              return WeaponId::M1911;
    }
}
...
static inline void applyFactionLoadout(Actor& a) {
    ...
    // Secondary (sidearm) chance
    a.hasWeapon2 = false;
    a.weapon2 = WeaponInstance{};
    a.weapon2.id = WeaponId::None;

    // Militia rarely have a sidearm; Axis/Allies more often
    int chance = 0;
    switch (a.team) {
    case Faction::Axis:   chance = 55; break;
    case Faction::Allies: chance = 55; break;
    case Faction::Rebels: chance = 35; break;
    case Faction::Militia:chance = 15; break;
    default:              chance = 25; break;
    }

    if (irand(0, 99) < chance) {
        WeaponId sid = pickSidearmForFaction(a.team);
        if (sid != WeaponId::None && sid != a.weapon.id) {
            // applyWeaponInstance writes to a.weapon, so do the same trick as player loadout:
            WeaponInstance savedPrimary = a.weapon;
            applyWeaponInstance(a, sid);
            a.weapon2 = a.weapon;
            a.weapon = savedPrimary;
...
static inline void applyPlayerLoadout(Actor& player) {
    ...
    // Secondary (for testing / progression)
    player.hasWeapon2 = (gPlayerStartWeapon2 != WeaponId::None);
    if (player.hasWeapon2) {
        // applyWeaponInstance writes into player.weapon (active slot weapon),
        // so we temporarily apply, then copy to weapon2.
        WeaponInstance savedPrimary = player.weapon;

        applyWeaponInstance(player, gPlayerStartWeapon2, gPlayerStartMag2Override, gPlayerStartReserve2Override);
        player.weapon2 = player.weapon;

        // restore primary back into weapon
        player.weapon = savedPrimary;
    }
    else {
        // keep clean defaults
        player.weapon2 = WeaponInstance{};
        player.weapon2.id = WeaponId::None;
    }
}


static inline bool weaponCanFire(const Actor& a) {
    const WeaponInstance& w = activeW(a);
    return !w.reloading && w.fireTimer <= 0.0f && w.magAmmo > 0;
}


static inline void weaponStartReload(Actor& a) {
    WeaponInstance& w = activeW(a);
    if (w.reloading) return;

    const WeaponDef& d = weaponDef(w.id);
    if (d.magSize <= 0) return;
    if (w.magAmmo >= d.magSize) return;
    if (w.reserveAmmo <= 0) return;

    w.reloading = true;

    if (w.id != WeaponId::None) {
        const WeaponDef& wd = weaponDef(w.id);
        EjectType et = weaponEjectType(w.id, wd);

        if (et == EjectType::ShotgunBreak) {
            // Eject spent shells when opening the action:
            int spent = std::clamp(wd.magSize - w.magAmmo, 0, wd.magSize);
            if (spent > 0) {
                a.pendingEjectCount = spent;
                a.pendingEjectKind = CasingKind::Shell;
                a.pendingEjectPing = false;
                a.pendingEjectS = 0.08f; // tiny delay feels like "open action"
            }
        }
    }

    w.reloadTimer = d.reloadTimeS;


    // Legacy gun mirrors ACTIVE weapon only
    a.gun.reloading = true;
    a.gun.reloadTimer = d.reloadTimeS;
}


static inline void weaponUpdateOne(WeaponInstance& w, float dt) {
    if (w.fireTimer > 0.0f) w.fireTimer = std::max(0.0f, w.fireTimer - dt);

    if (w.reloading) {
        w.reloadTimer -= dt;
        if (w.reloadTimer <= 0.0f) {
            const WeaponDef& d = weaponDef(w.id);
            int need = std::max(0, d.magSize - w.magAmmo);
            int take = std::min(need, w.reserveAmmo);
            w.magAmmo += take;
            w.reserveAmmo -= take;
            w.reloading = false;
            w.reloadTimer = 0.0f;
...
static inline void weaponUpdate(Actor& a, float dt) {
    weaponUpdateOne(a.weapon, dt);
    if (a.hasWeapon2 && a.weapon2.id != WeaponId::None) weaponUpdateOne(a.weapon2, dt);

    // Keep legacy gun aligned to ACTIVE weapon
    const WeaponInstance& w = activeW(a);
    const WeaponDef& d = weaponDef(w.id);
    a.gun.magSize = d.magSize;
    a.gun.inMag = w.magAmmo;
    a.gun.fireCooldownS = d.fireCooldownS;
    a.gun.reloadTimeS = d.reloadTimeS;
    a.gun.bulletDamage = (int)std::round(d.baseDamage);

    // reflect timers roughly
    a.gun.fireTimer = w.fireTimer;
    a.gun.reloading = w.reloading;
    a.gun.reloadTimer = w.reloadTimer;
}


// --- PATCH 9A: helpers for hover reveal labels ---
static inline const char* factionShortName(Faction f) {
    switch (f) {
...
static inline SDL_FPoint worldToScreenPt(const Vec2& w, float camX, float camY, float zoom) {
    return SDL_FPoint{ (w.x - camX) * zoom, (w.y - camY) * zoom };
}
...
private:
    ...
    size_t muzzleWrite = 0;

    std::vector<ImpactFX> impacts;
    size_t impactWrite = 0;

    std::vector<CasingFX> casings;
    size_t casingWrite = 0;
    ...
    bool mouseDownLPrev = false;
    bool mousePressedL = false; // true for 1 frame on press-down


    // -----------------------------------------------------------
// 8B/8C: ADS + Binoculars state
// -----------------------------------------------------------
    bool  adsHeld = false;          // RMB hold when NOT binoculars

    // --- PATCH 9B: ADS first-shot bookkeeping ---
    bool adsHeldPrev = false;
    bool adsFirstShotReady = false;

    bool  binoEquipped = false;     // toggled with key 5
    bool  binoHeld = false;         // RMB hold when bino equipped
    float zoomNormal = cfg::ZoomSandbox;  // user/base zoom (wheel edits this)

    bool  hasBinoFootprint = false;
    SDL_FRect binoFootprintScreen{ 0,0,0,0 }; // screen-space rect to draw as cursor

    // --- 9H: binocs focus cache for hover reveal ---
    bool hasBinoFocus = false;
    Vec2 binoFocusWorld{ 0,0 };


    // --- Loot menu (hold E)
    bool kE = false;
    bool lootMode = false;
    float lootHoldS = 0.0f;
    int lootIdx = -1;          // nearest loot drop index
    const float lootHoldThreshS = 0.22f;  // tap vs hold threshold
    const float lootRadius = 26.0f;

    // --- Map overlay (8A) ---
    bool mapOverlay = false;
    SDL_Texture* mapTex = nullptr;   // 1 pixel per tile
    bool mapTexDirty = true;
    Vec2 missionStartPos{ 0,0 };       // optional marker (set when mission starts)
    ...
    void drawHUD();
    void drawADSWeaponHUD();
    void drawHoverReveal();
    void drawCrosshair();
    void drawVisionOutlineAndRay(const Actor& a);
    void drawBinocularsPip();
    void drawBinoFogBoundary();
    void drawHeldWeapon(const Actor& a);
    void drawHeldWeapon(const Actor& a, Vec2 fwdOverride);
    void drawWeaponSilhouetteAt(const Vec2& pos, WeaponId wid, const Vec2& dir, float scale);
    // void drawSquadDebug();
    void drawProcCharacter(const Actor& a, SDL_Color bodyCol);
    ...
    static constexpr int kMaxCasings = 600;
    //int casingWrite = 0;

};


// ---------- Phonemes (ARPAbet-ish) ----------
...
static inline bool PhIsVowel(Ph p) {
    switch (p) {
    case Ph::AA: case Ph::AE: case Ph::AH: case Ph::AO: case Ph::AW: case Ph::AY:
    case Ph::EH: case Ph::ER: case Ph::EY:
    case Ph::IH: case Ph::IY:
    case Ph::OW: case Ph::OY:
    case Ph::UH: case Ph::UW:
        return true;
    default: return false;
    }
}

static inline bool PhIsStop(Ph p) {
    return p == Ph::P || p == Ph::B || p == Ph::T || p == Ph::D || p == Ph::K || p == Ph::G;
}

static inline bool PhIsFric(Ph p) {
    return p == Ph::F || p == Ph::V || p == Ph::TH || p == Ph::DH || p == Ph::S || p == Ph::Z || p == Ph::SH || p == Ph::ZH || p == Ph::HH;
}

static inline bool PhIsNasal(Ph p) {
    return p == Ph::M || p == Ph::N || p == Ph::NG;
}

static inline bool PhIsVoiced(Ph p) {
    if (PhIsVowel(p)) return true;
    // voiced frics + stops + nasals + liquids/glides
    if (p == Ph::B || p == Ph::D || p == Ph::G || p == Ph::V || p == Ph::DH || p == Ph::Z || p == Ph::ZH) return true;
    if (PhIsNasal(p)) return true;
    if (p == Ph::L || p == Ph::R || p == Ph::Y || p == Ph::W) return true;
    return false;
}



// ---------- token packet for speech ----------
struct SpeechTok {
    Ph      ph = Ph::SIL;
    uint8_t stress = 0;   // 0..2
    uint8_t flags = 0;   // 1=breathy 2=unvoiced 4=pause 8=yell 16=question
    uint8_t dur8 = 10;  // 10ms units
    uint8_t f0_8 = 64;  // 0..127 mapped to Hz in voice
};

// Larger because phonemes > syllables
static const int kMaxSpeechTok = 64;


// ---------- Audio globals ----------
static SDL_AudioDeviceID gAudioDev = 0;
static int gAudioSR = 48000;

// Keep it simple + safe: mutex queue from game thread -> audio callback.
// (We can switch to lock-free later.)
struct AudioEvent {
    enum Type : uint8_t { Fire, Reload, Footstep, HitSolid, HitFlesh, Bark } type;
    int   weaponId = 0;
    int   faction = 0;
    float x = 0, y = 0;
    float p0 = 0;   // generic param (speedNorm / energy / etc)
    float p1 = 0;   // generic param

    float maxAudible = 600.0f;

    uint32_t textHash = 0; // deterministic speech seed for this bark line

    SpeechTok speech[kMaxSpeechTok];
    uint8_t   speechN = 0;
};

static std::mutex gAudioMx;
static std::deque<AudioEvent> gAudioQ;

// ---------- Tiny RNG + noise ----------
static uint32_t gARand = 0xA1B2C3D4u;
static inline float arand01() { gARand = 1664525u * gARand + 1013904223u; return ((gARand >> 8) & 0x00FFFFFF) / float(0x00FFFFFF); }
static inline float arand11() { return arand01() * 2.0f - 1.0f; }
...
// ---------- speech proper ---------
static inline uint8_t clampu8(int v) { return (uint8_t)(v < 0 ? 0 : (v > 127 ? 127 : v)); }
...
// Hz <-> token pitch mapping (open-air natural range)
static inline uint8_t HzToF0_8(float hz) {
    // map 110..330 Hz -> 0..127
    float t = (hz - 110.f) / (330.f - 110.f);
    t = std::clamp(t, 0.f, 1.f);
    return clampu8((int)(t * 127.f));
}
static inline float F0_8ToHz(uint8_t v) {
    float t = v / 127.f;
    return 110.f + t * (330.f - 110.f);
}

// --- tiny lexicon override for common barks ---
// Put this ONCE (after enum class Ph exists, before LexLookup).

struct LexEntry { const char* w; const Ph* ph; uint8_t n; };

template <typename T, size_t N>
static constexpr uint8_t COUNT_OF(const T(&)[N]) { return (uint8_t)N; }

#define LEXW(name, ...) static const Ph name[] = { __VA_ARGS__ }
#define LEX_ENTRY(wordLit, arrName) { wordLit, arrName, COUNT_OF(arrName) }

LEXW(PH_GO, Ph::G, Ph::OW);
LEXW(PH_CONTACT, Ph::K, Ph::AA, Ph::N, Ph::T, Ph::AE, Ph::K, Ph::T);
LEXW(PH_RELOAD, Ph::R, Ph::IY, Ph::L, Ph::OW, Ph::D);
LEXW(PH_FLANK, Ph::F, Ph::L, Ph::AE, Ph::NG, Ph::K);
LEXW(PH_LEFT, Ph::L, Ph::EH, Ph::F, Ph::T);
LEXW(PH_RIGHT, Ph::R, Ph::AY, Ph::T);
LEXW(PH_MOVE, Ph::M, Ph::UW, Ph::V);
LEXW(PH_HOLD, Ph::HH, Ph::OW, Ph::L, Ph::D);
LEXW(PH_PUSH, Ph::P, Ph::UH, Ph::SH);
LEXW(PH_RETREAT, Ph::R, Ph::IH, Ph::T, Ph::R, Ph::IY, Ph::T);
LEXW(PH_ENEMY, Ph::EH, Ph::N, Ph::AH, Ph::M, Ph::IY);

static const LexEntry gLex[] = {
    LEX_ENTRY("GO",       PH_GO),
    LEX_ENTRY("CONTACT",  PH_CONTACT),
    LEX_ENTRY("RELOAD",   PH_RELOAD),
    LEX_ENTRY("FLANK",    PH_FLANK),
    LEX_ENTRY("LEFT",     PH_LEFT),
    LEX_ENTRY("RIGHT",    PH_RIGHT),
    LEX_ENTRY("MOVE",     PH_MOVE),
    LEX_ENTRY("HOLD",     PH_HOLD),
    LEX_ENTRY("PUSH",     PH_PUSH),
    LEX_ENTRY("RETREAT",  PH_RETREAT),
    LEX_ENTRY("ENEMY",    PH_ENEMY),
};
...
static inline bool LexLookup(const char* word, Ph* out, int& outN, int maxN) {
    // word is expected uppercase A-Z only
    for (const auto& e : gLex) {
        if (strcmp(e.w, word) == 0) {
            int n = std::min<int>(e.n, maxN);
            for (int i = 0; i < n; i++) out[i] = e.ph[i];
            outN = n;
            return true;
        }
    }
...
// --- fallback G2P (simple, good enough for barks) ---
static inline void PushPh(std::vector<Ph>& v, Ph p) { v.push_back(p); }

static inline bool StartsWith2(char a, char b, char c0, char c1) {
    return (a == c0 && b == c1);
}

static inline void G2P_Fallback(const char* w, std::vector<Ph>& out) {
    // w is uppercase letters only
    int n = (int)strlen(w);
    int i = 0;

    auto at = [&](int k)->char { return (k >= 0 && k < n) ? w[k] : 0; };
    ...
    while (i < n) {
        ...
        // digraphs
        if (StartsWith2(c, c1, 'T', 'H')) { PushPh(out, Ph::TH); i += 2; continue; }
        if (StartsWith2(c, c1, 'S', 'H')) { PushPh(out, Ph::SH); i += 2; continue; }
        if (StartsWith2(c, c1, 'C', 'H')) { PushPh(out, Ph::CH); i += 2; continue; }
        if (StartsWith2(c, c1, 'N', 'G')) { PushPh(out, Ph::NG); i += 2; continue; }
        if (StartsWith2(c, c1, 'P', 'H')) { PushPh(out, Ph::F);  i += 2; continue; }
        if (StartsWith2(c, c1, 'W', 'H')) { PushPh(out, Ph::W);  i += 2; continue; }

        // vowel groups (very basic)
        if (StartsWith2(c, c1, 'E', 'E') || StartsWith2(c, c1, 'E', 'A')) { PushPh(out, Ph::IY); i += 2; continue; }
        if (StartsWith2(c, c1, 'O', 'O')) { PushPh(out, Ph::UW); i += 2; continue; }
        if (StartsWith2(c, c1, 'O', 'W') || StartsWith2(c, c1, 'O', 'U')) { PushPh(out, Ph::AW); i += 2; continue; }
        if (StartsWith2(c, c1, 'A', 'Y') || StartsWith2(c, c1, 'A', 'I')) { PushPh(out, Ph::EY); i += 2; continue; }
        if (StartsWith2(c, c1, 'O', 'I')) { PushPh(out, Ph::OY); i += 2; continue; }

        // single letter mapping
        switch (c) {
            // vowels
        case 'A': PushPh(out, Ph::AE); break;
        case 'E': PushPh(out, Ph::EH); break;
        case 'I': PushPh(out, Ph::IH); break;
        case 'O': PushPh(out, Ph::AA); break;
        case 'U': PushPh(out, Ph::AH); break;
        case 'Y': PushPh(out, Ph::IY); break;

            // consonants
        case 'B': PushPh(out, Ph::B); break;
        case 'C': {
            // soft C before E/I/Y
            char nx = at(i + 1);
            if (nx == 'E' || nx == 'I' || nx == 'Y') PushPh(out, Ph::S);
            else PushPh(out, Ph::K);
        } break;
        case 'D': PushPh(out, Ph::D); break;
        case 'F': PushPh(out, Ph::F); break;
        case 'G': {
            // soft G before E/I/Y
            char nx = at(i + 1);
            if (nx == 'E' || nx == 'I' || nx == 'Y') PushPh(out, Ph::JH);
            else PushPh(out, Ph::G);
        } break;
        case 'H': PushPh(out, Ph::HH); break;
        case 'J': PushPh(out, Ph::JH); break;
        case 'K': PushPh(out, Ph::K); break;
        case 'L': PushPh(out, Ph::L); break;
        case 'M': PushPh(out, Ph::M); break;
        case 'N': PushPh(out, Ph::N); break;
        case 'P': PushPh(out, Ph::P); break;
        case 'Q': PushPh(out, Ph::K); PushPh(out, Ph::W); break;
        case 'R': PushPh(out, Ph::R); break;
        case 'S': PushPh(out, Ph::S); break;
        case 'T': PushPh(out, Ph::T); break;
        case 'V': PushPh(out, Ph::V); break;
        case 'W': PushPh(out, Ph::W); break;
        case 'X': PushPh(out, Ph::K); PushPh(out, Ph::S); break;
        case 'Z': PushPh(out, Ph::Z); break;
        default: break;
        }
        i++;
    }

    // tiny cleanup: remove adjacent duplicate vowels
    for (int k = 1; k < (int)out.size();) {
        if (PhIsVowel(out[k]) && out[k] == out[k - 1]) out.erase(out.begin() + k);
        else k++;
    }
}

// Build tokens (phonemes + prosody) into AudioEvent
static inline void BuildSpeechTokens(AudioEvent& e, const char* text)
{
    e.speechN = 0;
    ...
    // base pitch per faction (open-air natural)
    float base = 165.f;
    if (e.faction == 0) base = 145.f; // Axis deeper
    else if (e.faction == 1) base = 175.f; // Allies
    else if (e.faction == 2) base = 165.f; // Rebels
    else if (e.faction == 3) base = 190.f; // Militia higher
    if (yell) base *= 1.10f;

    // We'll parse words, look them up, or fallback G2P.
    // Tokens: phonemes with durations and stress.
    const int MAX = kMaxSpeechTok;

    auto pushPauseMs = [&](int ms) {
        if (e.speechN >= MAX) return;
        SpeechTok& t = e.speech[e.speechN++];
        t.ph = Ph::PAU;
        t.flags = 4 | (yell ? 8 : 0) | (question ? 16 : 0);
        t.dur8 = (uint8_t)std::clamp(ms / 10, 2, 60);
        t.stress = 0;
        t.f0_8 = HzToF0_8(base);
        };

    auto pushPhTok = [&](Ph ph, int stress, float hz, int durMs) {
        if (e.speechN >= MAX) return;
        SpeechTok& t = e.speech[e.speechN++];
        t.ph = ph;
        t.stress = (uint8_t)std::clamp(stress, 0, 2);
        t.flags = (yell ? 8 : 0) | (question ? 16 : 0);
        t.dur8 = (uint8_t)std::clamp(durMs / 10, 2, 50);
        t.f0_8 = HzToF0_8(hz);
        };

    // walk characters and build words
    char word[48]{};
    int  wn = 0;
    int  wordIndex = 0;

    int phraseWordIndex = 0;    // resets on punctuation
    int phraseWords = 0;        // optional estimate; we’ll just use index for now

    auto WordIsCommand = [&](const char* w)->bool {
        // keep this tiny; it makes barks pop immediately
        return
            strcmp(w, "CONTACT") == 0 ||
            strcmp(w, "RELOAD") == 0 ||
            strcmp(w, "FLANK") == 0 ||
    ...
    auto flushWord = [&]() {
        ...
        // lookup / fallback to phonemes list
        Ph phs[64];
        int phN = 0;

        if (!LexLookup(word, phs, phN, 64)) {
            std::vector<Ph> tmp;
            G2P_Fallback(word, tmp);
            phN = std::min<int>((int)tmp.size(), 64);
            for (int i = 0; i < phN; i++) phs[i] = tmp[i];
        }



        // prosody contour: statement falls, question rises
        // --- PHRASE SHAPING ---
// Stronger phrase arc that resets per punctuation.
        float t = std::min(1.0f, phraseWordIndex / 5.0f);

        // statements: start a bit higher, drift down
        // questions: drift up a bit toward end
        float contour = question ? (0.98f + 0.18f * t) : (1.08f - 0.20f * t);

        // command words get extra “accent”
        bool isCmd = WordIsCommand(word);
        float cmdMul = isCmd ? 1.06f : 1.00f;

        // stress: first vowel gets stress=1, but command words get stress=2 on their first vowel
        bool stressed = false;


        // detect last vowel index in this word (for word-final lengthening)
        int lastVowel = -1;
        for (int i = phN - 1; i >= 0; --i) {
            if (PhIsVowel(phs[i])) { lastVowel = i; break; }
        }

        for (int i = 0; i < phN && e.speechN < MAX; i++) {
            Ph p = phs[i];

            int stress = 0;
            if (!stressed && PhIsVowel(p)) {
                stress = yell ? 2 : (isCmd ? 2 : 1);
                stressed = true;
            }

            // micro-prosody: tiny per-phoneme wobble so it’s less robotic
            float micro = 1.0f;
            if (PhIsVowel(p)) micro = 1.00f + 0.02f * sinf((float)i * 1.7f);
            else              micro = 0.99f + 0.01f * sinf((float)i * 2.1f);

            float hz = base * contour * cmdMul * micro * (stress ? 1.12f : 1.0f);

            // --- DURATIONS ---
            // Aim: clearer syllable chunks.
            int durMs = 90;

            if (PhIsVowel(p)) {
                durMs = (stress ? 210 : 165);       // vowels longer = more “word identity”
                if (i == lastVowel) durMs = (int)(durMs * 1.18f); // word-final lengthening
            }
            else if (PhIsStop(p)) {
                durMs = 70;                         // stops less “clicky”
            }
            else if (PhIsFric(p)) {
                durMs = 120;                        // frics longer = less “ticks”
            }
            else if (PhIsNasal(p)) {
                durMs = 130;
            }
            else {
                durMs = 100;                        // liquids/glides
            }

            pushPhTok(p, stress, hz, durMs);
        }

        // inter-word pause: smaller if we’re mid-phrase, bigger if cmd word
        pushPauseMs(isCmd ? 95 : 65);
    ...
    // parse text
    for (int i = 0; text[i] && e.speechN < MAX; ++i) {
        char c = text[i];

        if (c == ',') { flushWord(); pushPauseMs(140); phraseWordIndex = 0; continue; }
        if (c == '.') { flushWord(); pushPauseMs(190); phraseWordIndex = 0; continue; }
        if (c == '!') { flushWord(); pushPauseMs(210); phraseWordIndex = 0; continue; }
        if (c == '?') { flushWord(); pushPauseMs(220); phraseWordIndex = 0; continue; }
        ...
        // accumulate uppercase letters only
        if (wn < (int)sizeof(word) - 2) {
            word[wn++] = (char)toupper((unsigned char)c);
        }
    }
    flushWord();
...
enum class MixBus : int {
    ...
    Footsteps,
    Voice,
    WeaponsDistant,   // reserved for later
    VoiceDistant,     // reserved for later
    COUNT
};

static float gBusGain[(int)MixBus::COUNT] = {
    0.85f, // Master
    0.95f, // Weapons
    0.85f, // Impacts
    0.22f, // Footsteps  (start LOW)
    1.25f, // Voice      (start HIGH)
    1.59f, // WeaponsDistant (later)
    0.75f  // VoiceDistant (later)
};

static inline void Audio_PrintMix() {
    SDL_Log("MIX: Master=%.2f Weap=%.2f WeapDist=%.2f Imp=%.2f Step=%.2f Voice=%.2f",
        gBusGain[(int)MixBus::Master],
        gBusGain[(int)MixBus::Weapons],
        gBusGain[(int)MixBus::WeaponsDistant],
        gBusGain[(int)MixBus::Impacts],
        gBusGain[(int)MixBus::Footsteps],
        gBusGain[(int)MixBus::Voice]
    );
}

// ---------- Simple biquad bandpass ----------
struct ABiquad {
    ...
    static ABiquad bandpass(float fc, float Q, int sr) {
        ABiquad bi;
        float w0 = 2.0f * 3.14159265f * (fc / float(sr));
...
struct AVoice {
    ...
    float gain = 1.0f;
    float pan = 0.0f; // -1..+1
    float lp = 20000.0f; // not used yet, placeholder
    bool  alive = true;
    virtual ~AVoice() {}
    virtual void render(float* outL, float* outR, int frames, int sr) = 0;

    MixBus bus = MixBus::Master;
};

// Small container of voices
static std::vector<AVoice*> gVoices;

static float gLisX = 0, gLisY = 0;
static float gRightX = 1, gRightY = 0;

// ---------- Spatial helper  ----------
static inline bool Audio_Spatialize(float srcX, float srcY, float maxAudible,
    float& outPan, float& outGain, float& outDist)
{
    float dx = srcX - gLisX;
    float dy = srcY - gLisY;
    ...
    if (d > maxAudible) { outPan = 0.0f; outGain = 0.0f; return false; }

    // pan by projection onto right vector
    float pan = (dx * gRightX + dy * gRightY);
    pan = clampf(pan / 420.0f, -1.0f, 1.0f); // tune width
...
static inline bool isVowelChar(char c) {
    c = (char)tolower((unsigned char)c);
...
static inline uint8_t cc2_for_consonant(char c0, char c1) {
    c0 = (char)tolower((unsigned char)c0);
    c1 = (char)tolower((unsigned char)c1);

    // clusters first
    if (c0 == 't' && c1 == 'h') return 110; // Th (109–111)
    if (c0 == 's' && c1 == 'h') return 100; // Sh (99–101)
    if (c0 == 't' && c1 == 'r') return 107; // Tr (106–108)
    if (c0 == 'd' && c1 == 'r') return 24;  // Dr (23–26)
    if (c0 == 'b' && c1 == 'r') return 15;  // Br (14–16)
    if (c0 == 'f' && c1 == 'r') return 38;  // Fr (37–39)
    if (c0 == 'g' && c1 == 'r') return 48;  // Gr (46–49)
    if (c0 == 'k' && c1 == 'r') return 64;  // Kr (63–65)
    if (c0 == 'p' && c1 == 'r') return 87;  // Pr (86–88)
    if (c0 == 's' && c1 == 'l') return 97;  // Sl (96–98)
    if (c0 == 'f' && c1 == 'l') return 35;  // Fl (33–36)

    // singles
    switch (c0) {
    case 'b': return 5;   // B (4–6)
    case 'd': return 21;  // D (20–22)
    case 'f': return 28;  // F (27–29)
    case 'g': return 41;  // G (40–42)
    case 'h': return 52;  // H (50–52)
    case 'k': return 58;  // K (56–59)
    case 'l': return 67;  // L (66–68)
    case 'm': return 70;  // M (69–72)
    case 'n': return 74;  // N (73–75)
    case 'p': return 81;  // P (79–82)
    case 'r': return 90;  // R (89–91)
    case 's': return 94;  // S (92–95)
    case 't': return 103; // T (102–105)
    case 'v': return 116; // V (115–118)
    case 'w': return 120; // W (119–121)
    case 'y': return 123; // Y (122–124)
    default:  return 127; // None
    }
}

static inline uint8_t cc3_for_vowel(char v) {
    v = (char)tolower((unsigned char)v);
    switch (v) {
    case 'a': return 26;  // æ-ish (23–30) or ɑː (8–15)
    case 'e': return 35;  // ə (31–37) / eɪ (91–97)
    case 'i': return 80;  // iː (76–82)
    case 'o': return 56;  // ɒ (53–60) / ɔː (38–45)
    case 'u': return 66;  // ʌ (61–67) / uː (68–75)
    case 'y': return 80;  // treat like iː
    default:  return 0;   // random
    }
}

static inline void BuildSpeechTokens(AudioEvent& e, const char* text, bool yell)
{
    (void)yell; // we detect '!' inside the phoneme builder now
    BuildSpeechTokens(e, text); // calls the phoneme/prosody builder (new one)
}
...
//{
  //  e.speechN = 0;
    ...
    // Simple phrase style from punctuation
    //bool question = false;
    //for (const char* p = text; *p; ++p) if (*p == '?') question = true;

    // Faction base pitch (matches your “vibe” goal)
    //float basePitch = 250.0f;
    //if (e.faction == 0) basePitch = 205.f; // Axis deep
    //else if (e.faction == 1) basePitch = 255.f; // Allies refined
    //else if (e.faction == 2) basePitch = 235.f; // Rebels gravelly
    //else if (e.faction == 3) basePitch = 295.f; // Militia dopey/high

    //if (yell) basePitch *= 1.08f;

    // Walk text and form crude CV syllables
    //const int N = kMaxSpeechTok;
    //int i = 0;
    //int wordPos = 0;

    //auto pushPause = [&](int ms) {
      //  if (e.speechN >= N) return;
        //SpeechTok& t = e.speech[e.speechN++];
        //t.flags = 8 | (yell ? 16 : 0);
        //t.dur8 = (uint8_t)std::clamp(ms / 10, 2, 40);
        //t.cc2 = 127; t.cc3 = 127;
        //t.pitch8 = 64;
        //};

    //auto pushCV = [&](uint8_t cc2, uint8_t cc3, float pitchHz, bool stress) {
      //  if (e.speechN >= N) return;
        //SpeechTok& t = e.speech[e.speechN++];
        //t.cc2 = cc2;
        //t.cc3 = cc3;
        //t.flags = (stress ? 1 : 0) | (yell ? 16 : 0);
        //t.dur8 = (uint8_t)std::clamp((int)(95 / 10), 5, 18); // ~95ms base
        // map Hz to pitch8 crudely (more than good enough)
        //float p = std::clamp((pitchHz - 140.f) / (420.f - 140.f), 0.f, 1.f);
        //t.pitch8 = clampu8((int)(p * 127.f));
        //};

    // First pass: collect characters
    //char prevC = 0;
    //while (text[i] && e.speechN < N)
    //{
      //  char c = text[i];

        // punctuation pauses
       // if (c == ',') { pushPause(120); i++; continue; }
       // if (c == '.' || c == '!') { pushPause(160); i++; continue; }
       // if (c == '?') { pushPause(170); i++; continue; }
       ...
       // if (c == ' ' || c == '\n' || c == '\t') {
        ...
        // find a consonant (optional) then a vowel
        //uint8_t cc2 = 127;
        //uint8_t cc3 = 0;

        // consume consonant(s)
        //if (!isVowelChar(c)) {
          //  char c1 = text[i + 1] ? text[i + 1] : 0;
            //cc2 = cc2_for_consonant(c, c1);

            // if it was a recognized cluster, skip 2 letters
            //bool clustered =
              //  ((tolower(c) == 't' && tolower(c1) == 'h') ||
                //    (tolower(c) == 's' && tolower(c1) == 'h') ||
                  //  (tolower(c) == 't' && tolower(c1) == 'r') ||
                    //(tolower(c) == 'd' && tolower(c1) == 'r') ||
                    //(tolower(c) == 'b' && tolower(c1) == 'r') ||
                    //(tolower(c) == 'f' && tolower(c1) == 'r') ||
                    //(tolower(c) == 'g' && tolower(c1) == 'r') ||
                    //(tolower(c) == 'k' && tolower(c1) == 'r') ||
                    //(tolower(c) == 'p' && tolower(c1) == 'r') ||
                    //(tolower(c) == 's' && tolower(c1) == 'l') ||
                    //(tolower(c) == 'f' && tolower(c1) == 'l'));

          //  i += clustered ? 2 : 1;

            // skip any extra consonants until vowel or end (crudely)
           // while (text[i] && !isVowelChar(text[i]) && text[i] != ' ' && e.speechN < N) {
             //   i++;
            //}
       // }

        // consume vowel (or fallback)
       // if (text[i] && isVowelChar(text[i])) {
         //   cc3 = cc3_for_vowel(text[i]);
           // i++;
        //}
        //else {
            // no vowel found: nasal-ish "m" vowel makes it still talk
        //    cc3 = 118; // nasal vowel bucket (113–120)
        //}

        // prosody: stress first syllable of a word, plus exclamation/yell
        //bool stress = (prevC == ' ' || prevC == 0) || yell;
        //prevC = c;

        // pitch contour: statements fall, questions rise
        //float phraseT = (wordPos < 6) ? (wordPos / 6.0f) : 1.0f;
        //float contour = question ? (0.98f + 0.12f * phraseT) : (1.05f - 0.14f * phraseT);
        //float pitchHz = basePitch * contour * (stress ? 1.08f : 1.0f);

        //pushCV(cc2, cc3, pitchHz, stress);
    //}
...
// ---------- Shot voice ----------
struct ShotVoice : public AVoice {
    ABiquad bodyBP, mechBP;
    float phase = 0;
    float f0 = 180.0f;

    ShotVoice(int weaponId) {
        // Tune a little per-weapon-id to get uniqueness even before Lua.
        float w = float((weaponId % 17) + 1);
        float bodyHz = 220.0f + w * 28.0f;         // body resonance
        float mechHz = 1200.0f + w * 65.0f;        // mechanical click band
        bodyBP = ABiquad::bandpass(bodyHz, 1.6f, gAudioSR);
        mechBP = ABiquad::bandpass(mechHz, 5.0f, gAudioSR);
        f0 = 140.0f + w * 6.0f;
    ...
    void render(float* outL, float* outR, int frames, int sr) override {
        float dt = 1.0f / sr;
        for (int i = 0; i < frames; i++) {
            ...
            // envelopes
            float u = t / T;
            float env = (u < 0.08f) ? (u / 0.08f) : expf(-(u - 0.08f) * 7.5f); // fast attack, decay

            // transient (noise crack)
            float crack = arand11();
            crack *= expf(-u * 18.0f);

            // body: rectified sine + bp
            phase += (f0 * dt); if (phase >= 1.0f) phase -= 1.0f;
            float buzz = sinf(2.0f * 3.14159265f * phase);
           

            float body = bodyBP.tick(buzz) * 0.75f;

            // mech: noisy tick band
            float mech = mechBP.tick(arand11()) * expf(-u * 30.0f) * 0.6f;

            float s = (crack * 0.8f + body * 0.9f + mech) * env * gain;

            // pan
            float l = s * (0.7071f * (1.0f - pan));
            float r = s * (0.7071f * (1.0f + pan));
            outL[i] += l; outR[i] += r;
...
// ---------- Distant shot voice (knock / thump layer) ----------
struct DistantShotVoice : public AVoice {
    ABiquad thumpBP;
    ABiquad knockBP;
    float phase = 0.0f;
    float baseHz = 220.0f;

    DistantShotVoice(int weaponId, float distNorm) {
        // distNorm 0..1 where 1 = very far (we’ll compute it in spawn)
        float w = float((weaponId % 17) + 1);

        // Thump band: low-mid "knock"
        float thumpHz = 240.0f + w * 10.0f; // subtle weapon-to-weapon variance
        thumpBP = ABiquad::bandpass(thumpHz, 1.1f, gAudioSR);

        // Slight higher knock band for presence
        float knockHz = 520.0f + w * 18.0f;
        knockBP = ABiquad::bandpass(knockHz, 2.0f, gAudioSR);

        baseHz = 170.0f + w * 4.0f;

        // Longer than near shot, but still short
        T = 0.18f;

        // Start quieter; distant bus can be turned up/down in mixer
        // Make farther shots a bit softer (distance already attenuates too).
        gain = 0.55f * (1.0f - 0.25f * distNorm);

        bus = MixBus::WeaponsDistant;
    }
    ...
    void render(float* outL, float* outR, int frames, int sr) override {
        ...
        for (int i = 0; i < frames; ++i) {
            ...
            // Smoothish envelope (no sharp crack)
            float envA = (u < 0.06f) ? (u / 0.06f) : 1.0f;
            float envD = expf(-u * 5.5f);
            float env = envA * envD;

            // Low "air thump": rectified sine + noise into BP
            phase += baseHz * dt; if (phase >= 1.0f) phase -= 1.0f;
            float s0 = sinf(2.0f * 3.14159265f * phase);
            s0 = (s0 > 0 ? s0 : 0.35f * s0);

            float n = arand11() * 0.65f;

            float th = thumpBP.tick(s0 * 0.9f + n * 0.4f) * 0.95f;
            float kn = knockBP.tick(n) * expf(-u * 10.0f) * 0.55f;

            float s = (th + kn) * env * gain;
...
// ---------- Reload voice ----------
struct ReloadVoice : public AVoice {
    ABiquad clickBP;
    float  seed = 0;

    ReloadVoice(int weaponId) {
        float w = float((weaponId % 13) + 1);
        clickBP = ABiquad::bandpass(1400.0f + w * 80.0f, 6.0f, gAudioSR);
        T = 0.28f;
        gain = 0.55f;
    }
    ...
    void render(float* outL, float* outR, int frames, int sr) override {
        ...
        for (int i = 0; i < frames; i++) {
            ...
            // 3 little “clicks” spaced out
            float click = 0.0f;
            auto pulse = [&](float center, float width) {
                float d = fabsf(u - center);
                return (d < width) ? (1.0f - d / width) : 0.0f;
                };
            float p = pulse(0.18f, 0.06f) + pulse(0.45f, 0.07f) + pulse(0.78f, 0.08f);
            if (p > 0.0f) click = clickBP.tick(arand11()) * p;

            float env = expf(-u * 1.5f);
            float s = click * env * gain;
...
// ---------- Footstep voice ----------
struct StepVoice : public AVoice {
    ABiquad thumpBP;
    float pitch = 1.0f;
    float noiseAmt = 1.0f;

    StepVoice(float speedNorm, float var01, float lrSign) {
        // speedNorm 0..1 influences weight
        float thumpHz = 120.0f + speedNorm * 60.0f;

        // correlated variation (-1..+1)
        float r = (var01 * 2.0f - 1.0f);

        // pitch wobble (±5%)
        pitch = 1.0f + r * (0.03f + 0.02f * speedNorm);

        // "crunch" wobble (±20%)
        noiseAmt = 1.0f + r * 0.20f;

        // tiny L/R foot bias (±0.05 pan)
        pan += lrSign * 0.05f;

        thumpBP = ABiquad::bandpass(thumpHz * pitch, 1.2f, gAudioSR);
        T = 0.11f;

        // gentler + subtle amp wobble (±4%)
        float ampMul = 1.0f + r * 0.04f;
        gain = (0.14f + 0.10f * speedNorm) * ampMul;
    }
    ...
    void render(float* outL, float* outR, int frames, int sr) override {
        ...
        for (int i = 0; i < frames; i++) {
            ...
            // Slight attack softness variation via noiseAmt (crunchier = sharper)
            float atk = 0.03f + (1.0f - clampf(noiseAmt, 0.8f, 1.2f)) * 0.01f; // ~0.02–0.04
            float envA = (u < atk) ? (u / atk) : 1.0f;
            float envD = expf(-u * 10.0f);
            float env = envA * envD;

            float s = thumpBP.tick(arand11() * 0.9f * noiseAmt) * env * gain;
...
// ---------- Impact voices ----------
struct HitSolidVoice : public AVoice {
    ABiquad ringBP;
    float ringPhase = 0, ringHz = 1400.0f;
    HitSolidVoice(int weaponId, float energy) {
        float w = float((weaponId % 11) + 1);
        ringHz = 900.0f + w * 120.0f;
        ringBP = ABiquad::bandpass(ringHz, 7.0f, gAudioSR);
        T = 0.16f;
        gain = 0.35f + 0.35f * clampf(energy, 0, 1);
    }
    void render(float* outL, float* outR, int frames, int sr) override {
        float dt = 1.0f / sr;
        for (int i = 0; i < frames; i++) {
            if (!alive) break;
            float u = t / T;
            float crack = arand11() * expf(-u * 25.0f);
            ringPhase += ringHz * dt; if (ringPhase > 1.0f) ringPhase -= 1.0f;
            float ring = sinf(2.0f * 3.14159265f * ringPhase);
            float s = (crack * 0.6f + ringBP.tick(ring) * 0.35f) * expf(-u * 6.0f) * gain;
            float l = s * (0.7071f * (1.0f - pan)); float r = s * (0.7071f * (1.0f + pan));
            outL[i] += l; outR[i] += r;
            t += dt;
            if (t >= T) alive = false;
        }
...
struct HitFleshVoice : public AVoice {
    ABiquad thudBP;
    HitFleshVoice(float energy) {
        thudBP = ABiquad::bandpass(170.0f, 1.0f, gAudioSR);
        T = 0.14f;
        gain = 0.30f + 0.35f * clampf(energy, 0, 1);
    }
    void render(float* outL, float* outR, int frames, int sr) override {
        float dt = 1.0f / sr;
        for (int i = 0; i < frames; i++) {
            if (!alive) break;
            float u = t / T;
            float env = expf(-u * 9.0f);
            float s = thudBP.tick(arand11() * 0.9f) * env * gain;
            float l = s * (0.7071f * (1.0f - pan)); float r = s * (0.7071f * (1.0f + pan));
            outL[i] += l; outR[i] += r;
            t += dt;
            if (t >= T) alive = false;
...
// ---------- Bark voice (placeholder “speech-ish”) ----------
//struct BarkVoice : public AVoice {
  //  ABiquad f1, f2, f3;
    //float phase = 0;
    //float f0 = 250.0f;
    //BarkVoice(int faction) {
      //  // rough faction color:
        // Axis deep, Rebels gravelly, Militia dopey/high, Allies refined/high
       // float form = 1.0f;
       // if (faction == 0) form = 0.92f;        // Axis (example enum order)
       // else if (faction == 1) form = 0.98f;   // Allies
       // else if (faction == 2) form = 1.03f;   // Rebels
       // else if (faction == 3) form = 1.10f;   // Militia

       // f1 = ABiquad::bandpass(500.0f * form, 6.0f, gAudioSR);
        //f2 = ABiquad::bandpass(1500.0f * form, 6.0f, gAudioSR);
        //f3 = ABiquad::bandpass(2500.0f * form, 6.0f, gAudioSR);

        //f0 = (faction == 0) ? 210.0f : (faction == 3 ? 310.0f : 260.0f);
        //T = 0.22f;
        //gain = 0.25f;
    //}
    //void render(float* outL, float* outR, int frames, int sr) override {
      //  float dt = 1.0f / sr;
        //for (int i = 0; i < frames; i++) {
          //  if (!alive) break;
            //float u = t / T;
            //float env = (u < 0.12f) ? (u / 0.12f) : expf(-(u - 0.12f) * 8.0f);

            // glottal-ish source + noise
            //phase += f0 * dt; if (phase >= 1) phase -= 1;
            //float buzz = sinf(2 * 3.14159265f * phase);
            //buzz = (buzz > 0 ? buzz : 0.55f * buzz);
            //float src = (buzz * 0.8f + arand11() * 0.25f);

            //float s = (0.45f * f1.tick(src) + 0.35f * f2.tick(src) + 0.20f * f3.tick(src)) * env * gain;
            //float l = s * (0.7071f * (1.0f - pan)); float r = s * (0.7071f * (1.0f + pan));
            //outL[i] += l; outR[i] += r;
...
// ---------- SpeechVoice (token-driven phoneme synth, open-air) ----------
static inline void PhFormants(Ph p, float& F1, float& F2, float& F3) {
    // Defaults: schwa-ish
    F1 = 520; F2 = 1500; F3 = 2500;

    switch (p) {
    case Ph::AA: F1 = 700; F2 = 1100; F3 = 2450; break;
    case Ph::AE: F1 = 660; F2 = 1700; F3 = 2500; break;
    case Ph::AH: F1 = 600; F2 = 1400; F3 = 2500; break;
    case Ph::AO: F1 = 500; F2 = 900; F3 = 2400; break;
    case Ph::AW: F1 = 620; F2 = 1150; F3 = 2450; break;
    case Ph::AY: F1 = 520; F2 = 1850; F3 = 2750; break;
    case Ph::EH: F1 = 530; F2 = 1850; F3 = 2500; break;
    case Ph::ER: F1 = 470; F2 = 1350; F3 = 2100; break;
    case Ph::EY: F1 = 480; F2 = 2050; F3 = 2700; break;
    case Ph::IH: F1 = 420; F2 = 2000; F3 = 2850; break;
    case Ph::IY: F1 = 300; F2 = 2200; F3 = 3000; break;
    case Ph::OW: F1 = 450; F2 = 950; F3 = 2400; break;
    case Ph::OY: F1 = 520; F2 = 1100; F3 = 2400; break;
    case Ph::UH: F1 = 420; F2 = 1400; F3 = 2500; break;
    case Ph::UW: F1 = 350; F2 = 900; F3 = 2200; break;
    default: break;
    }
}

static inline void PhNoiseBands(Ph p, float& n1, float& n2, float& mix) {
    // open-air: softer, less “radio hiss”
    n1 = 1800; n2 = 4200; mix = 0.55f;

    switch (p) {
    case Ph::S:  n1 = 4200; n2 = 7800; mix = 0.90f; break;
    case Ph::Z:  n1 = 3800; n2 = 7200; mix = 0.80f; break;
    case Ph::SH: n1 = 2500; n2 = 5200; mix = 0.85f; break;
    case Ph::ZH: n1 = 2400; n2 = 4800; mix = 0.75f; break;
    case Ph::F:  n1 = 1200; n2 = 3800; mix = 0.75f; break;
    case Ph::V:  n1 = 1100; n2 = 3300; mix = 0.65f; break;
    case Ph::TH: n1 = 1400; n2 = 4200; mix = 0.70f; break;
    case Ph::DH: n1 = 1300; n2 = 3600; mix = 0.60f; break;
    case Ph::HH: n1 = 900; n2 = 2200; mix = 0.60f; break;
    default: break;
    }
}

struct SpeechVoice : public AVoice {
    // vowel formants
    ABiquad f1, f2, f3;
    // noise shaping for frics
    ABiquad nBP1, nBP2;

    float nLP = 0.0f;   // low-passed noise state
    float vLP = 0.0f;   // voiced lowpass state


    // token store
    std::array<SpeechTok, kMaxSpeechTok> toks{};
    int   tokN = 0;
    int   tokI = 0;
    float tokT = 0.0f;
    float tokDur = 0.08f;
    ...
    // synthesis state
    float phase = 0.0f;
    float curF0 = 180.0f, tgtF0 = 180.0f;

    float curF1 = 520, curF2 = 1500, curF3 = 2500;
    float tgtF1 = 520, tgtF2 = 1500, tgtF3 = 2500;

    int faction = 0;

    void setTokens(const SpeechTok* t, int n) {
        tokN = std::clamp(n, 0, kMaxSpeechTok);
        for (int i = 0; i < tokN; i++) toks[i] = t[i];
        tokI = 0;
        tokT = 0.0f;
        tokDur = (tokN > 0) ? (toks[0].dur8 * 0.01f) : 0.05f;

        float total = 0.f;
        for (int i = 0; i < tokN; i++) total += toks[i].dur8 * 0.01f;
        T = std::max(0.05f, total);
    }

    SpeechVoice(int faction_, uint32_t textHash) {
        (void)textHash;
        faction = faction_;

        // open-air natural: slightly quieter than your earlier barks (tune with mix bus)
        gain = 0.24f;

        // initialize filters to defaults
        f1 = ABiquad::bandpass(curF1, 6.5f, gAudioSR);
        f2 = ABiquad::bandpass(curF2, 7.0f, gAudioSR);
        f3 = ABiquad::bandpass(curF3, 7.5f, gAudioSR);

        nBP1 = ABiquad::bandpass(2200.f, 2.0f, gAudioSR);
        nBP2 = ABiquad::bandpass(5200.f, 2.0f, gAudioSR);
    }

    inline Ph curPh() const { return (tokI >= 0 && tokI < tokN) ? toks[tokI].ph : Ph::SIL; }

    inline int findNextVowel(int start) const {
        for (int i = start; i < tokN; i++) {
            if (PhIsVowel(toks[i].ph)) return i;
        }
        return -1;
    }
    ...
    void render(float* outL, float* outR, int frames, int sr) override {
        ...
        for (int i = 0; i < frames; i++) {
            ...
            // token advance / entry
            if (tokI < tokN) {
                tokDur = std::max(0.02f, toks[tokI].dur8 * 0.01f);
                ...
                if (tokT <= 0.00001f) {
                    ...
                    // pitch
                    tgtF0 = F0_8ToHz(toks[tokI].f0_8);

                    // look-ahead coarticulation: pull vowels slightly toward next vowel
                    float F1, F2, F3;
                    if (PhIsVowel(p)) {
                        PhFormants(p, F1, F2, F3);

                        int nv = findNextVowel(tokI + 1);
                        if (nv >= 0) {
                            float nF1, nF2, nF3;
                            PhFormants(toks[nv].ph, nF1, nF2, nF3);
                            // open-air: mild coarticulation (keeps words clearer)
                            float mix = 0.18f;
                            F1 = F1 * (1.f - mix) + nF1 * mix;
                            F2 = F2 * (1.f - mix) + nF2 * mix;
                            F3 = F3 * (1.f - mix) + nF3 * mix;
                        }
                        tgtF1 = F1; tgtF2 = F2; tgtF3 = F3;
                    }
                    else {
                        // consonants: keep formants drifting gently toward next vowel
                        int nv = findNextVowel(tokI + 1);
                        if (nv >= 0) {
                            float nF1, nF2, nF3;
                            PhFormants(toks[nv].ph, nF1, nF2, nF3);
                            tgtF1 = nF1; tgtF2 = nF2; tgtF3 = nF3;
                        }
                    }

                    // rebuild fricative noise bands per phoneme
                    float nb1, nb2, nmix;
                    PhNoiseBands(p, nb1, nb2, nmix);
                    nBP1 = ABiquad::bandpass(nb1, 2.0f, gAudioSR);
                    nBP2 = ABiquad::bandpass(nb2, 2.0f, gAudioSR);
                }
            }

            // smooth pitch/formants
            curF0 += (tgtF0 - curF0) * 0.02f;
            curF1 += (tgtF1 - curF1) * 0.06f;
            curF2 += (tgtF2 - curF2) * 0.06f;
            curF3 += (tgtF3 - curF3) * 0.06f;

            if ((i & 31) == 0) {
                f1 = ABiquad::bandpass(curF1, 8.0f, gAudioSR);
                f2 = ABiquad::bandpass(curF2, 9.0f, gAudioSR);
                f3 = ABiquad::bandpass(curF3, 10.0f, gAudioSR);
            }

            // envelopes
            float s = 0.0f;
            Ph p = (tokI < tokN) ? toks[tokI].ph : Ph::SIL;

            // pause token
            if (p == Ph::PAU || p == Ph::SIL || tokI >= tokN) {
                s = 0.0f;
            }
            else {
                float u = tokT / std::max(0.001f, tokDur);

                // per-phoneme envelope
                float atk = 0.10f, rel = 0.18f;
                if (PhIsStop(p)) { atk = 0.02f; rel = 0.20f; }
                if (PhIsFric(p)) { atk = 0.06f; rel = 0.14f; }
                if (PhIsVowel(p)) { atk = 0.16f; rel = 0.22f; }

                float env = 1.0f;
                if (u < atk) env = u / atk;
                else if (u > (1.0f - rel)) env = std::max(0.0f, (1.0f - u) / rel);

                // stress adds a little amplitude + clarity
                float stressMul = 1.0f;
                if (tokI < tokN) {
                    if (toks[tokI].stress == 1) stressMul = 1.10f;
                    if (toks[tokI].stress >= 2) stressMul = 1.20f;
                }
                ...
                // source
                float noise = arand11();
                nLP += (noise - nLP) * 0.08f;   // 0.05..0.12 range; lower = smoother
                float nSoft = nLP;
                // slight breathiness to reduce “pure buzzy synth”
               
                // --- glottal-ish source (less ring-mod, more throat) ---
                phase += curF0 * dt;
                if (phase >= 1.0f) phase -= 1.0f;

                // a softer pulse: fast rise, slower fall
                float x = phase; // 0..1
                float pulse = 0.0f;
                if (x < 0.22f) {
                    // quick opening
                    ...
                    // slower closing
                    float y = (x - 0.22f) / (1.0f - 0.22f);
                    pulse = expf(-y * 4.2f);
                }

                // add tiny jitter (stops it “locking” like a synth)
                float jitter = arand11() * 0.0065f; // 0.003..0.01
                pulse += jitter;

                // breath/aspiration mixed into voiced signal
                float breath = nSoft * 0.11f;        // low-passed noise already computed
                float buzz = pulse * 0.92f + breath; // voiced + air



                float voiced = 0.0f;

                bool voicedOn = PhIsVoiced(p);
                ...
                float burst = 0.0f;
                if (PhIsStop(p)) {
                    // shorter + quieter burst = less clicky
                    if (u < 0.08f) burst = (1.0f - u / 0.08f) * 0.45f;
                }
                ...
                float fric = (nBP1.tick(nSoft) * 0.55f + nBP2.tick(nSoft) * 0.45f);


                // mix voiced/noise depending on phoneme class
                float vMix = 0.0f;
                float nMix = 0.0f;
                ...
                if (PhIsVowel(p)) {
                    ...
                    // frics should be mostly shaped noise
                    vMix = voicedOn ? 0.16f : 0.05f;
                    nMix = nmix * 0.62f;   // soften hiss further
                }
                else if (PhIsStop(p)) {
                    // stops should be short + mostly noisy burst, not a buzzy “note”
                    vMix = voicedOn ? 0.28f : 0.08f;
                    nMix = 0.18f + 0.34f * burst;
                }
                ...
                float src = voiced * vMix + fric * nMix;

                // Vowel resonator
                float form = (0.48f * f1.tick(src) + 0.34f * f2.tick(src) + 0.18f * f3.tick(src));


                // gentle lowpass / spectral tilt (reduces buzzy edge)
                vLP += (form - vLP) * 0.14f;     // 0.10..0.20
                form = 0.72f * form + 0.28f * vLP;


                // nasal softening (more low / less high)
                if (PhIsNasal(p)) {
                    form = form * 0.92f;
                }

                // stop burst should be more noisy than resonant
                if (PhIsStop(p) && burst > 0.0f) {
                    form = form * 0.88f + fric * (0.12f + 0.18f * burst);
                }


                // open-air: soften the top a tiny bit (less “talkbox”)
                float air = 0.90f + 0.10f * expf(-u * 2.0f);
                s = form * env * stressMul * air;
            }

            // global phrase envelope
            float ug = (T > 0.001f) ? (t / T) : 1.0f;
            float gA = (ug < 0.05f) ? (ug / 0.05f) : 1.0f;
            float gD = expf(-ug * 0.9f);
            float gEnv = gA * gD;

            float out = s * gEnv * gain;

            float l = out * (0.7071f * (1.0f - pan));
            float r = out * (0.7071f * (1.0f + pan));
            outL[i] += l;
            outR[i] += r;
...
static void Audio_Spawn(const AudioEvent& e) {
    float pan = 0.0f, att = 1.0f, dist = 0.0f;
    float audible = e.maxAudible;

    // Let gunfire always spawn (very quiet when far)
    if (e.type == AudioEvent::Fire)
        audible = std::max(audible, 5000.0f); // or map size

    if (!Audio_Spatialize(e.x, e.y, audible, pan, att, dist)) return;
    ...
    // 1) Create the voice
    switch (e.type) {
    case AudioEvent::Fire: {
        // Distance threshold: past this, swap to distant "knock" layer
        // Use a % of audible radius so loud weapons carry farther before changing timbre.
        float nearFrac = 0.35f; // tune 0.30–0.45
        float thr = e.maxAudible * nearFrac;

        float distNorm = clampf((dist - thr) / std::max(1.0f, e.maxAudible - thr), 0.0f, 1.0f);

        if (dist > thr) {
            v = new DistantShotVoice(e.weaponId, distNorm);
            // bus already set in constructor to WeaponsDistant
        }
        else {
            v = new ShotVoice(e.weaponId);
        }
    } break;

    case AudioEvent::Reload:   v = new ReloadVoice(e.weaponId); break;
    case AudioEvent::Footstep: {
        float lrSign = (e.p1 < 0.0f) ? -1.0f : +1.0f;
        float var01 = fabsf(e.p1); // 0..1
        v = new StepVoice(e.p0, var01, lrSign);
    } break;

    case AudioEvent::HitSolid: v = new HitSolidVoice(e.weaponId, e.p0); break; // p0 = energy
    case AudioEvent::HitFlesh: v = new HitFleshVoice(e.p0); break;          // p0 = energy
    case AudioEvent::Bark: {
        auto* sv = new SpeechVoice(e.faction, e.textHash);
        sv->setTokens(e.speech, e.speechN);
        v = sv;
        v->bus = MixBus::Voice;
    } break;
    ...
    // 2) Assign bus (do not override voices that already chose a bus)
    if (v->bus == MixBus::Master) {
        switch (e.type) {
        case AudioEvent::Fire:     v->bus = MixBus::Weapons;   break;
        case AudioEvent::Reload:   v->bus = MixBus::Weapons;   break;
        case AudioEvent::Footstep: v->bus = MixBus::Footsteps; break;
        case AudioEvent::HitSolid: v->bus = MixBus::Impacts;   break;
        case AudioEvent::HitFlesh: v->bus = MixBus::Impacts;   break;
        case AudioEvent::Bark:     v->bus = MixBus::Voice;     break;
        default:                   v->bus = MixBus::Master;    break;
        }
    }
    ...
    // 4) Apply attenuation + bus gains (spawn-time)
    // 4) changed to use att not gain
    float g = att * gBusGain[(int)MixBus::Master] * gBusGain[(int)v->bus];
    v->gain *= g;
...
static void Audio_Mix(float* stream, int frames) {
    static std::vector<float> L, R;
    if ((int)L.size() < frames) { L.assign(frames, 0.f); R.assign(frames, 0.f); }
    ...
    // Consume queued events
    {
        ...
        while (!gAudioQ.empty()) {
            Audio_Spawn(gAudioQ.front());
            gAudioQ.pop_front();
        }
    }

    // Render voices
    for (size_t i = 0; i < gVoices.size();) {
        AVoice* v = gVoices[i];
        v->render(L.data(), R.data(), frames, gAudioSR);
        if (!v->alive) {
            delete v;
            gVoices[i] = gVoices.back();
            gVoices.pop_back();
        }
        else {
            ++i;
    ...
    // Output interleaved stereo + soft limiter
    for (int i = 0; i < frames; i++) {
        float l = L[i], r = R[i];
        // soft clip
        l = clampf(l, -0.98f, 0.98f);
        r = clampf(r, -0.98f, 0.98f);
        stream[2 * i + 0] = l;
        stream[2 * i + 1] = r;
    }
}

static void SDLCALL Audio_Callback(void* userdata, Uint8* bytes, int len) {
    float* stream = (float*)bytes;
    int frames = len / (int)(sizeof(float) * 2);
    Audio_Mix(stream, frames);
}

// ---------- Public-ish audio API ----------
static bool Audio_Init() {
    if (gAudioDev) return true;

    SDL_AudioSpec want{}, have{};
    want.freq = 48000;
    want.format = AUDIO_F32SYS;
    want.channels = 2;
    want.samples = 256;            // low latency
    want.callback = Audio_Callback;

    gAudioDev = SDL_OpenAudioDevice(nullptr, 0, &want, &have, 0);
    if (!gAudioDev) {
        SDL_Log("Audio_Init: SDL_OpenAudioDevice failed: %s", SDL_GetError());
        return false;
    }

    SDL_Log("have: freq=%d fmt=%d ch=%d samples=%d",
        have.freq, have.format, have.channels, have.samples);


    gAudioSR = have.freq;
    SDL_PauseAudioDevice(gAudioDev, 0);
    SDL_Log("Audio_Init OK: %d Hz, %d ch, samples=%d", have.freq, have.channels, have.samples);
    return true;
}
...
static inline void Audio_Post(AudioEvent e) {
    if (!gAudioDev) return;
    std::lock_guard<std::mutex> lk(gAudioMx);
    // cap queue
    if (gAudioQ.size() > 256) gAudioQ.pop_front();
    gAudioQ.push_back(e);
}

static inline void Audio_PostFire(int weaponId, float x, float y, int faction, float audible) {
    AudioEvent e; e.type = AudioEvent::Fire; e.weaponId = weaponId; e.x = x; e.y = y; e.faction = faction;
    e.maxAudible = audible;
    Audio_Post(e);
}
static inline void Audio_PostReload(int weaponId, float x, float y, int faction) {
    AudioEvent e; e.type = AudioEvent::Reload; e.weaponId = weaponId; e.x = x; e.y = y; e.faction = faction;
    Audio_Post(e);
}
static inline void Audio_PostFootstep(float x, float y, float speedNorm, int faction, float var01, float lrSign) {
    AudioEvent e; e.type = AudioEvent::Footstep; e.x = x; e.y = y; e.p0 = speedNorm; e.p1 = var01; e.faction = faction;
    e.maxAudible = 240.0f;
    // store lrSign in p1? We already used p1. So: encode lrSign into p1 sign:
    // We'll do p1 = var01 * (lrSign < 0 ? -1 : +1)
    e.p1 = var01 * (lrSign < 0 ? -1.0f : +1.0f);
    Audio_Post(e);
}

static inline void Audio_PostHitSolid(int weaponId, float x, float y, float energy) {
    AudioEvent e; e.type = AudioEvent::HitSolid; e.weaponId = weaponId; e.x = x; e.y = y; e.p0 = energy;
    Audio_Post(e);
    e.maxAudible = 520.0f; // impacts
}
static inline void Audio_PostHitFlesh(float x, float y, float energy) {
    AudioEvent e; e.type = AudioEvent::HitFlesh; e.x = x; e.y = y; e.p0 = energy;
    Audio_Post(e);
    e.maxAudible = 520.0f; // impacts
}
static inline void Audio_PostBark(float x, float y, int faction, const char* text) {

    if (!gSpeechAudioEnabled) return;  // <--- ADD THIS LINE

    AudioEvent e;
    e.type = AudioEvent::Bark;
    e.x = x; e.y = y;
    e.faction = faction;
    e.textHash = fnv1a32(text);
    e.maxAudible = 520.0f;

    BuildSpeechTokens(e, text);
...
static void Audio_UpdateFootstepsFor(Actor& a, const Vec2& vel, float walkSpeed, float dt) {
    float sp = length(vel);
    float n = (walkSpeed > 0.01f) ? clampf(sp / walkSpeed, 0.f, 1.f) : 0.f;

    // Don't step for micro-movement or aiming wiggles
    if (n < 0.28f) { a.footAccS = 0.0f; return; }

    // Base interval: walk slower, sprint faster
    float base = 0.64f - 0.32f * n;          // walk ~0.64, sprint ~0.32
    base = clampf(base, 0.28f, 0.75f);

    // If footNextS not initialized, seed it
    if (a.footNextS <= 0.01f) a.footNextS = base;

    a.footAccS += dt;
    if (a.footAccS >= a.footNextS) {
        a.footAccS = 0.0f;

        float var01 = arand01();

        // Derive a consistent left/right toggle from time accumulator (good enough)
        float lrSign = (fmodf(a.footNextS * 1000.0f, 2.0f) < 1.0f) ? -1.0f : +1.0f;


        // post footstep (audible max set inside Audio_PostFootstep)
        Audio_PostFootstep(a.pos.x, a.pos.y, n, (int)a.team, var01, lrSign);

        // jitter keeps crowds from phase-locking
        float jitter = 0.05f + 0.08f * (1.0f - n);
        a.footNextS = base + frand(-jitter, jitter);
    }
}

static inline void Audio_SetListener(float x, float y, float facingX, float facingY) {
    gLisX = x; gLisY = y;
    // facing -> right (perp-right)
    float fx = facingX, fy = facingY;
...
static inline void Mix_Bump(MixBus b, float d) {
    float& v = gBusGain[(int)b];
    v = std::clamp(v + d, 0.0f, 2.0f);
}
...
bool Game::initSDL() {
    // ✅ Add SDL_INIT_AUDIO (and TIMER is a nice safe add for audio timing)
    ...
    // ✅ Start audio AFTER SDL_Init includes SDL_INIT_AUDIO
    Audio_Init();
    SDL_Log("Audio dev id = %u", (unsigned)gAudioDev);
    ...
    // ❌ REMOVE THIS — it kills the audio immediately
    // Audio_Shutdown();
...
void Game::initWorld() {
    ...
    // Use your global tuning knobs (keeps v7 consistent)
    ...
    // Table-driven loadout (primary + optional secondary)
    applyPlayerLoadout(player);

    // Build hit rig now so damage zones work immediately
    buildDefaultHitRig(player);

    // Sync legacy Gun mirror to the ACTIVE weapon (so old HUD bits don't lie)
    // dt=0 is fine; it updates nothing but keeps your mirror consistent.
    weaponUpdate(player, 0.0f);

    // --- Default zoom baseline (tight) ---
    zoomNormal = cfg::ZoomSandbox;     // your baseline zoom (now tight)
    zoom = zoomNormal;

    // --- Center camera using zoom-aware half-screen ---
    camX = player.pos.x - (cfg::ScreenW / (2.0f * zoom));
    camY = player.pos.y - (cfg::ScreenH / (2.0f * zoom));
...
bool Game::init() {
    ...
    muzzleFlashes.resize(256); // fixed pool (feel free: 128..512)
    impacts.resize(256);       // fixed pool
    casings.resize(600);       // fixed pool

    SDL_Log("FX POOLS READY: muzzles=%d impacts=%d casings=%d",
        (int)muzzleFlashes.size(),
        (int)impacts.size(),
        (int)casings.size());


    for (auto& m : muzzleFlashes) m.ttl = 0.0f;
    for (auto& i : impacts) i.ttl = 0.0f;
    for (auto& c : casings) c.ttl = 0.0f;

    muzzleWrite = impactWrite = casingWrite = 0;
...
void Game::updateCamera(float dt) {
    ...
    Vec2 camCenter = player.pos;

    if (adsHeld && playerPresent && player.alive()) {
        float zoomMul = 1.20f, pull = 0.22f;
        adsParamsForWeapon(player.weapon.id, zoomMul, pull);

        Vec2 aim{ (float)worldMouseX, (float)worldMouseY };

        // True ADS: center between player and crosshair, weighted by weapon pull
        // pull ~0.2 pistols/SMG, ~0.3 rifles
        camCenter = player.pos + (aim - player.pos) * pull;
    }


    float targetX = camCenter.x - visibleW * 0.5f;
    float targetY = camCenter.y - visibleH * 0.5f;
...
// -----------------------------------------------------------
// Held-weapon primitive drawing (Seam B)
// -----------------------------------------------------------
void Game::drawHeldWeapon(const Actor& a, Vec2 fwdOverride) {
    WeaponId wid = activeW(a).id;
    if (wid == WeaponId::None) return;

    const WeaponDef& wd = weaponDef(wid);

    Vec2 fwd = fwdOverride;
    if (length(fwd) < 0.001f) fwd = Vec2{ 1, 0 };
    Vec2 right = perpRight(fwd);

    float len = 10.0f + std::clamp(wd.maxRange * 0.020f, 0.0f, 18.0f);
    float thick = 2.0f + std::clamp(wd.baseDamage * 0.030f, 0.0f, 4.0f);

    if (wd.magSize <= 10 && wd.maxRange < 260.0f) {
        len *= 0.60f;
        thick = std::max(2.0f, thick * 0.75f);
    }
    if (wd.pellets > 1) {
        thick += 1.0f;
        len += 2.0f;
    ...
    Vec2 muzzleBase = a.pos + fwd * (a.w * 0.30f);
    muzzleBase = muzzleBase + right * (a.h * 0.10f);

    Vec2 barrelStart = muzzleBase;
    Vec2 barrelEnd = barrelStart + fwd * len;

    SDL_SetRenderDrawColor(renderer, 25, 25, 25, 255);

    int passes = (int)std::round(thick);
    passes = std::clamp(passes, 2, 7);
    float half = (passes - 1) * 0.5f;
    for (int i = 0; i < passes; ++i) {
        float off = (i - half) * 0.8f;
        Vec2 o = right * off;
        SDL_RenderDrawLineF(renderer,
            (barrelStart.x + o.x) - camX, (barrelStart.y + o.y) - camY,
            (barrelEnd.x + o.x) - camX, (barrelEnd.y + o.y) - camY
        );
    }

    Vec2 recvC = barrelStart + fwd * (len * 0.25f);
    SDL_FRect recv;
    recv.w = 6.0f; recv.h = 4.0f;
    recv.x = (recvC.x - recv.w * 0.5f) - camX;
    recv.y = (recvC.y - recv.h * 0.5f) - camY;
    SDL_RenderFillRectF(renderer, &recv);

    Vec2 stockC = barrelStart - fwd * 3.5f;
    SDL_FRect stock;
    stock.w = 7.0f; stock.h = 3.5f;
    stock.x = (stockC.x - stock.w * 0.5f) - camX;
    stock.y = (stockC.y - stock.h * 0.5f) - camY;
    SDL_RenderFillRectF(renderer, &stock);

    bool autoish = (wd.fireCooldownS > 0.0f && wd.fireCooldownS < 0.14f);
    bool bigMag = (wd.magSize >= 20);
    if (autoish || bigMag) {
        Vec2 magC = recvC + right * 4.0f;
        SDL_FRect mag;
        mag.w = 2.5f; mag.h = 7.0f;
        mag.x = (magC.x - mag.w * 0.5f) - camX;
        mag.y = (magC.y - mag.h * 0.5f) - camY;
        SDL_RenderFillRectF(renderer, &mag);
    }
}

// Wrapper keeps old call sites working
void Game::drawHeldWeapon(const Actor& a) {
    Vec2 fwd = normalize(a.facing);
    if (a.lastShotT > 0.0f) fwd = normalize(a.lastShotDir);
    this->drawHeldWeapon(a, fwd);
}


void Game::drawWeaponSilhouetteAt(const Vec2& pos, WeaponId wid, const Vec2& dir, float scale) {
    if (wid == WeaponId::None) return;
    const WeaponDef& wd = weaponDef(wid);

    Vec2 fwd = normalize(dir);
    if (length(fwd) < 0.001f) fwd = Vec2{ 1,0 };
    Vec2 right = perpRight(fwd);

    float len = (10.0f + std::clamp(wd.maxRange * 0.020f, 0.0f, 18.0f)) * scale;
    float thick = (2.0f + std::clamp(wd.baseDamage * 0.030f, 0.0f, 4.0f)) * scale;

    // Pistols shorter
    if (wd.magSize <= 10 && wd.maxRange < 260.0f) { len *= 0.60f; thick *= 0.75f; }
    // Shotguns chunkier
    if (wd.pellets > 1) { thick += 1.0f * scale; len += 2.0f * scale; }

    SDL_SetRenderDrawColor(renderer, 25, 25, 25, 255);

    Vec2 barrelStart = pos;
    Vec2 barrelEnd = pos + fwd * len;

    int passes = std::clamp((int)std::round(thick), 2, 7);
    float half = (passes - 1) * 0.5f;
    for (int i = 0; i < passes; ++i) {
        float off = (i - half) * (0.8f * scale);
        Vec2 o = right * off;
        SDL_RenderDrawLineF(renderer,
            (barrelStart.x + o.x) - camX, (barrelStart.y + o.y) - camY,
            (barrelEnd.x + o.x) - camX, (barrelEnd.y + o.y) - camY
...
void Game::drawMuzzleFlashes() {
    for (const auto& mf : muzzleFlashes) {
        
        if (mf.ttl <= 0.0f) continue;

        Vec2 d = normalize(mf.dir);
        if (length(d) < 0.001f) d = Vec2{ 1,0 };
        Vec2 r = perpRight(d);

        Vec2 tip = mf.pos + d * mf.size;
        Vec2 a = mf.pos + r * (mf.size * 0.35f);
        Vec2 b = mf.pos - r * (mf.size * 0.35f);

        // Pale flash
        SDL_SetRenderDrawColor(renderer, 235, 225, 180, 255);

        // “Filled triangle” approximation: draw multiple outlines
        for (int p = 0; p < mf.passes; ++p) {
            float off = (p - (mf.passes - 1) * 0.5f) * 0.6f;
            Vec2 o = r * off;

            SDL_RenderDrawLineF(renderer, (mf.pos.x + o.x) - camX, (mf.pos.y + o.y) - camY,
                (tip.x + o.x) - camX, (tip.y + o.y) - camY);
            SDL_RenderDrawLineF(renderer, (a.x + o.x) - camX, (a.y + o.y) - camY,
                (tip.x + o.x) - camX, (tip.y + o.y) - camY);
            SDL_RenderDrawLineF(renderer, (b.x + o.x) - camX, (b.y + o.y) - camY,
                (tip.x + o.x) - camX, (tip.y + o.y) - camY);
        }
    }
}
...
void Game::drawImpacts() {
    ...
    for (const auto& fx : impacts) {

        if (fx.ttl <= 0.0f) continue;

        float life = std::max(0.001f, fx.ttl + fx.age);
        float t = std::clamp(fx.age / life, 0.0f, 1.0f);

        // expand a bit over time
        float s = fx.size * (1.0f + 0.65f * t);

        // fade out
        Uint8 a = (Uint8)std::round(255.0f * (1.0f - t));
        a = (Uint8)std::min<int>(a, 220);

        Vec2 d = normalize(fx.dir);
        Vec2 r = perpRight(d);

        if (fx.flesh) {
            // blood puff: 2–4 tiny circles + a short streak
            SDL_SetRenderDrawColor(renderer, 150, 25, 25, a);

            // streak
            Vec2 p0 = fx.pos - d * (s * 0.15f);
            Vec2 p1 = fx.pos + d * (s * 0.55f);
            SDL_RenderDrawLineF(renderer, p0.x - camX, p0.y - camY, p1.x - camX, p1.y - camY);

            // little “blot” circles approximated by small rects
            int blots = 3 + irand(0, 1);
            for (int i = 0; i < blots; ++i) {
                float ox = frand(-s * 0.35f, s * 0.35f);
                float oy = frand(-s * 0.35f, s * 0.35f);
                float w = frand(1.5f, 2.5f);
                SDL_FRect rr{ (fx.pos.x + ox - w) - camX, (fx.pos.y + oy - w) - camY, w * 2, w * 2 };
                SDL_RenderFillRectF(renderer, &rr);
            }
        }
        else {
            // dust/spark puff: short rays + a small dot
            SDL_SetRenderDrawColor(renderer, 210, 200, 170, a);

            // center dot
            SDL_FRect dot{ (fx.pos.x - 1.5f) - camX, (fx.pos.y - 1.5f) - camY, 3.0f, 3.0f };
            SDL_RenderFillRectF(renderer, &dot);

            // rays (3)
            for (int i = 0; i < 3; ++i) {
                float off = (i - 1) * (s * 0.25f);
                Vec2 o = r * off;
                Vec2 p0 = fx.pos + o;
                Vec2 p1 = fx.pos + o + d * (s * 0.55f);
                SDL_RenderDrawLineF(renderer, p0.x - camX, p0.y - camY, p1.x - camX, p1.y - camY);
            }
        }
    }
...
SDL_Color Game::factionColor(Faction f) const {
    switch (f) {
    case Faction::Axis:    return SDL_Color{ 78,  92,  98, 255 }; // field blue-grey
    case Faction::Militia: return SDL_Color{ 88,  98,  84, 255 }; // green-grey
    case Faction::Allies:  return SDL_Color{ 85, 107,  47, 255 }; // olive drab
    case Faction::Rebels:  return SDL_Color{ 25,  25,  25, 255 }; // near-black base
...
void Game::drawWorld() {
    ...
    for (const auto& leaf : leaves) {
        ...
        for (const auto& e : actors) {
            if (!e.alive()) continue;
            SDL_FRect er = rectFrom(e.pos, e.w + 8.f, e.h + 10.f);
            er.x -= camX;
            er.y -= camY;
        ...
        for (const auto& c : casings) {
            ...
            if (c.ttl <= 0.0f) continue;

            if (c.kind == CasingKind::Brass) SDL_SetRenderDrawColor(renderer, 180, 150, 60, 230);
            else if (c.kind == CasingKind::Shell) SDL_SetRenderDrawColor(renderer, 140, 45, 25, 230);
            else SDL_SetRenderDrawColor(renderer, 140, 140, 140, 235); // clip

            float w = (c.kind == CasingKind::Clip) ? 6.0f : 3.0f;
            float h = (c.kind == CasingKind::Shell) ? 2.2f : 1.6f;
            if (c.kind == CasingKind::Clip) h = 2.8f;

            SDL_FRect r{ c.pos.x - w * 0.5f - camX, c.pos.y - h * 0.5f - camY, w, h };
            SDL_RenderFillRectF(renderer, &r);

            // Optional: tiny “ping sparkle” for the clip
            if (c.ping) {
                SDL_SetRenderDrawColor(renderer, 240, 240, 240, 180);
                SDL_RenderDrawLineF(renderer, (c.pos.x - 3) - camX, (c.pos.y - 3) - camY,
                    (c.pos.x + 3) - camX, (c.pos.y + 3) - camY);
            }
        }
...
static inline void drawOrientedBoxLines(SDL_Renderer* r, Vec2 c, Vec2 f, Vec2 right, float L, float W, float camX, float camY) {
    Vec2 A = c + f * (L * 0.5f) - right * (W * 0.5f);
    Vec2 B = c + f * (L * 0.5f) + right * (W * 0.5f);
    Vec2 C = c - f * (L * 0.5f) + right * (W * 0.5f);
    Vec2 D = c - f * (L * 0.5f) - right * (W * 0.5f);
    SDL_RenderDrawLineF(r, A.x - camX, A.y - camY, B.x - camX, B.y - camY);
    SDL_RenderDrawLineF(r, B.x - camX, B.y - camY, C.x - camX, C.y - camY);
    SDL_RenderDrawLineF(r, C.x - camX, C.y - camY, D.x - camX, D.y - camY);
    SDL_RenderDrawLineF(r, D.x - camX, D.y - camY, A.x - camX, A.y - camY);
}

// --- Filled convex quad (used to fill rotated rectangles) ---
static inline void fillTri(SDL_Renderer* r, Vec2 p0, Vec2 p1, Vec2 p2) {
    // Sort by y
    ...
    if (std::fabs(y2 - y0) < 0.01f) return;

    auto interpX = [](Vec2 a, Vec2 b, float y) -> float {
        if (std::fabs(b.y - a.y) < 0.0001f) return a.x;
        float t = (y - a.y) / (b.y - a.y);
        return a.x + (b.x - a.x) * t;
        };

    int iy0 = (int)std::floor(y0);
    int iy2 = (int)std::ceil(y2);
    ...
    for (int iy = iy0; iy <= iy2; ++iy) {
        ...
        bool lower = (y >= y1);
        float xl = lower ? interpX(p1, p2, y) : interpX(p0, p1, y);
        float xr = interpX(p0, p2, y);

        if (xl > xr) std::swap(xl, xr);
        SDL_RenderDrawLineF(r, xl, y, xr, y);
    }
}

static inline void fillQuad(SDL_Renderer* r, Vec2 A, Vec2 B, Vec2 C, Vec2 D) {
    fillTri(r, A, B, C);
    fillTri(r, A, C, D);
}

static inline void fillOrientedBox(SDL_Renderer* r, Vec2 c, Vec2 f, Vec2 right,
    float L, float W, float camX, float camY) {
    Vec2 A = c + f * (L * 0.5f) - right * (W * 0.5f);
    Vec2 B = c + f * (L * 0.5f) + right * (W * 0.5f);
    Vec2 C = c - f * (L * 0.5f) + right * (W * 0.5f);
    Vec2 D = c - f * (L * 0.5f) - right * (W * 0.5f);
...
// -----------------------------------------------------------
// Minimal FABRIK 2D chain (used for procedural arms/legs)
// -----------------------------------------------------------
struct Chain2D {
    std::vector<Vec2> p;   // joints (N+1)
    std::vector<float> L;  // segment lengths (N)
};

static inline void chainInit(Chain2D& c, int segments, float segLen, Vec2 root, Vec2 dir) {
    c.p.assign(segments + 1, root);
    c.L.assign(segments, segLen);
    Vec2 d = normalize(dir);
    for (int i = 0; i < segments; ++i) {
        c.p[i + 1] = c.p[i] + d * segLen;
    }
}

static inline void fabrikSolve(Chain2D& c, Vec2 root, Vec2 target, int iters = 6, float eps = 0.25f) {
    float total = 0.f;
    for (float l : c.L) total += l;

    float dist = length(target - root);
    if (dist >= total) {
        Vec2 dir = normalize(target - root);
        c.p[0] = root;
        for (int i = 0; i < (int)c.L.size(); ++i) c.p[i + 1] = c.p[i] + dir * c.L[i];
        return;
    }
    ...
    for (int k = 0; k < iters; ++k) {
        // forward
        c.p.back() = target;
        for (int i = (int)c.p.size() - 2; i >= 0; --i) {
            Vec2 dir = normalize(c.p[i] - c.p[i + 1]);
            c.p[i] = c.p[i + 1] + dir * c.L[i];
        }
        // backward
        c.p[0] = base;
        for (int i = 0; i < (int)c.p.size() - 1; ++i) {
            Vec2 dir = normalize(c.p[i + 1] - c.p[i]);
            c.p[i + 1] = c.p[i] + dir * c.L[i];
        }

        if (length(c.p.back() - target) < eps) break;
    }
}

void Game::drawProcCharacter(const Actor& a, SDL_Color col) {
    float ta = a.panim.torsoAng;
    float wa = a.panim.weaponAng;

    auto tint = [](SDL_Color c, float k) -> SDL_Color {
        return SDL_Color{
            (Uint8)std::clamp((int)std::round(c.r * k), 0, 255),
            (Uint8)std::clamp((int)std::round(c.g * k), 0, 255),
            (Uint8)std::clamp((int)std::round(c.b * k), 0, 255),
            255
        };
        };

    bool rebel = (a.team == Faction::Rebels);
    SDL_Color armCol = rebel ? SDL_Color{ 200,200,200,255 } : tint(col, 0.92f);
    SDL_Color headCol = rebel ? SDL_Color{ 220,220,220,255 } : tint(col, 1.10f);
    SDL_Color weapCol = rebel ? SDL_Color{ 210,210,210,255 } : SDL_Color{ 205,205,205,255 };



    Vec2 tf{ std::cos(ta), std::sin(ta) };
    Vec2 tr{ -tf.y, tf.x };

    Vec2 wf{ std::cos(wa), std::sin(wa) };
    Vec2 wr{ -wf.y, wf.x };

    float torsoL = a.w * kBodyLenMul; // shorter (less “tank”)
    float torsoW = a.h;               // unchanged width


    float spN = std::clamp(a.panim.speed / 120.0f, 0.0f, 1.6f);
    float step = std::sin(a.panim.gait);

    // torso
    setDraw(renderer, col);
    fillOrientedBox(renderer, a.pos, tf, tr, torsoL, torsoW, camX, camY);

    // pack behind
    SDL_Color packCol{ (Uint8)(col.r * 0.65f), (Uint8)(col.g * 0.65f), (Uint8)(col.b * 0.65f), 255 };
    setDraw(renderer, packCol);
    Vec2 packC = a.pos - tf * (torsoL * 0.28f);
    fillOrientedBox(renderer, packC, tf, tr, 8.0f, 7.0f, camX, camY);

    // head (small square)
    Vec2 headC = a.pos + tf * (torsoL * 0.10f) + tr * (0.8f * std::sin(a.panim.gait * 2.0f) * spN);
    setDraw(renderer, headCol);
    SDL_FRect hr{ headC.x - 3.0f - camX, headC.y - 3.0f - camY, 6.0f, 6.0f };
    SDL_RenderFillRectF(renderer, &hr);


    // shoulders / anchors
    float shoulderHalf = torsoW * 0.52f;
    Vec2 shC = a.pos + tf * (torsoL * 0.05f) + tf * (1.2f * step * spN);
    Vec2 shL = shC - tr * shoulderHalf;
    Vec2 shR = shC + tr * shoulderHalf;

    // weapon base + muzzle (MATCH weaponMuzzlePos() so visuals == ballistics)
    Vec2 weapBase = a.pos + wf * (a.w * 0.26f) + wr * (a.h * 0.10f);

    // ---- Weapon-derived grips (MATCH drawHeldWeapon sizing) ----
    WeaponId wid = activeW(a).id;
    float len = 14.0f;
    ...
    if (wid != WeaponId::None) {
        ...
        len = 10.0f + std::clamp(wd.maxRange * 0.020f, 0.0f, 18.0f);

        if (wd.magSize <= 10 && wd.maxRange < 260.0f) {
            len *= 0.60f; // pistols shorter
        }
        if (wd.pellets > 1) {
            len += 2.0f;  // shotguns a bit longer feel
        }
    }

    // Aim direction should respect shot-lock so AI doesn't drift
    Vec2 aimF = wf;
    if (a.lastShotT > 0.0f) aimF = normalize(a.lastShotDir);
    Vec2 aimR = perpRight(aimF);

    // MUST match drawHeldWeapon muzzleBase anchor
    Vec2 muzzleBase = a.pos + aimF * (a.w * 0.30f) + aimR * (a.h * 0.10f);

    // Hand targets
    float supportF = std::clamp(0.55f * len, 7.0f, 15.0f);
    float triggerF = std::clamp(0.18f * len, 1.5f, 4.0f);

    Vec2 handR = muzzleBase + aimF * triggerF + aimR * 2.2f;
    Vec2 handL = muzzleBase + aimF * supportF - aimR * 1.6f;



    // --- FABRIK arms helper (draw segments as filled boxes) ---
    auto drawChainBoxes = [&](const Chain2D& c, float boxW, SDL_Color col) {
        setDraw(renderer, col);
        for (int i = 0; i < (int)c.L.size(); ++i) {
            Vec2 A = c.p[i];
            Vec2 B = c.p[i + 1];
            Vec2 d = B - A;
            float L = length(d);
            ...
            fillOrientedBox(renderer, center, f, r, L, boxW, camX, camY);
        }
        };

    // ------------------------------------------------------------
    // Two-chain arms: UpperArm (short) + Forearm (reach)
    // BUT: Right elbow is constrained to sit on the shoulder line.
    // ------------------------------------------------------------
    ...
    // segment counts
    const int upSegs = 2;  // shoulder -> elbow (short)
    const int lowSegs = 6;  // elbow -> hand (reach)

    // lengths (your values)
    const float upLenSeg = 1.5f;
    const float lowLenSeg = 2.6f;

    // draw both parts
    auto drawArm = [&](const Chain2D& up, const Chain2D& low, SDL_Color c) {
        drawChainBoxes(up, 3.0f, c);
        drawChainBoxes(low, 3.0f, c);
        };

    // ---- Shoulder-line basis (for elbow alignment) ----
    // Shoulder line direction from left shoulder to right shoulder
    Vec2 shDir = normalize(shR - shL);
    if (length(shDir) < 0.001f) shDir = aimR; // fallback

    // ---- Choose elbow targets ----
    // LEFT: keep your existing “near shoulder + slight tuck” behavior (works well)
    float spN_local = std::clamp(a.panim.speed / 120.0f, 0.0f, 1.6f);

    float upperReachL = 3.8f;    // forward from shoulder (small = close)
    float flareOutL = 2.0f;    // tuck inward
    flareOutL *= (1.0f - 0.25f * spN_local);

    Vec2 elbowTargetL = shL + aimF * upperReachL - aimR * flareOutL;

    // RIGHT: force elbow to sit ON the shoulder line (your request)
    // This makes the upper arm look short/occluded and prevents the sharp kink near grip.
    float elbowInsetR = 3.2f;  // how far inward along shoulder line (try 2.6..4.6)
    float elbowBackR = 0.9f;  // small -aimF pull to reduce 90° kink (try 0.0..1.8)

    Vec2 elbowTargetR = shR - shDir * elbowInsetR - aimF * elbowBackR;

    // ---- Solve upper arms to elbow targets ----
    chainInit(upR, upSegs, upLenSeg, shR, aimF);
    chainInit(upL, upSegs, upLenSeg, shL, aimF);

    fabrikSolve(upR, shR, elbowTargetR, 6, 0.18f);
    fabrikSolve(upL, shL, elbowTargetL, 6, 0.20f);

    // Actual elbow joints
    Vec2 elbowR = upR.p.back();
    Vec2 elbowL = upL.p.back();

    // Optional: make RIGHT upper arm even more “in line” with shoulders by snapping elbow back onto shoulder line.
    // This is the hard guarantee.
    {
        // Project elbowR onto the shoulder line through shR (direction shDir)
        // elbowR = shR + shDir * t
        float t = (elbowR.x - shR.x) * shDir.x + (elbowR.y - shR.y) * shDir.y;
        elbowR = shR + shDir * t;

        // Write back to the chain end joint so drawing + forearm root matches
        upR.p.back() = elbowR;
    }

    // ---- Forearms: elbow -> hand ----
    // Give forearms a gentle initial bend so they don’t look ruler-straight.
    Vec2 foreInitR = normalize(aimF + aimR * 0.35f);
    Vec2 foreInitL = normalize(aimF - aimR * 0.25f);

    chainInit(lowR, lowSegs, lowLenSeg, elbowR, foreInitR);
    chainInit(lowL, lowSegs, lowLenSeg, elbowL, foreInitL);

    fabrikSolve(lowR, elbowR, handR, 8, 0.25f);
    fabrikSolve(lowL, elbowL, handL, 8, 0.25f);

    // Draw arms
    drawArm(upR, lowR, armCol);
    drawArm(upL, lowL, armCol);



    // legs only when moving
    if (a.panim.speed > 6.0f) {
        Vec2 hipC = a.pos - tf * (torsoL * 0.18f);
        Vec2 hipL = hipC - tr * (torsoW * 0.25f);
        Vec2 hipR = hipC + tr * (torsoW * 0.25f);

        float stride = (3.0f + 2.6f * spN);
        float sL = std::sin(a.panim.gait);
        float sR = std::sin(a.panim.gait + 3.14159265f);

        Vec2 footL = hipL - tf * (torsoL * 0.22f + stride * std::max(0.0f, sL));
        Vec2 footR = hipR - tf * (torsoL * 0.22f + stride * std::max(0.0f, sR));

        setDraw(renderer, SDL_Color{ 185,185,185,255 });
        SDL_RenderDrawLineF(renderer, hipL.x - camX, hipL.y - camY, footL.x - camX, footL.y - camY);
        SDL_RenderDrawLineF(renderer, hipR.x - camX, hipR.y - camY, footR.x - camX, footR.y - camY);
    }
}
...
void Game::drawActors() {
    ...
    for (const auto& d : lootDrops) {
        if (!d.taken) {
            // Primary weapon on ground
            if (d.hasP) drawWeaponSilhouetteAt(d.pos, d.pInst.id, Vec2{ 1,0 }, 0.95f);

            // Secondary slightly offset (optional)
            if (d.hasS) drawWeaponSilhouetteAt(d.pos + Vec2{ 0, 6 }, d.sInst.id, Vec2{ 1,0 }, 0.85f);
        }
    ...
    // player
    if (playerPresent) {
        drawProcCharacter(player, factionColor(player.team));
        drawHeldWeapon(player); // optional: keep for now (or remove later)
        // Keep pr for UI placement (labels, etc) even though we no longer draw the body rect.
        //SDL_FRect pr = rectFrom(player.pos, player.w, player.h);
        //pr.x -= camX;
        //pr.y -= camY;
    ...
    for (int idx = 0; idx < (int)actors.size(); ++idx) {
        ...
        drawProcCharacter(e, factionColor(e.team));
        drawHeldWeapon(e); // optional

        // Keep er for UI placement/highlights (HVT box, selection box, labels)
        ...
        if (lootMode && playerPresent && player.alive()) {
            // Refresh lootIdx each frame so it follows the nearest drop (optional)
            int li = findNearestLoot(lootDrops, player.pos, lootRadius);
            if (li >= 0) lootIdx = li;

            if (lootIdx >= 0 && lootIdx < (int)lootDrops.size()) {
                LootDrop& d = lootDrops[lootIdx];


                // --- Loot UI layout (safe on-screen)
                float cx = cfg::ScreenW * 0.5f;

                // Put the UI higher so it never clips the bottom.
                // (0.52–0.56 feels good; 0.65 is too low once you add 2 rows of boxes.)
                float cy = cfg::ScreenH * 0.54f;

                // Slightly smaller boxes so 2 rows always fit.
                const float boxW = 54.0f;
                const float boxH = 18.0f;
                const float gapX = 18.0f;
                const float gapY = 14.0f;

                // 2 columns (player on left, container/corpse on right)
                // 2 rows (primary row, secondary row)
                SDL_FRect pP{ cx - (boxW + gapX + boxW), cy - (boxH + gapY), boxW, boxH };
                SDL_FRect pS{ cx - (boxW + gapX + boxW), cy,               boxW, boxH };

                SDL_FRect cP{ cx + (gapX),              cy - (boxH + gapY), boxW, boxH };
                SDL_FRect cS{ cx + (gapX),              cy,               boxW, boxH };

                auto centerOf = [](const SDL_FRect& r) {
                    return Vec2{ r.x + r.w * 0.5f, r.y + r.h * 0.5f };
                    };

                // These positions are screen-space; drawWeaponSilhouetteAt expects world-space.
                // So we can either:
                //  A) write a screen-space version, OR
                //  B) temporarily “fake” cam by adding camX/camY back.
                // Easiest: B:
                auto screenToWorld = [&](const Vec2& s) {
                    return Vec2{ s.x + camX, s.y + camY };
                    };

                drawWeaponSilhouetteAt(screenToWorld(centerOf(pP)), player.weapon.id, Vec2{ 1,0 }, 0.55f);
                drawWeaponSilhouetteAt(screenToWorld(centerOf(pS)), player.hasWeapon2 ? player.weapon2.id : WeaponId::None, Vec2{ 1,0 }, 0.55f);
                drawWeaponSilhouetteAt(screenToWorld(centerOf(cP)), d.hasP ? d.pInst.id : WeaponId::None, Vec2{ 1,0 }, 0.55f);
                drawWeaponSilhouetteAt(screenToWorld(centerOf(cS)), d.hasS ? d.sInst.id : WeaponId::None, Vec2{ 1,0 }, 0.55f);


                SDL_RenderDrawRectF(renderer, &pP);
                SDL_RenderDrawRectF(renderer, &pS);
                SDL_RenderDrawRectF(renderer, &cP);
                SDL_RenderDrawRectF(renderer, &cS);



                // --- 11B UI: labels + ammo lines
                {
                    // Player primary
                    drawText(std::string("1 P: ") + weaponName(player.weapon.id),
                        (int)pP.x + 6, (int)pP.y + 2, cfg::ColUIAlt);
                    {
                        char ab[64];
                        std::snprintf(ab, sizeof(ab), "M%d R%d", player.weapon.magAmmo, player.weapon.reserveAmmo);
                        drawText(ab, (int)pP.x + 6, (int)pP.y + 18, cfg::ColUI);
                    }

                    // Player secondary
                    if (player.hasWeapon2 && player.weapon2.id != WeaponId::None) {
                        drawText(std::string("2 S: ") + weaponName(player.weapon2.id),
                            (int)pS.x + 6, (int)pS.y + 2, cfg::ColUIAlt);
                        char ab[64];
                        std::snprintf(ab, sizeof(ab), "M%d R%d", player.weapon2.magAmmo, player.weapon2.reserveAmmo);
                        drawText(ab, (int)pS.x + 6, (int)pS.y + 18, cfg::ColUI);
                    }
                    else {
                        drawText("2 S: NONE", (int)pS.x + 6, (int)pS.y + 10, cfg::ColUIAlt);
                    }

                    // Corpse primary
                    if (d.hasP && d.pInst.id != WeaponId::None) {
                        drawText(std::string("1 P: ") + weaponName(d.pInst.id),
                            (int)cP.x + 6, (int)cP.y + 2, cfg::ColUIAlt);
                        char ab[64];
                        std::snprintf(ab, sizeof(ab), "M%d R%d", d.pInst.magAmmo, d.pInst.reserveAmmo);
                        drawText(ab, (int)cP.x + 6, (int)cP.y + 18, cfg::ColUI);
                    }
                    else {
                        drawText("1 P: NONE", (int)cP.x + 6, (int)cP.y + 10, cfg::ColUIAlt);
                    }

                    // Corpse secondary
                    if (d.hasS && d.sInst.id != WeaponId::None) {
                        drawText(std::string("2 S: ") + weaponName(d.sInst.id),
                            (int)cS.x + 6, (int)cS.y + 2, cfg::ColUIAlt);
                        char ab[64];
                        std::snprintf(ab, sizeof(ab), "M%d R%d", d.sInst.magAmmo, d.sInst.reserveAmmo);
                        drawText(ab, (int)cS.x + 6, (int)cS.y + 18, cfg::ColUI);
                    }
                    else {
                        drawText("2 S: NONE", (int)cS.x + 6, (int)cS.y + 10, cfg::ColUIAlt);
                    }

                    // Optional: show loose ammo remaining
                    {
                        char ab[64];
                        std::snprintf(ab, sizeof(ab), "Loose %d", d.ammoLoose);
                        drawText(ab, (int)(cx - 30), (int)(cy + 54), cfg::ColUI);
                    }
                }



                // Text lines (use your existing drawText call)
                // Left: player
                //drawText(("1: PRIMARY " + weaponName(player.weapon.id)).c_str(), (int)left.x, (int)left.y - 18);

                // Right: loot
                // drawText(("LOOT " + weaponName(d.inst.id)).c_str(), (int)right.x, (int)right.y - 18);

                // Actions
                // drawText("Hold E: Loot  |  1 Swap Primary  |  F Take Ammo  |  Esc Close", 10, screenH - 40);
            }
        }
...
void Game::drawCrosshair() {
    if (!playerPresent || !player.alive()) return;
    if (mapOverlay) return; // optional: let map overlay hide it
    ...
    if (binoHeld) {
        if (hasBinoFootprint) {
            ...
            SDL_Rect r{
                (int)binoFootprintScreen.x,
                (int)binoFootprintScreen.y,
                (int)binoFootprintScreen.w,
                (int)binoFootprintScreen.h
            };
    ...
    // compute aim distance in world
    Vec2 aim{ (float)worldMouseX, (float)worldMouseY };
    float d = length(aim - player.pos);

    const WeaponInstance& w = activeW(player);
    const WeaponDef& wd = weaponDef(w.id);
    float spreadRad = (wd.spreadDeg * 3.14159265f / 180.0f);

    // expected lateral deviation ~ tan(spread) * distance
    float devWorld = std::tan(spreadRad) * d;

    // convert to screen pixels
    float devPx = devWorld * zoom;

    // clamp for sanity
    devPx = std::clamp(devPx, 2.0f, 80.0f);

    SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_BLEND);
    SDL_SetRenderDrawColor(renderer, 255, 255, 255, 200);

    // simple "spread box" crosshair
    SDL_RenderDrawLine(renderer, mx - 10, my, mx + 10, my);
    SDL_RenderDrawLine(renderer, mx, my - 10, mx, my + 10);

    SDL_Rect box{ (int)(mx - devPx), (int)(my - devPx), (int)(devPx * 2), (int)(devPx * 2) };
...
void Game::drawHUD() {
    ...
    char zb[64];
    std::snprintf(zb, sizeof(zb), "Zoom: %.2fx | F1 labels  F2 barks  F3 hearing  F4 vision", zoom);
...
// --- PATCH 9E: bottom HUD text while ADS ---
void Game::drawADSWeaponHUD() {
    if (!adsHeld || !playerPresent || !player.alive() || mapOverlay) return;

    // Screen-space draw (render() already resets viewport+scale before HUD)
    const WeaponInstance& w = activeW(player);
    std::string wname = weaponName(w.id);
    char buf[256];
    std::snprintf(buf, sizeof(buf), "%s  |  Mag %d  Reserve %d",
        wname.c_str(),
        w.magAmmo,
        w.reserveAmmo
    );
    ...
    int x = 12;
    int y = cfg::ScreenH - 22;
    drawText(buf, x, y, cfg::ColUIAlt);
}

// --- PATCH 9F: hover reveal while ADS or Binocs ---
// Reveals faction / HVT / weapon when aiming (ADS) or using binoculars.
void Game::drawHoverReveal() {
    if (!playerPresent || !player.alive()) return;
    if (!adsHeld) return; // hover reveal only for ADS; binocs reveal handled in PiP
    if (mapOverlay) return;

    // World point under cursor (your existing worldMouseX/Y are already correct)
    Vec2 aimW{ (float)worldMouseX, (float)worldMouseY };

    // Choose reveal reach:
    // - ADS: a bit shorter (still useful)
    // - Binocs: longer (identification at distance)
    float maxDist = binoHeld ? 900.0f : 520.0f;

    // Cursor proximity threshold in WORLD units (how close cursor must be to target)
    float cursorSnap = binoHeld ? 45.0f : 20.0f;
    ...
    for (int i = 0; i < (int)actors.size(); ++i) {
        ...
        // Must be close to cursor (so it feels like “hover”)
        float dc = length(a.pos - aimW);
        if (dc > cursorSnap) continue;

        // Also limit by distance from player (so you don't ID the whole map)
        float dp = length(a.pos - player.pos);
        if (dp < bestD) {
            bestD = dp;
            bestIdx = i;
        }
    ...
    const Actor& t = actors[bestIdx];

    // Build label
    std::string lab;
    lab += factionShortName(t.team);
    if (t.isHVT) lab += " [HVT]";
    lab += " ";
    lab += weaponName(t.weapon.id);

    // Draw near crosshair (screen space)
    SDL_FPoint s = worldToScreenPt(t.pos, camX, camY, zoom);

    // Small offset so it doesn’t sit directly on top of the crosshair
    int tx = (int)s.x + 10;
    int ty = (int)s.y - 18;

    // Clamp to screen a bit
    tx = std::clamp(tx, 6, cfg::ScreenW - 220);
    ty = std::clamp(ty, 6, cfg::ScreenH - 20);

    drawText(lab, tx, ty, cfg::ColUIAlt);
}
...
bool Game::sees(const Actor& a, const Vec2& targetPos, bool& outLOS) const {
    ...
    // Base effective vision range (your alarm scaling)
    float effRange = a.visionRange * (1.0f + 0.12f * (float)mission.alarmLevel);

    // --- Foliage concealment (Tree tile acts as “brush”) ---
    // If the TARGET is standing in a Tree tile, reduce detection range and narrow FOV slightly.
    // This makes “staying hidden in foliage” actually work again.
    int tc = int(targetPos.x / cfg::TileSize);
    int tr = int(targetPos.y / cfg::TileSize);
    bool targetInTrees = false;
    if (inBoundsTile(tc, tr)) {
        Tile t = map.at(tc, tr);
        targetInTrees = (t == Tile::Tree);
    }

    // Close-range always breaks concealment (you can’t hide if they’re right on top of you)
    const float closeReveal = cfg::TileSize * 1.2f;

    float fovMul = 1.0f;
    if (targetInTrees && dist > closeReveal) {
        // Alarm makes everyone a bit more keyed-up (less conceal effect when alarm is high)
        float concealMul = (mission.alarmLevel >= 3) ? 0.75f : (mission.alarmLevel >= 1 ? 0.65f : 0.55f);
        effRange *= concealMul;
        fovMul = 0.85f;
    }
    ...
    if (angDeg > (a.visionFOVDeg * 0.5f) * fovMul) { outLOS = false; return false; }
...
void Game::updateSquadBrain(int sid, float dt) {
    ...
    // When lightly suppressed and confident, squads are more willing to reposition/flank.
        ...
        const char* txt = planBark(s.side, s.intent);
        pushBark(barkPos, txt, 2.0f);
        Audio_PostBark(barkPos.x, barkPos.y, (int)s.side, txt);
        ...
        if (s.intent == SquadIntent::Flank || s.intent == SquadIntent::Advance) {
            ...
            if (frand(0.f, 1.f) < 0.03f) {
                for (int idx : s.members) {
                    ...
                    Audio_PostBark(a.pos.x, a.pos.y, (int)a.team, "What do we do!?");
    ...
    if (!s.intentExecuting && s.coordTimer <= 0.0f) {
        ...
        Audio_PostBark(barkPos.x, barkPos.y, (int)s.side, "Go, go!");
            ...
            if (s.mode == SquadMode::Calm) {
                ...
                if (barksEnabled && s.calmBarkTimerS <= 0.0f) {
                    ...
                    if (frand(0.f, 1.f) < 0.35f) {
                        const char* txt = calmBark(s.side);
                        pushBark(barkPos, txt, 2.0f);
                        Audio_PostBark(barkPos.x, barkPos.y, (int)s.side, txt);
...
void Game::updateAI(Actor& a, float dt) {
    ...
    // Shot-lock timer (weapon visuals align with bullet direction briefly)
    if (a.lastShotT > 0.0f) a.lastShotT = std::max(0.0f, a.lastShotT - dt);
    ...
    if (a.burstCooldownS > 0.0f) a.burstCooldownS = std::max(0.0f, a.burstCooldownS - dt);
    if (a.panicS > 0.0f) a.panicS = std::max(0.0f, a.panicS - dt);
    ...
    // -------------------------------------------------
// AI LOOTING (only when calm / not engaged)
// -------------------------------------------------
    if (!hasThreat && !a.recentlyHit) {
        // Only occasionally, and mostly while patrolling/idle
        bool calmState = (a.state == AIState::Patrol || a.state == AIState::Idle);
        if (calmState && frand(0.f, 1.f) < 0.10f * dt) {
            const float aiLootRadius = 34.0f;

            int li = findNearestLoot(lootDrops, a.pos, aiLootRadius);
            if (li >= 0 && li < (int)lootDrops.size()) {
                LootDrop& d = lootDrops[li];
                if (!d.taken) {


                    // 1) If same weapon: take ammo into reserve up to cap
                    if (d.pInst.id == a.weapon.id) {
                        int cap = ammoCapForWeapon(a.weapon.id);
                        int room = std::max(0, cap - a.weapon.reserveAmmo);

                        int takeFromReserve = std::min(room, d.pInst.reserveAmmo);
                        a.weapon.reserveAmmo += takeFromReserve;
                        d.pInst.reserveAmmo -= takeFromReserve;
                        room -= takeFromReserve;

                        // Optionally also strip mag if still room
                        if (room > 0) {
                            int takeFromMag = std::min(room, d.pInst.magAmmo);
                            a.weapon.reserveAmmo += takeFromMag;
                            d.pInst.magAmmo -= takeFromMag;
                        }




                        // If fully emptied and no weapon to keep, mark taken
                        if (d.pInst.magAmmo <= 0 && d.pInst.reserveAmmo <= 0) {
                            d.taken = true;
                        }
                    }
                    else {
                        // 2) Consider upgrading weapon if loot is better
                        float curS = weaponAIScore(a.weapon.id);
                        float newS = weaponAIScore(d.sInst.id);

                        bool lootHasAmmo = (d.sInst.magAmmo + d.sInst.reserveAmmo) > 0;
                        bool upgrade = lootHasAmmo && (newS > curS * 1.18f); // require clear improvement

                        if (upgrade) {
                            // Swap weapons (loot becomes old weapon)
                            std::swap(a.weapon, d.sInst);


                            // Small delay so it doesn’t chain-swap instantly
                            a.nextThink = std::max(a.nextThink, 0.8f);
                        }
                    }
                }
            }
    ...
    if (hasThreat && seesThreat && barksEnabled && a.barkCooldown <= 0.f) {
        ...
        if (frand(0.f, 1.f) < 0.20f) { // occasional, not spam
            ...
            Audio_PostBark(a.pos.x, a.pos.y, (int)a.team, line.c_str());
        ...
        !hasThreat)                          // only when not busy with enemies
    {
        ...
        if (dist < a.visionRange * 0.8f && losClear(a.pos, player.pos))
        {
            ...
            if (!playerKnownToAllies)
            {
                ...
                Audio_UpdateFootstepsFor(a, desired, a.moveWalkSpeed, dt);
                ...
                if (a.barkCooldown <= 0.f && alliesCuriosityTimer < 2.0f)
                {
                    ...
                    Audio_PostBark(a.pos.x, a.pos.y, (int)a.team, "Who is that?");
                ...
                if (alliesCuriosityTimer > 5.0f && !playerKnownToAllies)
                {
                    ...
                    if (a.barkCooldown <= 0.f)
                    {
                        ...
                        Audio_PostBark(a.pos.x, a.pos.y, (int)a.team, "Must be with another unit.");
        ...
        if (sees(a, player.pos, los) && los) {
            ...
            if (barksEnabled && a.barkCooldown <= 0.f) {
                if (haveEnemy) {
                    ...
                    Audio_PostBark(a.pos.x, a.pos.y, (int)a.team, "Stay low. Roads aren't safe.");
    ...
    if (a.nextThink <= 0.f) {
        ...
        // --- Panic rules ---
        // panicS is already decaying above. recentlyHit bumps it in Attack, but we also use it here.
        bool lowHP = (a.hpMax > 0) ? (a.hp <= int(a.hpMax * 0.40f)) : (a.hp <= 25);
        bool skittishFaction = (a.team == Faction::Militia);

        // If we’re panicking and either low HP or skittish, prioritize survival behaviour.
        if (a.panicS > 0.0f && (lowHP || skittishFaction)) {
            // If we know a threat position (heard/seen), run away from it.
            Vec2 awayFrom = hasThreat ? threatPos : a.lastShotOrigin;
            if (length(awayFrom - a.pos) < 1.0f) awayFrom = a.pos - a.facing * 10.0f;

            Vec2 fleeDir = normalize(a.pos - awayFrom);
            if (length(fleeDir) < 0.001f) fleeDir = normalize(Vec2(frand(-1.f, 1.f), frand(-1.f, 1.f)));

            Vec2 fleeDest = a.pos + fleeDir * 260.0f;

            // Clamp to map bounds-ish
            fleeDest.x = std::clamp(fleeDest.x, 1.0f * cfg::TileSize, (map.cols - 2.0f) * cfg::TileSize);
            fleeDest.y = std::clamp(fleeDest.y, 1.0f * cfg::TileSize, (map.rows - 2.0f) * cfg::TileSize);

            a.state = AIState::Flee;
            a.hasOrder = true;
            a.orderPos = fleeDest;
            clearPath(a);
            buildPath(a.pos, a.orderPos, a);
            a.repathTimer = 0.6f;
            return; // commit to panic-flee this think tick
        }
    ...
    case AIState::Attack: {
        if (hasThreat && seesThreat) {
            ...
            if (weaponCanFire(a)) {
                WeaponId wid = activeW(a).id;
                if (wid == WeaponId::None) break;
                const WeaponDef& wd = weaponDef(wid);

                // Barrel direction (perfect aim direction)
                Vec2 to = threatPos - a.pos;
                Vec2 dir0 = normalize(to);

                // Ensure the gun/barrel is visually aligned with the shot
                // (Do this here so it's correct even at very close range)
                a.facing = dir0;

                // Decide if this behaves like an automatic weapon
                bool isAuto = (wd.fireCooldownS > 0.0f && wd.fireCooldownS < 0.14f);

                // Trigger panic bursts when recently hit (simple but effective)
                if (a.recentlyHit) a.panicS = std::max(a.panicS, 0.9f);

                // Shared spread calc
                auto computeSpreadRad = [&]() -> float {
                    float spreadRad = (wd.spreadDeg * 3.14159265f / 180.0f);
                    if (a.armWoundS > 0.0f) spreadRad *= 1.6f;
                    return spreadRad;
                    };

                // Fire exactly ONE trigger event (1 pellet loop for shotgun etc.)
                auto fireOneShot = [&]() {
                    float spreadRad = computeSpreadRad();

                    // LOCK visual weapon direction to this shot for a moment (prevents rare drift)
                    a.lastShotDir = dir0;     // <-- PASTE THESE TWO LINES
                    a.lastShotT = 0.12f;    // <-- right here

                    // Muzzle origin (critical for alignment)
                    Vec2 muzzle = weaponMuzzlePos(a, wd);

                    // Muzzle flash uses the barrel direction (NO spread)
                    pushMuzzleFlashRing(muzzleFlashes, muzzleWrite, muzzle, dir0, wd);


                    // 🔊 v8 audio
                    Audio_PostFire((int)wid, muzzle.x, muzzle.y, (int)a.team, weaponNoiseRadiusPx(a.weapon.id));

                    //casings AI
                    EjectType et = weaponEjectType(a.weapon.id, wd);

                    // Eject per shot (semi/auto + pump shotguns)
                    if (et == EjectType::SemiAuto) {
                        pushCasingRing(casings, casingWrite, weaponEjectPos(a, wd), dir0, CasingKind::Brass, false);
                    }
                    else if (et == EjectType::ShotgunPump) {
                        pushCasingRing(casings, casingWrite, weaponEjectPos(a, wd), dir0, CasingKind::Shell, false);
                    }
                    else if (et == EjectType::BoltAction) {
                        // delayed: bolt lift/throw
                        a.pendingEjectCount = 1;
                        a.pendingEjectKind = CasingKind::Brass;
                        a.pendingEjectPing = false;
                        a.pendingEjectS = 0.16f; // tune 0.12–0.22
                    }
                    ...
                    for (int p = 0; p < std::max(1, wd.pellets); ++p) {
                        ...
                        Vec2 dirShot{ std::cos(ang), std::sin(ang) };

                        Bullet b;
                        b.pos = muzzle;                 // ✅ comes from gun
                        b.dir = dirShot;                // ✅ spread only affects bullet
                        b.traveled = 0.f;
                    ...
                    // Spend ammo + start cooldown
                    int beforeMag = a.weapon.magAmmo;
                    a.weapon.magAmmo--;

                    if (et == EjectType::GarandClip && beforeMag > 0 && a.weapon.magAmmo == 0) {
                        // eject clip + "ping" bark
                        pushCasingRing(casings, casingWrite, weaponEjectPos(a, wd), dir0, CasingKind::Clip, true);
                        barks.push_back({ a.pos, "PING", 0.45f });
                    }

                    a.weapon.fireTimer = wd.fireCooldownS;
                    a.gun.inMag = a.weapon.magAmmo;

                    // World response / noise can stay at actor pos (or muzzle; either is fine)
                    sounds.push_back({ a.pos, weaponNoiseRadiusPx(wid), cfg::HearDecayS });
                    raiseAlarm(1);
                    };

                // --- BURST LOGIC ---
                if (isAuto) {
                    if (a.burstCooldownS > 0.0f) {
                        // cooling down between bursts: do nothing
                    }
                    else {
                        if (a.burstShotsLeft <= 0) {
                            a.burstShotsLeft = (a.panicS > 0.0f) ? irand(8, 14) : irand(2, 4);
                        }
                        ...
                        a.burstShotsLeft--;
                        if (a.burstShotsLeft <= 0) {
                            a.burstCooldownS = (a.panicS > 0.0f)
                                ? frand(0.10f, 0.25f)
                                : frand(0.25f, 0.55f);
                        }
                    }
                    ...
                    // Non-auto weapons: fire normally (single shot / shotgun)
                    fireOneShot();
                }
            }
    ...
    Audio_UpdateFootstepsFor(a, desiredVel, a.moveWalkSpeed, dt);
    ...
    if (a.weapon.magAmmo <= 0 && !a.weapon.reloading) {
        weaponStartReload(a);
        Audio_PostReload((int)a.weapon.id, a.pos.x, a.pos.y, (int)a.team);
    }
...
void Game::handleEvents() {
    ...
    while (SDL_PollEvent(&e)) {
        ...
        case SDL_MOUSEBUTTONUP: {
            if (e.button.button == SDL_BUTTON_LEFT) {
                ...
                mouseDownR = false;
                              ...
                              //
                              //        zoomNormal *= factor;
                              //        zoomNormal = std::clamp(zoomNormal, 0.5f, 2.5f);
                              //
                              //        // If not aiming/using binos, apply immediately
                              //        if (!adsHeld && !binoHeld) {
                              //            zoom = zoomNormal;
                              //        }
                              //    } break;
        ...
        case SDL_KEYDOWN: {
            ...
            case SDLK_1: {
                if (!lootMode) { setActiveSlot(player, 0); break; }
                if (lootIdx < 0 || lootIdx >= (int)lootDrops.size()) break;

                LootDrop& d = lootDrops[lootIdx];
                if (d.taken) break;

                // Swap corpse PRIMARY <-> player PRIMARY
                if (!d.hasP || d.pInst.id == WeaponId::None) break;

                WeaponInstance tmp = player.weapon;
                player.weapon = d.pInst;
                d.pInst = tmp;

                // After swapping, keep active slot valid
                setActiveSlot(player, 0);

                if (barksEnabled) barks.push_back({ player.pos, "Primary swapped.", 1.0f });
            } break;

            case SDLK_2: {
                if (!lootMode) { setActiveSlot(player, 1); break; }
                if (lootIdx < 0 || lootIdx >= (int)lootDrops.size()) break;

                LootDrop& d = lootDrops[lootIdx];
                if (d.taken) break;

                // Swap corpse SECONDARY <-> player SECONDARY
                if (!d.hasS || d.sInst.id == WeaponId::None) break;

                // Ensure player has a secondary slot storage
                if (!player.hasWeapon2) {
                ...
                WeaponInstance tmp = player.weapon2;
                player.weapon2 = d.sInst;
                d.sInst = tmp;

                setActiveSlot(player, 1);
            ...
            case SDLK_f: {
                if (!lootMode) break;
                // Scoop ammo from the current container without leaving the menu
                if (!playerPresent || !player.alive()) break;
                if (lootIdx < 0 || lootIdx >= (int)lootDrops.size()) break;

                LootDrop& d = lootDrops[lootIdx];
                if (d.taken) break;

                // We only allow ammo pickup if the corpse actually has a weapon matching what you're holding
                // (since we don't have explicit ammo types yet).
                WeaponInstance& pw = activeW(player);
                if (pw.id == WeaponId::None) break;

                bool compatible =
                    (d.hasP && d.pInst.id == pw.id) ||
                    (d.hasS && d.sInst.id == pw.id);

                if (!compatible) {
                    if (barksEnabled) barks.push_back({ player.pos, "No compatible ammo.", 1.0f });
                    break;
                }

                int cap = ammoCapForWeapon(pw.id);
                if (pw.reserveAmmo >= cap) {
                    if (barksEnabled) barks.push_back({ player.pos, "Ammo full.", 1.0f });
                    break;
                }

                if (d.ammoLoose <= 0) {
                    if (barksEnabled) barks.push_back({ player.pos, "No ammo.", 1.0f });
                    break;
                }

                // Take a small chunk (deliberate action)
                int gain = irand(10, 22);
                gain = std::min(gain, d.ammoLoose);
                gain = std::min(gain, cap - pw.reserveAmmo);
                ...
                // Optional: if container is empty of everything, mark taken
                bool emptyWeapons =
                    (!d.hasP || d.pInst.id == WeaponId::None) &&
                    (!d.hasS || d.sInst.id == WeaponId::None);

                if (emptyWeapons && d.ammoLoose <= 0) d.taken = true;

                if (barksEnabled) barks.push_back({ player.pos, "Ammo taken.", 1.0f });
            } break;


            case SDLK_5:
                binoEquipped = !binoEquipped;
                if (barksEnabled && playerPresent) {
                    barks.push_back({ player.pos, binoEquipped ? "Binoculars equipped." : "Binoculars stowed.", 1.2f });
                }
                break;
            ...
            case SDLK_e: {
                ...
                // If we're already in loot mode, ignore (menu handles actions)
                if (lootMode) break;

                // Tap E behavior stays as-is, but it will only trigger if we never cross hold threshold.
                // We'll actually run the tap logic on KEYUP now, so remove the existing tap logic here.
            } break;
            ...
            case SDLK_F9: Audio_PrintMix(); break;
            case SDLK_LEFTBRACKET:  Mix_Bump(MixBus::Footsteps, -0.05f); Audio_PrintMix(); break;
            case SDLK_RIGHTBRACKET: Mix_Bump(MixBus::Footsteps, +0.05f); Audio_PrintMix(); break;
            case SDLK_MINUS:        Mix_Bump(MixBus::Voice, -0.05f);     Audio_PrintMix(); break;
            case SDLK_EQUALS:       Mix_Bump(MixBus::Voice, +0.05f);     Audio_PrintMix(); break;
            case SDLK_COMMA:        Mix_Bump(MixBus::Weapons, -0.05f);   Audio_PrintMix(); break;
            case SDLK_PERIOD:       Mix_Bump(MixBus::Weapons, +0.05f);   Audio_PrintMix(); break;
            case SDLK_SEMICOLON:    Mix_Bump(MixBus::WeaponsDistant, -0.05f); Audio_PrintMix(); break;
            case SDLK_QUOTE:        Mix_Bump(MixBus::WeaponsDistant, +0.05f); Audio_PrintMix(); break;
        ...
        case SDL_KEYUP: {
            ...
            case SDLK_e: {
                ...
                // If loot mode was active, closing the menu happens in update() when kE becomes false.
                if (lootMode) break;

                // Tap E action: take compatible loose ammo only (container rules)
                if (!playerPresent || !player.alive()) break;
                ...
                // Only allow ammo pickup if corpse has a weapon matching what you're holding
                WeaponInstance& pw = activeW(player);
                if (pw.id == WeaponId::None) break;
                ...
                if (!compatible) {
                    if (barksEnabled) barks.push_back({ player.pos, "Shift+E to swap.", 1.0f });
                    break;
                }
                ...
                // If container has nothing left, mark taken
                bool emptyWeapons =
                    (!d.hasP || d.pInst.id == WeaponId::None) &&
                    (!d.hasS || d.sInst.id == WeaponId::None);
                ...
                if (barksEnabled) barks.push_back({ player.pos, "Picked up ammo.", 1.0f });
            } break;
            }
        }
        }
...
static inline float atan2fSafe(const Vec2& v) {
    if (std::fabs(v.x) < 1e-6f && std::fabs(v.y) < 1e-6f) return 0.0f;
    return std::atan2(v.y, v.x);
}

// Call this AFTER you have updated a.pos for the frame
static inline void updateProcAnim(Actor& a, float dt) {
    if (!a.panim.init) {
        a.panim.prevPos = a.pos;
        a.panim.weaponAng = atan2fSafe(a.facing);
        a.panim.torsoAng = a.panim.weaponAng;
        a.panim.gait = 0.0f;
        a.panim.init = true;
        return;
    }

    if (dt > 1e-4f) a.panim.vel = (a.pos - a.panim.prevPos) * (1.0f / dt);
    else           a.panim.vel = Vec2{ 0,0 };

    a.panim.prevPos = a.pos;

    a.panim.speed = length(a.panim.vel);
    if (a.panim.speed > 2.0f) a.panim.moveDir = a.panim.vel * (1.0f / a.panim.speed);

    // Weapon is the "vehicle": follow facing (mouse/AI aim)
    a.panim.weaponAng = atan2fSafe(a.facing);

    // Torso is passenger: follows weapon, but clamp twist when moving
    float targetTorso = a.panim.weaponAng;

    if (a.panim.speed > 6.0f) {
        float moveAng = atan2fSafe(a.panim.moveDir);
        float maxTwist = 1.25f; // ~72 degrees. Tune.
        float d = wrapPi(targetTorso - moveAng);
        d = std::clamp(d, -maxTwist, +maxTwist);
        targetTorso = wrapPi(moveAng + d);
    }

    float follow = 1.0f - std::exp(-dt * 14.0f);
    a.panim.torsoAng = lerpAngle(a.panim.torsoAng, targetTorso, follow);

    // gait only advances when moving
    if (a.panim.speed > 6.0f) {
        float spN = std::clamp(a.panim.speed / 120.0f, 0.0f, 1.7f);
        float freq = 7.0f + 7.0f * spN;
        a.panim.gait = wrapPi(a.panim.gait + dt * freq);
    }
}
...
void Game::update(float dt) {
    ...
    mousePressedL = (mouseDownL && !mouseDownLPrev);
    mouseDownLPrev = mouseDownL;

    // -----------------------------------------------------------
// 8B/8C: ADS + Binoculars hold logic
// -----------------------------------------------------------
    binoHeld = false;
    adsHeld = false;

    if (playerPresent && player.alive() && mode == Mode::Player && !mapOverlay) {
        // RMB behaviour depends on equipped binoculars
        if (mouseDownR && binoEquipped) {
            binoHeld = true;
        }
        else if (mouseDownR && !binoEquipped) {
            adsHeld = true;
        }
    }

    // --- PATCH 9C: ADS transition => arm first-shot bonus ---
    if (adsHeld && !adsHeldPrev) {
        adsFirstShotReady = true;   // first shot after entering ADS gets bonus
    }
    if (!adsHeld) {
        adsFirstShotReady = false;  // leaving ADS clears it
    }
    adsHeldPrev = adsHeld;


    // Zoom target: ADS zooms main camera a bit; binoculars keep main zoom stable
    float zoomTarget = zoomNormal;

    if (binoHeld) {
        // Main view expands while using binoculars
        float zoomWide = 0.85f;              // smaller = more zoomed OUT
        zoomTarget = std::clamp(zoomWide, 0.55f, zoomNormal);
    }
    else if (adsHeld && playerPresent) {
        float zoomMul = 1.18f, pull = 0.22f;
        adsParamsForWeapon(player.weapon.id, zoomMul, pull);
        zoomTarget = std::clamp(zoomNormal * zoomMul, 0.5f, 2.5f);
    }

    // Smooth zoom
    zoom += (zoomTarget - zoom) * std::min(1.0f, dt * 10.0f);
    ...
    // Udptate muzzles each tick
    for (auto& mf : muzzleFlashes) {
        if (mf.ttl <= 0.0f) continue;
        mf.ttl -= dt;
    }
    ...
    // update casings
    auto updatePendingEject = [&](Actor& a, float dt) {
        if (a.pendingEjectCount <= 0) return;
        a.pendingEjectS -= dt;
        if (a.pendingEjectS <= 0.0f) {
            // Emit the queued casings
            for (int i = 0; i < a.pendingEjectCount; ++i) {
                // We need the current weapon def to compute eject pos.
                WeaponId wid = activeW(a).id; // player swaps; AI usually single
                if (wid == WeaponId::None) break;
                const WeaponDef& wd = weaponDef(wid);

                pushCasingRing(casings, casingWrite, weaponEjectPos(a, wd), a.facing, a.pendingEjectKind, a.pendingEjectPing);
            }
            a.pendingEjectCount = 0;
            a.pendingEjectKind = CasingKind::Brass;
            a.pendingEjectPing = false;
            a.pendingEjectS = 0.0f;
        }
        };

    updatePendingEject(player, dt);
    for (auto& a : actors) if (a.alive()) updatePendingEject(a, dt);
    ...
    if (playerPresent && player.alive()) {
        ...
        Audio_SetListener(player.pos.x, player.pos.y, player.facing.x, player.facing.y);
        ...
        if (kE && !lootMode) {
            ...
            // Only enter loot mode if player is alive and near loot
            if (lootHoldS >= lootHoldThreshS && playerPresent && player.alive()) {
                int li = findNearestLoot(lootDrops, player.pos, lootRadius);
                if (li >= 0) {
                    lootMode = true;
                    lootIdx = li;
                    ...
                    // No loot nearby: don't enter menu, reset timer so taps still work
                    lootHoldS = 0.0f;
                }
            }
        }
        ...
        // Exit loot mode when E released
        if (lootMode && !kE) {
            lootMode = false;
            lootIdx = -1;
        }
        ...
        Audio_UpdateFootstepsFor(player, move, cfg::PlayerWalkSpeed, dt);


        updateProcAnim(player, dt);
        ...
        // --- FINAL player facing for this frame (single source of truth) ---
        if (!binoHeld) {
            Vec2 aimDir = aimDirFromMouse(player.pos, worldMouseX, worldMouseY);
...
// Semi-auto = must click again (mousePressedL). Auto = can hold (mouseDownL).
        if (!binoHeld && mode == Mode::Player) {
            WeaponInstance& pw = activeW(player);
            if (pw.id != WeaponId::None) {
                const WeaponDef& wd = weaponDef(pw.id);

                bool isAuto = (wd.fireCooldownS > 0.0f && wd.fireCooldownS < 0.14f);
                bool trigger = isAuto ? mouseDownL : mousePressedL;

                if (trigger && weaponCanFire(player)) {
                    // --- keep the rest of your firing code EXACTLY as-is ---
                    Vec2 dir0 = normalize(player.facing); // single source of truth for aim

                    float spreadRad = (wd.spreadDeg * 3.14159265f / 180.0f);
                    if (player.armWoundS > 0.0f) spreadRad *= 1.6f;

                    if (adsHeld && adsFirstShotReady) spreadRad *= 0.35f;

                    Vec2 muzzle = weaponMuzzlePos(player, wd);
                    pushMuzzleFlashRing(muzzleFlashes, muzzleWrite, muzzle, dir0, wd);


                    // 🔊 v8 audio
                    Audio_PostFire((int)pw.id, muzzle.x, muzzle.y, (int)player.team, weaponNoiseRadiusPx(pw.id));

                    EjectType et = weaponEjectType(pw.id, wd);

                    // Eject per shot (semi/auto + pump shotguns)
                    if (et == EjectType::SemiAuto) {
                        pushCasingRing(casings, casingWrite, weaponEjectPos(player, wd), dir0, CasingKind::Brass, false);
                    }
                    else if (et == EjectType::ShotgunPump) {
                        pushCasingRing(casings, casingWrite, weaponEjectPos(player, wd), dir0, CasingKind::Shell, false);
                    }
                    else if (et == EjectType::BoltAction) {
                        // delayed: bolt lift/throw
                        player.pendingEjectCount = 1;
                        player.pendingEjectKind = CasingKind::Brass;
                        player.pendingEjectPing = false;
                        player.pendingEjectS = 0.16f; // tune 0.12–0.22
                    }



                    const int pelletCount = std::max(1, wd.pellets);
                    ...
                    //for GARAND clip
                    int beforeMag = pw.magAmmo;
                    pw.magAmmo--;

                    if (et == EjectType::GarandClip && beforeMag > 0 && pw.magAmmo == 0) {
                        // eject clip + "ping" bark
                        pushCasingRing(casings, casingWrite, weaponEjectPos(player, wd), dir0, CasingKind::Clip, true);
                        barks.push_back({ player.pos, "PING", 0.45f });
                    }


                    pw.fireTimer = wd.fireCooldownS;

                    player.gun.inMag = pw.magAmmo;
                    player.gun.fireTimer = pw.fireTimer;

                    mission.shotsFired++;
                    sounds.push_back({ player.pos, weaponNoiseRadiusPx(pw.id), cfg::HearDecayS * 0.7f });
                    raiseAlarm(1);

                    if (adsHeld && adsFirstShotReady) adsFirstShotReady = false;
                }
            }
        }
    ...
    if (playerPresent && player.alive()) {
        int li = findNearestLoot(lootDrops, player.pos, 26.0f);
        if (li >= 0) {
            const LootDrop& d = lootDrops[li];

            std::string msg = "E: ";
            if (d.pInst.id == player.weapon.id) {
                msg += "Take ammo";
            }
            else {
                msg += "Swap to ";
                msg += weaponName(d.pInst.id);
            }

            // TODO: replace drawText(...) with your real text function
            // Example if you have something like drawText:
            // drawText(msg.c_str(), 10, screenH - 24, {255,255,255,255});

            // If you don’t have a neat place yet, you can even push this into
            // your existing F1/F2 debug overlay for now as a test.
        }
    }
    ...
    for (int bi = 0; bi < (int)bullets.size(); ++bi)
    {
        ...
        // Lifetime / map collision
        if (b.traveled > b.maxRange || bulletHitsSolid(b)) {
            // Dust impact where the bullet dies (only if it hit solid; range expiry can still puff lightly)
            pushImpactRing(impacts, impactWrite, b.pos, b.dir, false);

            // 🔊 v8 audio
            Audio_PostHitSolid((int)b.wid, b.pos.x, b.pos.y, 1.0f);
        ...
        if (playerPresent && player.alive() && areEnemies(b.src, player.team))
        {
            ...
            if (d < (player.w * 0.5f + 2.f))
            {
                ...
                // Wounds
                if (z == HitZone::Legs)
                    player.legWoundS = std::max(player.legWoundS, 3.0f);
                if (z == HitZone::ArmL || z == HitZone::ArmR)
                    player.armWoundS = std::max(player.armWoundS, 3.0f);
                ...
                //impact hit link bit for player hit
                pushImpactRing(impacts, impactWrite, b.pos, b.dir, true);

                // 🔊 v8 audio
                Audio_PostHitFlesh(b.pos.x, b.pos.y, 1.0f);
        ...
        for (int i = 0; i < (int)actors.size(); ++i)
        {
            ...
            if (d < (a.w * 0.5f + 2.f))
            {
                ...
                // Wounds
                if (z == HitZone::Legs)
                    a.legWoundS = std::max(a.legWoundS, 3.0f);
                if (z == HitZone::ArmL || z == HitZone::ArmR)
                    a.armWoundS = std::max(a.armWoundS, 3.0f);

                // Player callout when *player side* is the shooter
                ...
                // Suppression spike on hit
                if (a.squadId >= 0)
                    addSuppression(a.squadId, 12.0f);

                //impact hit link bit for actor hit
                pushImpactRing(impacts, impactWrite, b.pos, b.dir, true);
                ...
                if (a.hp <= 0)
                {
                    ...
                    // --- Loot container: corpse holds primary + secondary + loose ammo
                    {
                        LootDrop d;
                        d.pos = a.pos;
                        d.srcTeam = (int)a.team;

                        // Primary snapshot
                        d.hasP = (a.weapon.id != WeaponId::None);
                        if (d.hasP) {
                            d.pInst = a.weapon;
                            ensureWeaponIdentity(d.pInst);
                        }

                        // Secondary snapshot
                        d.hasS = (a.hasWeapon2 && a.weapon2.id != WeaponId::None);
                        if (d.hasS) {
                            d.sInst = a.weapon2;
                            ensureWeaponIdentity(d.sInst);
                        }

                        // Loose ammo by faction (you already have this function)
                        d.ammoLoose = ammoRollForTeam((int)a.team);
                ...
                raiseAlarm(1);
                break; // stop checking this bullet vs other actors
            }
        }
    }
    ...
    // Keep world mouse synced to current cam/zoom (critical for ADS/binocs aiming)
    worldMouseX = (int)(camX + (mouseX / zoom));
    worldMouseY = (int)(camY + (mouseY / zoom));
...
void Game::rebuildMapTexture() {
    mapTexDirty = false;

    if (!renderer) return;
    if (map.cols <= 0 || map.rows <= 0) return;

    // (Re)create texture sized to tiles
    if (mapTex) { SDL_DestroyTexture(mapTex); mapTex = nullptr; }
    mapTex = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888,
        SDL_TEXTUREACCESS_STREAMING, map.cols, map.rows);
    if (!mapTex) return;

    SDL_SetTextureBlendMode(mapTex, SDL_BLENDMODE_BLEND);

    void* pixels = nullptr;
    int pitch = 0;
    if (SDL_LockTexture(mapTex, nullptr, &pixels, &pitch) != 0) return;

    auto put = [&](int x, int y, Uint32 rgba) {
    ...
    // Color choices: readable, but still “tactical paper map”
    Uint32 colLand = SDL_MapRGBA(fmt, 70, 80, 60, 255);
    Uint32 colWater = SDL_MapRGBA(fmt, 45, 70, 110, 255);
    Uint32 colTree = SDL_MapRGBA(fmt, 40, 95, 55, 255);
    Uint32 colWall = SDL_MapRGBA(fmt, 120, 120, 120, 255);
    Uint32 colRoad = SDL_MapRGBA(fmt, 90, 80, 70, 255);
    ...
    for (int r = 0; r < map.rows; ++r) {
        for (int c = 0; c < map.cols; ++c) {
            ...
            // Adjust these cases to match your Tile enum names if needed
            switch (t) {
            case Tile::Water: col = colWater; break;
            case Tile::Tree:  col = colTree;  break;
            case Tile::Wall:  col = colWall;  break;
                // If you have roads/floors, map them here:
                // case Tile::Road: col = colRoad; break;
            default:          col = colLand;  break;
            }
...
void Game::drawMapOverlay() {
    ...
    if (mapTexDirty || !mapTex) rebuildMapTexture();

    // Big overlay
    const int ow = cfg::ScreenW;
    const int oh = cfg::ScreenH;
    const int ox = 0;
    const int oy = 0;
    ...
    // Inner map rect (padding)
    SDL_Rect inner{ ox, oy, ow, oh };

    if (mapTex) {

        int tw = 0, th = 0;
        SDL_QueryTexture(mapTex, nullptr, nullptr, &tw, &th);
        char buf[128];
        std::snprintf(buf, sizeof(buf), "mapTex %dx%d  world %dx%d", tw, th, map.cols, map.rows);
        drawText(buf, ox + 14, oy + 34, cfg::ColUIAlt);

        SDL_RenderCopy(renderer, mapTex, nullptr, &inner);
    }

    auto drawMarker = [&](const Vec2& world, SDL_Color col) {
        if (map.cols <= 0 || map.rows <= 0) return;

        float tx = world.x / (float)(map.cols * cfg::TileSize);
        float ty = world.y / (float)(map.rows * cfg::TileSize);

        int px = inner.x + (int)std::round(tx * inner.w);
        int py = inner.y + (int)std::round(ty * inner.h);

        SDL_Rect m{ px - 3, py - 3, 6, 6 };
    ...
    // Markers
    if (playerPresent) {
        drawMarker(player.pos, SDL_Color{ 255, 255, 255, 255 }); // player
    }

    // Objective marker (based on what your mission state exposes)
    // These field names appear in your mission logic in this file.
    if (mission.active) {
        if (mission.kind == MissionKind::Intel && mission.docPresent) {
            drawMarker(mission.docPos, SDL_Color{ 255, 215, 0, 255 });
        }
        else if (mission.kind == MissionKind::HVT && mission.hvtPresent) {
            drawMarker(mission.hvtPos, SDL_Color{ 255, 120, 120, 255 });
        }
        else if (mission.kind == MissionKind::Sabotage && mission.sabotagePresent) {
            drawMarker(mission.sabotagePos, SDL_Color{ 255, 170, 60, 255 });
        }
        else if (mission.kind == MissionKind::Rescue && mission.rescuePresent) {
            drawMarker(mission.rescuePos, SDL_Color{ 160, 220, 255, 255 });
        }

        if (mission.extractPresent) {
            drawMarker(mission.extractPos, SDL_Color{ 140, 255, 140, 255 });
        }
    }

    // Optional: mission start marker (if you set it)
    if (missionStartPos.x != 0.0f || missionStartPos.y != 0.0f) {
        drawMarker(missionStartPos, SDL_Color{ 180, 180, 255, 255 });
    }
    // Small caption plate so text stays readable
    SDL_Rect cap{ ox + 8, oy + 8, 380, 22 };
    SDL_SetRenderDrawColor(renderer, 0, 0, 0, 140);
    SDL_RenderFillRect(renderer, &cap);

    drawText("MAP (M to close) - live world, no pause", ox + 14, oy + 12, cfg::ColUIAlt);
}

void Game::drawBinocularsPip() {
    if (!binoHeld || !playerPresent || !player.alive()) return;

    hasBinoFootprint = false;
    Vec2 aimW = (binoHeld && hasBinoFocus)
        ? binoFocusWorld
        : Vec2{ (float)worldMouseX, (float)worldMouseY };



    // PiP rect (top-right)
    const int pipW = (int)(cfg::ScreenW * 0.38f);
    const int pipH = (int)(cfg::ScreenH * 0.32f);
    SDL_Rect pip{ cfg::ScreenW - pipW - 18, 18, pipW, pipH };

    // Save main camera
    float saveCamX = camX, saveCamY = camY;
    float saveZoom = zoom;

    // Compute binocular look direction (player -> world mouse)
    Vec2 aim{ (float)worldMouseX, (float)worldMouseY };
    Vec2 dir = aim - player.pos;
    float dist = length(dir);
    if (dist < 1.0f) dist = 1.0f;
    dir = dir * (1.0f / dist);

    // Continuous magnification: closer aim => larger zoom
    // (tune these to taste)
    float d01 = std::clamp(dist / 650.0f, 0.0f, 1.0f);
    float pipZoom = 2.6f - 1.3f * d01;          // 2.6 near, 1.3 far
    pipZoom = std::clamp(pipZoom, 1.2f, 3.0f);

    // Focus point a bit ahead of player, scaled by distance
    float focusDist = std::clamp(dist, 120.0f, 720.0f);
    Vec2 focus = player.pos + dir * focusDist;

    hasBinoFocus = true;
    binoFocusWorld = focus;


    // Set viewport to pip, then render world inside it
    SDL_RenderSetViewport(renderer, &pip);
    SDL_RenderSetScale(renderer, pipZoom, pipZoom);

    float visW = pip.w / pipZoom;
    float visH = pip.h / pipZoom;
    camX = focus.x - visW * 0.5f;
    camY = focus.y - visH * 0.5f;

    // Clamp to map bounds (reuse same logic as updateCamera)
    float mapW = map.cols * cfg::TileSize;
    float mapH = map.rows * cfg::TileSize;
    float maxCamX = std::max(0.0f, mapW - visW);
    float maxCamY = std::max(0.0f, mapH - visH);
    ...
    // Crosshair in PiP (center) - viewport is already pip, so use local coords
    SDL_RenderSetScale(renderer, 1.f, 1.f);
    ...
    // --- Binocs reveal label INSIDE PiP (pip-local coordinates) ---
    {
        // Pick a target near the binocs focus point
        int bestIdx = -1;
        float bestD = 999999.0f;

        const float snap = 60.0f;      // how close an actor must be to the focus point
        const float maxDist = 1100.0f; // max distance from player to identify

        for (int i = 0; i < (int)actors.size(); ++i) {
            const Actor& a = actors[i];
            if (!a.alive()) continue;

            float dc = length(a.pos - focus);
            if (dc > snap) continue;

            float dp = length(a.pos - player.pos);
            if (dp > maxDist) continue;

            if (dp < bestD) { bestD = dp; bestIdx = i; }
        }
        ...
        if (bestIdx >= 0) {
            ...
            // Draw at top-left of the PiP (pip-local coords)
            drawText(lab, 6, 6, cfg::ColUIAlt);
        }
    }



    // --- Cache PiP footprint rectangle in MAIN SCREEN SPACE (do NOT draw here) ---
    {
        float pipWorldW = pip.w / pipZoom;
        float pipWorldH = pip.h / pipZoom;

        float wx0 = focus.x - pipWorldW * 0.5f;
        float wy0 = focus.y - pipWorldH * 0.5f;
        float wx1 = wx0 + pipWorldW;
        float wy1 = wy0 + pipWorldH;

        // Project using the MAIN camera/zoom (saved values), not the PiP camera
        float sx0 = (wx0 - saveCamX) * saveZoom;
        float sy0 = (wy0 - saveCamY) * saveZoom;
        float sx1 = (wx1 - saveCamX) * saveZoom;
        float sy1 = (wy1 - saveCamY) * saveZoom;

        hasBinoFootprint = true;
        binoFootprintScreen = SDL_FRect{ sx0, sy0, (sx1 - sx0), (sy1 - sy0) };
    }
    ...
    SDL_RenderSetViewport(renderer, nullptr);
    SDL_RenderSetScale(renderer, saveZoom, saveZoom);
    camX = saveCamX; camY = saveCamY; zoom = saveZoom;

    // Tether line (draw in main view space)
    SDL_RenderSetScale(renderer, zoom, zoom);
    SDL_SetRenderDrawColor(renderer, 255, 255, 255, 80);
    SDL_RenderDrawLineF(renderer,
        player.pos.x - camX, player.pos.y - camY,
        focus.x - camX, focus.y - camY);

    // Return to screen-space defaults for any later overlay draws
    SDL_RenderSetViewport(renderer, nullptr);
    SDL_RenderSetScale(renderer, 1.f, 1.f);

}

void Game::drawBinoFogBoundary() {
    if (!binoHeld || mapOverlay) return;

    // This boundary represents what the player would normally see (zoomNormal)
    float fogZoom = binoHeld ? zoom : zoomNormal;
    float visWn = cfg::ScreenW / fogZoom;
    float visHn = cfg::ScreenH / fogZoom;


    // Normal viewport rectangle in WORLD space around current camera center
    // Camera top-left is camX/camY at current zoom, but we want the "normal zoom" box centered on player.
    Vec2 center = player.pos;
    float nx0 = center.x - visWn * 0.5f;
    float ny0 = center.y - visHn * 0.5f;
    float nx1 = nx0 + visWn;
    float ny1 = ny0 + visHn;

    // Convert normal box to SCREEN (using current camX/camY + current zoom)
    float sx0 = (nx0 - camX) * zoom;
    float sy0 = (ny0 - camY) * zoom;
    float sx1 = (nx1 - camX) * zoom;
    float sy1 = (ny1 - camY) * zoom;

    // Clamp to screen
    int ix0 = (int)std::floor(std::clamp(sx0, 0.0f, (float)cfg::ScreenW));
    int iy0 = (int)std::floor(std::clamp(sy0, 0.0f, (float)cfg::ScreenH));
    int ix1 = (int)std::ceil(std::clamp(sx1, 0.0f, (float)cfg::ScreenW));
    int iy1 = (int)std::ceil(std::clamp(sy1, 0.0f, (float)cfg::ScreenH));

    // Draw fog outside that rect
    SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_BLEND);

    // green-ish fog into void
    SDL_SetRenderDrawColor(renderer, 20, 60, 25, 170);

    SDL_Rect top{ 0, 0, cfg::ScreenW, iy0 };
    SDL_Rect bot{ 0, iy1, cfg::ScreenW, cfg::ScreenH - iy1 };
    SDL_Rect left{ 0, iy0, ix0, iy1 - iy0 };
    SDL_Rect right{ ix1, iy0, cfg::ScreenW - ix1, iy1 - iy0 };
    ...
    // Boundary outline
    SDL_SetRenderDrawColor(renderer, 200, 255, 200, 110);
    SDL_Rect box{ ix0, iy0, ix1 - ix0, iy1 - iy0 };
    SDL_RenderDrawRect(renderer, &box);
}
...
void Game::render() {
    ...
    // Map overlay (8A) - ensure screen-space state first (PiP may have changed viewport/scale)
    SDL_RenderSetViewport(renderer, nullptr);
    SDL_RenderSetScale(renderer, 1.f, 1.f);
    if (mapOverlay) drawMapOverlay();

    // Fog mask (screen space)
    SDL_RenderSetViewport(renderer, nullptr);
    SDL_RenderSetScale(renderer, 1.f, 1.f);
    if (binoHeld) drawBinoFogBoundary();


    // PiP on top (it "deletes" fog by overwriting)
    SDL_RenderSetViewport(renderer, nullptr);
    SDL_RenderSetScale(renderer, 1.f, 1.f);
    if (binoHeld) drawBinocularsPip();

    // HUD + crosshair last
    SDL_RenderSetViewport(renderer, nullptr);
    SDL_RenderSetScale(renderer, 1.f, 1.f);
    drawHUD();
    drawHoverReveal();     // <-- PATCH 9F (draw before crosshair so crosshair stays top)
    drawCrosshair();
    drawADSWeaponHUD(); // <--- PATCH 9E
    drawBarks();
