...
struct LootDrop {
    ...
    // --- NEW: full persistent weapon instance (future-proof)
    WeaponInstance inst{};
    bool hasInst = false;

    // --- Legacy fields (keep for now so existing loot code still works)
    WeaponId wid = WeaponId::None;
    int ammoLoose = 0;   // bullets added to reserve
    int magAmmo = 0;   // ammo inside the dropped weapon mag

    int srcTeam = 0;     // keep as int to avoid Faction visibility issues
    bool taken = false;
};
    ...
    for (int idx = 0; idx < (int)actors.size(); ++idx) {
        ...
        if (lootMode && playerPresent && player.alive()) {
            ...
            if (lootIdx >= 0 && lootIdx < (int)lootDrops.size()) {
                ...
                float cy = cfg::ScreenH * 0.65f;

                SDL_FRect left{ cx - 190, cy - 40, 120, 34 };   // wider + taller
                SDL_FRect right{ cx + 70,  cy - 40, 120, 34 };

                SDL_RenderDrawRectF(renderer, &left);
                SDL_RenderDrawRectF(renderer, &right);

                // --- PATCH 9G: weapon + ammo text inside loot silhouettes ---
                {
                    // Player primary weapon
                    std::string pName = weaponName(player.weapon.id);

                    char pAmmo[64];
                    std::snprintf(pAmmo, sizeof(pAmmo), "M%d R%d", player.weapon.magAmmo, player.weapon.reserveAmmo);

                    // Text positions inside the small rects
                    int px = (int)left.x + 4;
                    int py = (int)left.y + 2;

                    drawText(pName, px, py, cfg::ColUIAlt);
                    drawText(pAmmo, px, py + 12, cfg::ColUI);
                }
                {
                    // Corpse weapon (from loot instance, guaranteed valid by your setup above)
                    std::string cName = weaponName(d.inst.id);

                    char cAmmo[64];
                    std::snprintf(cAmmo, sizeof(cAmmo), "M%d R%d", d.inst.magAmmo, d.inst.reserveAmmo);

                    int cx2 = (int)right.x + 4;
                    int cy2 = (int)right.y + 2;

                    drawText(cName, cx2, cy2, cfg::ColUIAlt);
                    drawText(cAmmo, cx2, cy2 + 12, cfg::ColUI);
                }
    ...
    while (SDL_PollEvent(&e)) {
        ...
        case SDL_KEYDOWN: {
            ...
            case SDLK_1: {
                ...
                // Swap primary (same behavior as Shift+E)
                LootDrop back;
                back.pos = d.pos;
                back.inst = player.weapon;
                back.inst.id = player.weapon.id;
                back.hasInst = true;
                ensureWeaponIdentity(back.inst);

                back.srcTeam = (int)player.team;
                back.taken = false;
                syncLootLegacyFromInst(back);

                WeaponInstance picked = d.inst;
                applyWeaponInstance(player, picked.id);
                player.weapon.magAmmo = picked.magAmmo;
                player.weapon.reserveAmmo = picked.reserveAmmo;
                player.gun.inMag = player.weapon.magAmmo;

                player.weapon.uid = picked.uid;
                player.weapon.visualSeed = picked.visualSeed;
                ensureWeaponIdentity(player.weapon); // safety
            ...
            case SDLK_2: {
                ...
                // 11B will define the lootMode meaning for 2 (swap secondary)
            } break;


            case SDLK_f: {
                if (!lootMode) break;
                // v1: same as tapping E (ammo-only), but without leaving menu
                ...
                int gain = irand(10, 22); // slightly bigger because deliberate action
            ...
            if (d < (a.w * 0.5f + 2.f))
            {
                ...
                if (a.hp <= 0)
                {
                    ...
                    // --- Loot drop: weapon + ammo
                    {
                        ...
                        const WeaponDef& wd = weaponDef(d.wid);
                        d.magAmmo = std::clamp(a.weapon.magAmmo, 0, wd.magSize);
                        d.ammoLoose = ammoRollForTeam((int)a.team);

                        // New: snapshot full instance (persistent)
                        d.inst = a.weapon;
                        ensureWeaponIdentity(d.inst);
