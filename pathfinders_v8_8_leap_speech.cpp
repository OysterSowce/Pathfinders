...
static inline void BuildSpeechTokens(AudioEvent& e, const char* text)
{
    ...
    float base = 195.f;
    if (e.faction == 0) base = 175.f; // Axis deeper
    else if (e.faction == 1) base = 205.f; // Allies
    else if (e.faction == 2) base = 195.f; // Rebels
    else if (e.faction == 3) base = 225.f; // Militia higher
    if (yell) base *= 1.10f;
    ...
    auto flushWord = [&]() {
        ...
        for (int i = 0; i < phN && e.speechN < MAX; i++) {
            ...
            int durMs = 60;
            if (PhIsVowel(p)) durMs = (stress ? 120 : 95);
            else if (PhIsStop(p)) durMs = 45;
            else if (PhIsFric(p)) durMs = 70;
            else if (PhIsNasal(p)) durMs = 80;
            else durMs = 60;
    ...
    void render(float* outL, float* outR, int frames, int sr) override {
        ...
        for (int i = 0; i < frames; i++) {
            ...
            if (p == Ph::PAU || p == Ph::SIL || tokI >= tokN) {
                ...
                float burst = 0.0f;
                if (PhIsStop(p)) {
                    // a little burst early
                    if (u < 0.22f) burst = (1.0f - u / 0.22f);
                }
                ...
                float fric = (nBP1.tick(noise) * 0.55f + nBP2.tick(noise) * 0.45f);
                ...
                if (PhIsVowel(p)) {
                    ...
                    vMix = voicedOn ? 0.25f : 0.05f;
                    nMix = nmix;
                }
                else if (PhIsStop(p)) {
                    vMix = voicedOn ? 0.55f : 0.10f;
                    nMix = 0.35f + 0.40f * burst;
                }
                else if (PhIsNasal(p)) {
                    vMix = 0.75f;
                    nMix = 0.10f;
                ...
                if (PhIsStop(p) && burst > 0.0f) {
                    form = form * (0.78f) + fric * (0.22f + 0.35f * burst);
                }
