...
void Game::updateCamera(float dt) {
    ...
    // Base camera center is player
    Vec2 camCenter = player.pos;

    // ADS: pull camera slightly toward mouse aim point
    if (adsHeld && playerPresent && player.alive()) {
        ...
        Vec2 toAim = aim - player.pos;

        // clamp the influence so the camera doesn't fly across the map
        float d = length(toAim);
        if (d > 1.0f) {
            float maxInfluence = 520.0f; // pixels in world space
            float t = std::min(1.0f, d / maxInfluence);
            camCenter = player.pos + toAim * (pull * t);
        }
    }
...
void Game::drawCrosshair() {
    ...
    // --- Binoculars cursor override ---
    if (binoHeld) {
        // Draw a simple binocular reticle at the mouse position (screen space)
        int mx = mouseX;
        int my = mouseY;
        ...
        // Optional: small outer box so it feels distinct from ADS
        SDL_Rect box{ mx - 6, my - 6, 12, 12 };
        SDL_RenderDrawRect(renderer, &box);

        return; // IMPORTANT: skip weapon spread crosshair
    }
