...
static inline void pushMuzzleFlash(std::vector<MuzzleFlash>& flashes,
    const Vec2& muzzlePos, const Vec2& dir,
    const WeaponDef& wd)
{
    MuzzleFlash mf;
    mf.pos = muzzlePos;
    mf.dir = normalize(dir);

    // Short lifetime (tight + punchy)
    mf.ttl = 0.055f;

    // Scale by “loudness”
    mf.size = 5.0f + std::clamp(wd.baseDamage * 0.04f, 0.0f, 5.0f);
    if (wd.pellets > 1) mf.size += 3.0f; // shotguns pop

    // Thickness for autos
    mf.passes = (wd.fireCooldownS > 0.0f && wd.fireCooldownS < 0.12f) ? 3 : 2;

    flashes.push_back(mf);
}

static inline void pushImpact(std::vector<ImpactFX>& impacts,
    const Vec2& pos, const Vec2& dir,
    bool flesh, WeaponId wid)
{
    // Cap to avoid runaway spam
    if (impacts.size() > 420) {
        impacts.erase(impacts.begin(), impacts.begin() + 60);
    }

    // Shotgun/pellet spam control: fewer FX when weapon has lots of pellets
    int pellets = 1;
    if (wid != WeaponId::None) pellets = std::max(1, weaponDef(wid).pellets);
    if (pellets > 1) {
        // ~1/3 chance per pellet hit to emit an FX
        if (irand(0, 2) != 0) return;
    }

    ImpactFX fx;
    fx.pos = pos;
    fx.dir = normalize(dir);
    if (length(fx.dir) < 0.001f) fx.dir = Vec2{ 1,0 };
    fx.flesh = flesh;

    fx.ttl = flesh ? 0.22f : 0.18f;
    fx.size = flesh ? 7.0f : 6.0f;

    impacts.push_back(fx);
}

static inline void pushCasing(std::vector<CasingFX>& casings,
    const Vec2& pos,
    const Vec2& facing,
    CasingKind kind,
    bool ping)
{
    if (casings.size() >= 600) {
        casings[0] = casings.back();   // overwrite oldest slot
        casings.pop_back();            // remove last
    }
        ...
        if (hasThreat && seesThreat) {
            ...
            if (weaponCanFire(a)) {
                ...
                auto fireOneShot = [&]() {
                    ...
                    pushMuzzleFlash(muzzleFlashes, muzzle, dir0, wd);
                    ...
                    if (et == EjectType::SemiAuto) {
                        pushCasing(casings, weaponEjectPos(a, wd), dir0, CasingKind::Brass, false);
                    }
                    else if (et == EjectType::ShotgunPump) {
                        pushCasing(casings, weaponEjectPos(a, wd), dir0, CasingKind::Shell, false);
                    }
                    ...
                    if (et == EjectType::GarandClip && beforeMag > 0 && a.weapon.magAmmo == 0) {
                        ...
                        pushCasing(casings, weaponEjectPos(a, wd), dir0, CasingKind::Clip, true);
...
// -----------------------------------------------------------
    ...
    for (int i = (int)muzzleFlashes.size() - 1; i >= 0; --i) {
        muzzleFlashes[i].ttl -= dt;
        if (muzzleFlashes[i].ttl <= 0.0f) {
            muzzleFlashes.erase(muzzleFlashes.begin() + i);
        }
    }

    // Update impact puffs
    for (int i = (int)impacts.size() - 1; i >= 0; --i) {
        ImpactFX& fx = impacts[i];
        fx.ttl -= dt;
        fx.age += dt;
        if (fx.ttl <= 0.0f) impacts.erase(impacts.begin() + i);
    }

    //erase casings
    int w = 0;
    for (int r = 0; r < (int)casings.size(); ++r) {
        CasingFX c = casings[r];
        c.ttl -= dt;
    ...
    auto updatePendingEject = [&](Actor& a, float dt) {
        ...
        if (a.pendingEjectS <= 0.0f) {
            ...
            for (int i = 0; i < a.pendingEjectCount; ++i) {
                ...
                pushCasing(casings, weaponEjectPos(a, wd), a.facing, a.pendingEjectKind, a.pendingEjectPing);
            }
...
// Semi-auto = must click again (mousePressedL). Auto = can hold (mouseDownL).
        if (!binoHeld && mode == Mode::Player) {
            ...
            if (pw.id != WeaponId::None) {
                ...
                if (trigger && weaponCanFire(player)) {
                    ...
                    // Eject per shot (semi/auto + pump shotguns)
                    if (et == EjectType::SemiAuto) {
                        pushCasing(casings, weaponEjectPos(player, wd), dir0, CasingKind::Brass, false);
                    }
                    else if (et == EjectType::ShotgunPump) {
                        pushCasing(casings, weaponEjectPos(player, wd), dir0, CasingKind::Shell, false);
                    }
                    else if (et == EjectType::BoltAction) {
                        // delayed: bolt lift/throw
                    ...
                    if (et == EjectType::GarandClip && beforeMag > 0 && pw.magAmmo == 0) {
                        // eject clip + "ping" bark
                        pushCasing(casings, weaponEjectPos(player, wd), dir0, CasingKind::Clip, true);
    ...
    for (int bi = 0; bi < (int)bullets.size(); ++bi)
    {
        ...
        if (b.traveled > b.maxRange || bulletHitsSolid(b)) {
            ...
            pushImpact(impacts, b.pos, b.dir, false, b.wid);
        ...
        if (playerPresent && player.alive() && areEnemies(b.src, player.team))
        {
            ...
            if (d < (player.w * 0.5f + 2.f))
            {
                ...
                pushImpact(impacts, b.pos, b.dir, true, b.wid);
