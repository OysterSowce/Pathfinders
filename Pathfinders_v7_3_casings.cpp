...
// -----------------------------------------------------------
    ...
    if (playerPresent && player.alive()) {
        ...
        if (mouseDownL && !binoHeld && mode == Mode::Player && weaponCanFire(player)) {
            WeaponInstance& pw = activeW(player);
            if (pw.id == WeaponId::None) {
                // Nothing equipped; should be rare, but safe.
            }
            else {
                ...
                // Wounds worsen spread
                if (player.armWoundS > 0.0f) spreadRad *= 1.6f;

                // --- ADS first-shot accuracy boost ---
                if (adsHeld && adsFirstShotReady) {
                    // tune: 0.35 = strong first-shot “settle”
                    spreadRad *= 0.35f;
                }
                ...
                for (int p = 0; p < pelletCount; ++p) {
                    ...
                    // IMPORTANT: use ACTIVE weapon id (not player.weapon.id)
                    b.wid = pw.id;
                ...
                // Mirror legacy gun to ACTIVE weapon so any old logic/UI stays consistent
                player.gun.inMag = pw.magAmmo;
                player.gun.fireTimer = pw.fireTimer;

                // Mission stats + world response
                mission.shotsFired++;
                sounds.push_back({ player.pos, weaponNoiseRadiusPx(pw.id), cfg::HearDecayS * 0.7f });
                raiseAlarm(1);

                // Consume first-shot bonus once we fire
                if (adsHeld && adsFirstShotReady) adsFirstShotReady = false;

                // Optional auto-reload behavior (commented — enable if you like the feel)
                // if (pw.magAmmo <= 0 && pw.reserveAmmo > 0) weaponStartReload(player);
            }
        }
