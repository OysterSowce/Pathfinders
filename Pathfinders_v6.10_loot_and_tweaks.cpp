...
struct LootDrop {
    ...
    Faction srcFaction = Faction::Militia;
    bool taken = false;
};
    ...
    while (SDL_PollEvent(&e)) {
        ...
        case SDL_KEYDOWN: {
            ...
            case SDLK_e: {
                ...
                // Same weapon as player: ammo-only pickup (small amount)
                if (d.wid == player.weapon.id) {
                    int available = d.ammoLoose + d.magAmmo;
                    int gain = std::min(available, irand(6, 14)); // "small top-up"
                    player.weapon.reserveAmmo += gain;

                    d.taken = true;

                    if (barksEnabled) {
                        barks.push_back({ player.pos, "Picked up ammo.", 1.0f });
                    }
                    break;
                }

                // Different weapon: swap (Stalker rules)
                {
                    // Create a loot drop out of the player's current weapon
                    LootDrop back;
                    back.pos = d.pos;
                    back.wid = player.weapon.id;
                    back.srcFaction = player.team; // or leave default if you removed srcFaction

                    const WeaponDef& cur = weaponDef(back.wid);
                    back.magAmmo = std::clamp(player.weapon.magAmmo, 0, cur.magSize);
                    back.ammoLoose = std::max(0, player.weapon.reserveAmmo);

                    // Equip the corpse’s weapon
                    applyWeaponInstance(player, d.wid);

                    // Fill mag from dropped mag first
                    int magFill = std::min(wd.magSize, d.magAmmo);
                    player.weapon.magAmmo = magFill;

                    // Remaining ammo from drop → reserve
                    int remain = std::max(0, (d.magAmmo + d.ammoLoose) - magFill);
                    player.weapon.reserveAmmo = remain;

                    // keep legacy gun bridge in sync
                    player.gun.inMag = player.weapon.magAmmo;

                    // Replace the loot with the player's old weapon
                    d = back;

                    if (barksEnabled) {
                        barks.push_back({ player.pos, "Weapon swapped.", 1.0f });
                    }
                }
            } break;
            ...
            if (d < (a.w * 0.5f + 2.f))
            {
                ...
                if (a.hp <= 0)
                {
                    ...
                    // --- Loot drop: weapon + ammo
                    {
                        ...
                        d.pos = a.pos;
                        d.wid = a.weapon.id;
                        d.srcFaction = a.team; // your Actor uses 'team' as faction
