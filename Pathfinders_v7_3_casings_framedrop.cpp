...
static inline void pushCasing(std::vector<CasingFX>& casings,
    ...
    bool ping)
{
    if (casings.size() > 600) casings.erase(casings.begin(), casings.begin() + 100);
...
static WeaponId gPlayerStartWeapon = WeaponId::M1_CARBINE;
...
static WeaponId gPlayerStartWeapon2 = WeaponId::SMLE; // choose something valid in your table
...
// -----------------------------------------------------------
    ...
    for (int i = (int)casings.size() - 1; i >= 0; --i) {
        casings[i].ttl -= dt;
        casings[i].pos = casings[i].pos + casings[i].vel * dt;
        casings[i].vel = casings[i].vel * 0.92f;
        if (casings[i].ttl <= 0.0f) casings.erase(casings.begin() + i);
    }
