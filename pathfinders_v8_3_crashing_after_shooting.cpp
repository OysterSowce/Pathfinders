    ...
    bool ping)
{
    ...
    // Hard cap WITHOUT erase-shifts
    if ((int)casings.size() >= MAX) {
        // overwrite a random slot to keep variety, avoids always nuking the oldest
        int idx = irand(0, MAX - 1);
        casings[idx] = casings.back();
        casings.pop_back();
        // now size is MAX-1, so push_back returns to MAX
    }
...
static float gRightX = 1, gRightY = 0; // listener right vector
...
struct StepVoice : public AVoice {
    ...
    StepVoice(float speedNorm) {
        ...
        T = 0.11f;
        gain = 0.14f + 0.10f * speedNorm; //much gentler
    }
    void render(float* outL, float* outR, int frames, int sr) override {
        float dt = 1.0f / sr;
        for (int i = 0; i < frames; i++) {
...
static void Audio_Spawn(const AudioEvent& e) {
    float pan = 0, gain = 1;
    if (!Audio_Spatialize(e.x, e.y, e.maxAudible, pan, gain)) return;
    ...
    //per-voice gain multiply
    float g = gBusGain[(int)MixBus::Master] * gBusGain[(int)v->bus];
    v->gain *= g;
        ...
        if (hasThreat && seesThreat) {
            ...
            if (weaponCanFire(a)) {
                ...
                auto fireOneShot = [&]() {
                    ...
                    // 🔊 v8 audio
                    Audio_PostFire((int)wid, muzzle.x, muzzle.y, (int)a.team, weaponNoiseRadiusPx(wid));
    ...
    while (SDL_PollEvent(&e)) {
        ...
        case SDL_KEYDOWN: {
            ...
            case SDLK_x:
                ...
                //case SDLK_1: paintBrush = 1; break;mix_bump
...
// -----------------------------------------------------------
    ...
    //swap-remove casings
    for (int i = 0; i < (int)casings.size(); ) {
        CasingFX& c = casings[i];

        c.ttl -= dt;
        if (c.ttl <= 0.0f) {
            casings[i] = casings.back();
            casings.pop_back();
            continue; // do not i++
        }
