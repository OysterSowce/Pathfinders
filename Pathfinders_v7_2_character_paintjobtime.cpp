...
void Game::drawProcCharacter(const Actor& a, SDL_Color col) {
    ...
    drawOrientedBoxLines(renderer, a.pos, tf, tr, torsoL, torsoW, camX, camY);
    ...
    drawOrientedBoxLines(renderer, packC, tf, tr, 8.0f, 7.0f, camX, camY);
    ...
    Vec2 handR = weapBase + wf * 2.0f + wr * 1.5f;
    Vec2 handL = weapBase + wf * 6.0f - wr * 1.0f;

    // arms (two-hand inference)
    setDraw(renderer, SDL_Color{ 210,210,210,255 });
    auto armLine = [&](Vec2 A, Vec2 B) {
        SDL_RenderDrawLineF(renderer, A.x - camX, A.y - camY, B.x - camX, B.y - camY);
        Vec2 d = normalize(B - A);
        Vec2 n{ -d.y, d.x };
        SDL_RenderDrawLineF(renderer, A.x - camX + n.x, A.y - camY + n.y, B.x - camX + n.x, B.y - camY + n.y);
        };
    armLine(shR, handR);
    armLine(shL, handL);
    ...
    SDL_RenderDrawLineF(renderer, weapBase.x - camX, weapBase.y - camY, muzzle.x - camX, muzzle.y - camY);
