// Pathfinders_v7.cpp
// Refining actors and FABRIK starting
...
void Game::drawProcCharacter(const Actor& a, SDL_Color col) {
    ...
    setDraw(renderer, SDL_Color{ 235,235,235,255 });
    ...
    // Right-handed feel: trigger hand sits to the weapon-right, support hand to weapon-left, further forward
    Vec2 handR = weapBase + wf * 1.6f + wr * 2.2f;   // trigger hand (right side, closer)
    Vec2 handL = weapBase + wf * 7.2f - wr * 1.6f;   // support hand (left side, further)


    // arms (two-hand inference)
    setDraw(renderer, SDL_Color{ 210,210,210,255 });
    auto armRect = [&](Vec2 A, Vec2 B, float w) {
        Vec2 d = B - A;
        float L = length(d);
        if (L < 0.01f) return;
        Vec2 f = d * (1.0f / L);
        Vec2 r{ -f.y, f.x };
        Vec2 c = (A + B) * 0.5f;
        fillOrientedBox(renderer, c, f, r, L, w, camX, camY);
        };

    // arms: filled bars
    setDraw(renderer, armCol); // (we’ll define armCol in Part C5)
    armRect(shR, handR, 3.0f);
    armRect(shL, handL, 3.0f);
