...
void Game::drawCrosshair() {
    ...
    // --- Binoculars cursor override: footprint rectangle only ---
    if (binoHeld) {
        if (hasBinoFootprint) {
            SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_BLEND);
        ...
        return; // IMPORTANT: no weapon spread crosshair while binos held
    }
...
// -----------------------------------------------------------
    ...
    if (playerPresent && player.alive()) {
        ...
        // --- Face where we aim (mouse) ---
        if (playerPresent && player.alive()) {
            Vec2 aimDir = aimDirFromMouse(player.pos, worldMouseX, worldMouseY);

            // Optional: if binoculars are held, don't rotate player to mouse (keeps it fair/grounded)
            if (!binoHeld) {
    ...
    // Keep world mouse in sync even if cam/zoom changed without mouse motion
    worldMouseX = int(camX + mouseX / zoom);
    worldMouseY = int(camY + mouseY / zoom);
...
void Game::drawBinocularsPip() {
    ...
    // --- Draw PiP footprint rectangle on main view ---
    {
        ...
        // project using main cam + zoom
        float sx0 = (wx0 - camX) * zoom;
        float sy0 = (wy0 - camY) * zoom;
        float sx1 = (wx1 - camX) * zoom;
        float sy1 = (wy1 - camY) * zoom;
        ...
        SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_BLEND);
        SDL_SetRenderDrawColor(renderer, 255, 255, 255, 140);
        SDL_Rect r{ (int)sx0, (int)sy0, (int)(sx1 - sx0), (int)(sy1 - sy0) };
        SDL_RenderDrawRect(renderer, &r);
    }
...
void Game::drawBinoFogBoundary() {
    ...
    float visWn = cfg::ScreenW / zoomNormal;
    float visHn = cfg::ScreenH / zoomNormal;
...
void Game::render() {
    ...
    // 8C: binocular PiP (renders a second view)
    if (binoHeld) {
        drawBinocularsPip();
    }
