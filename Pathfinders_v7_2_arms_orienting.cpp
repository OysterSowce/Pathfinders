...
void Game::drawHeldWeapon(const Actor& a) {
    ...
    // Derive silhouette dimensions from weapon character
    float len = 10.0f + std::clamp(wd.maxRange * 0.020f, 0.0f, 18.0f);     // 10..28ish
    float thick = 2.0f + std::clamp(wd.baseDamage * 0.030f, 0.0f, 4.0f);   // 2..6ish
    ...
    // Shotguns chunkier + slightly longer feel
    if (wd.pellets > 1) {
        thick += 1.0f;
        len += 2.0f;
    }

    // Anchor weapon near "hands" (front-ish of body)
    Vec2 muzzleBase = a.pos + fwd * (a.w * 0.30f);

    // Slight lateral offset so it doesn't sit dead-center on the torso
    muzzleBase = muzzleBase + right * (a.h * 0.10f);

    // Shapes: barrel line, receiver block, stock block, optional mag block
    Vec2 barrelStart = muzzleBase;
    Vec2 barrelEnd = barrelStart + fwd * len;

    // Color: dark metal (simple, readable on any faction)
    SDL_SetRenderDrawColor(renderer, 25, 25, 25, 255);

    // --- “thick line” barrel by drawing multiple parallel lines
    int passes = (int)std::round(thick);
    passes = std::clamp(passes, 2, 7);
    float half = (passes - 1) * 0.5f;
    for (int i = 0; i < passes; ++i) {
        float off = (i - half) * 0.8f; // spacing between parallel lines
        Vec2 o = right * off;
    ...
    // --- Receiver (small block near start)
    ...
    // --- Magazine hint for autos (fast weapons) and LMG-ish (bigger mag sizes)
...
void Game::drawProcCharacter(const Actor& a, SDL_Color col) {
    ...
    float weapLen = 14.0f;
    WeaponId wid = a.weapon.id; // draw what the actor is holding
    if (wid != WeaponId::None) {
        const WeaponDef& wd = weaponDef(wid);
        weapLen = heldWeaponLen(wd);
    }
    Vec2 muzzle = weapBase + wf * weapLen;


    // hand anchors on weapon
    // Hand anchors: right-handed stance, but support hand moves further forward on long weapons
    float supportF = std::clamp(0.55f * weapLen, 7.0f, 15.0f); // scales with weapon length
    float triggerF = std::clamp(0.18f * weapLen, 1.5f, 4.0f);

    Vec2 handR = weapBase + wf * triggerF + wr * 2.2f;     // trigger hand stays fairly close
    Vec2 handL = weapBase + wf * supportF - wr * 1.6f;     // support hand rides forward on long guns



    // --- FABRIK arms (8 segments) ---
    ...
    Chain2D armR, armL;
    const int segs = 8;
    const float segLen = 2.6f; // tune 2.2–3.0

    // Give FABRIK an initial bend bias so elbows don't collapse into 90° kinks
    Vec2 initR = normalize(wf + wr * 0.55f);  // right elbow can flare out
    Vec2 initL = normalize(wf - wr * 0.35f);  // left elbow tucks in slightly
    chainInit(armR, segs, segLen, shR, initR);
    chainInit(armL, segs, segLen, shL, initL);


    // solve to hand targets
    fabrikSolve(armR, shR, handR, 7, 0.35f);
    fabrikSolve(armL, shL, handL, 7, 0.35f);

    auto nudgeElbow = [&](Chain2D& c, float sign, float amount) {
        int mid = (int)c.p.size() / 2; // middle joint index
        // Nudge elbow backward along -wf and sideways along wr to shape the bend
        c.p[mid] = c.p[mid] - wf * (amount * 0.6f) + wr * (sign * amount);
        };

    nudgeElbow(armL, -1.0f, 1.2f); // left elbow back/tucked
    nudgeElbow(armR, +1.0f, 0.8f); // right elbow out a bit


    // draw as filled boxes
    drawChainBoxes(armR, 3.0f, armCol);
    drawChainBoxes(armL, 3.0f, armCol);



    // weapon
    setDraw(renderer, SDL_Color{ 240,240,240,255 });
    setDraw(renderer, weapCol);
    {
        Vec2 d = muzzle - weapBase;
        float L = length(d);
        if (L > 0.01f) {
            Vec2 f = d * (1.0f / L);
            Vec2 r{ -f.y, f.x };
            Vec2 c = (weapBase + muzzle) * 0.5f;
            fillOrientedBox(renderer, c, f, r, L, 2.2f, camX, camY);
        }
    }
        ...
        if (hasThreat && seesThreat) {
            ...
            if (weaponCanFire(a)) {
                ...
                auto fireOneShot = [&]() {
                    ...
                    sounds.push_back({ a.pos, weaponNoiseRadiusPx(a.weapon.id), cfg::HearDecayS });
                    raiseAlarm(1);
                    };
